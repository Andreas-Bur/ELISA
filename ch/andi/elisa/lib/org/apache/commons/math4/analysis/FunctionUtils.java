...
public class FunctionUtils {
	 ...
	 * <p>
	 * This method handle the case with one free parameter and several
	 * derivatives. For the case with several free parameters and only first
	 * order derivatives, see
	 * {@link #toDifferentiable(MultivariateFunction, MultivariateVectorFunction)}.
	 * There are no direct support for intermediate cases, with several free
	 * parameters and order 2 or more derivatives, as is would be difficult to
	 * specify all the cross derivatives.
	 * </p>
	 * <p>
	 * Note that the derivatives are expected to be computed only with respect
	 * to the raw parameter x of the base function, i.e. they are df/dx,
	 * df<sup>2</sup>/dx<sup>2</sup>, ... Even if the built function is later
	 * used in a composition like f(sin(t)), the provided derivatives should
	 * <em>not</em> apply the composition with sine and its derivatives by
	 * themselves. The composition will be done automatically here and the
	 * result will properly contain f(sin(t)), df(sin(t))/dt,
	 * df<sup>2</sup>(sin(t))/dt<sup>2</sup> despite the provided derivatives
	 * functions know nothing about the sine function.
	 * </p>
	 * 
	 * @param f
	 *            base function f(x)
	 * @param derivatives
	 *            derivatives of the base function, in increasing
	 *            differentiation order
	 * @return a differentiable function with value and all specified
	 *         derivatives
	 ...
	 * <p>
	 * This method handle the case with several free parameters and only first
	 * order derivatives. For the case with one free parameter and several
	 * derivatives, see
	 * {@link #toDifferentiable(UnivariateFunction, UnivariateFunction...)}.
	 * There are no direct support for intermediate cases, with several free
	 * parameters and order 2 or more derivatives, as is would be difficult to
	 * specify all the cross derivatives.
	 * </p>
	 * <p>
	 * Note that the gradient is expected to be computed only with respect to
	 * the raw parameter x of the base function, i.e. it is df/dx<sub>1</sub>,
	 * df/dx<sub>2</sub>, ... Even if the built function is later used in a
	 * composition like f(sin(t), cos(t)), the provided gradient should
	 * <em>not</em> apply the composition with sine or cosine and their
	 * derivative by itself. The composition will be done automatically here and
	 * the result will properly contain f(sin(t), cos(t)), df(sin(t), cos(t))/dt
	 * despite the provided derivatives functions know nothing about the sine or
	 * cosine functions.
	 * </p>
	 * 
	 * @param f
	 ...
	 * <p>
	 * This converter is only a convenience method. Beware computing only one
	 * derivative does not save any computation as the original function will
	 * really be called under the hood. The derivative will be extracted from
	 * the full {@link DerivativeStructure} result.
	 * </p>
	 * 
	 * @param f
	 *            original function, with value and all its derivatives
	 * @param order
	 *            of the derivative to extract
	 ...
	 *            original function, with value and all its derivatives
	 * @param orders
	 *            of the derivative to extract, for each free parameters
	 * @return function computing the derivative at required order
	 * @see #derivative(UnivariateDifferentiableFunction, int)
	 * @see #toDifferentiable(MultivariateFunction, MultivariateVectorFunction)
	 */
