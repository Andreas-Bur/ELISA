 ...
 * number</a>. It allows to allocate a cache of precomputed values.
 */
public class FactorialDouble {
	 ...
	 *
	 * @param numValues
	 *            Number of values of the function to compute.
	 * @param cache
	 *            Cached values.
	...
	private FactorialDouble(int numValues, double[] cache) {
		...
		for (int i = 0, max = numValues < 2 ? numValues : 2; i < max; i++) {
			factorialsDouble[i] = 1;
		}

		final int beginCopy = 2;
		final int endCopy = cache == null || cache.length <= beginCopy ? beginCopy
				: cache.length <= numValues ? cache.length : numValues;
	 ...
	 *
	 * @param cacheSize
	 *            Number of precomputed values of the function.
	...
	public FactorialDouble withCache(final int cacheSize) {
		return new FactorialDouble(cacheSize, factorialsDouble);
	}

	/**
	 * Computes the factorial of {@code n}. The result should be small enough to
	 * fit into a {@code double}: The largest {@code n} for which {@code n!}
	 * does not exceed {@code Double.MAX_VALUE} is 170.
	 * {@code Double.POSITIVE_INFINITY} is returned for {@code n > 170}.
	 *
	 * @param n
	 *            Argument.
	 * @return {@code n!}
