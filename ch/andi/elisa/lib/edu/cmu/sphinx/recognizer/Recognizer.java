 ...
 * usage of a recognizer is like so: <code>
 ...
 *   }
 * </code> Note that some Recognizer methods may throw an IllegalStateException
 * if the recognizer is not in the proper state
 */
public class Recognizer implements Configurable, ResultProducer {
	 ...
	 *             if the recognizer is not in the <code>ALLOCATED</code> state
	 */
	 ...
	 * recognizer is in the <code> allocated
	 * </code> state.
	 *
	 * @throws IllegalStateException
	 *             if the recognizer is not in the <code>ALLOCATED</code> state
