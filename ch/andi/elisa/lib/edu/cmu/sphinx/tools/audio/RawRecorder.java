...
public class RawRecorder {
	...
	public RawRecorder(AudioFormat audioFormat) throws LineUnavailableException {
		...
		/*
		 * Some machines, such as my Mac OS X PowerBook, don't support a wide
		 * range of input formats. So...we may need to read data in using a
		 * different format and then resample to the desired format. Here, I'm
		 * just going to go for 44.1kHz 16-bit signed little endian data if the
		 * given audio format is not supported.
		 */
	...
	public short[] stop() {
		synchronized (lock) {
			...
			try {
				...
				if (downsample) {
					samples = Downsampler.downsample(samples, (int) (inFormat.getSampleRate() / 1000.0f),
