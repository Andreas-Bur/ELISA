...
/**
 * Copyright 1999-2002 Carnegie Mellon University. Portions Copyright 2002 Sun
 ...
 * <p>
 * User: Peter Wolf Date: Nov 10, 2005 Time: 2:42:06 PM Copyright 2005, Peter
 * Wolf
 * <p>
 * Runs a NIST corpus as used by the GALE project. The inputs are a CTL file,
 * and a REF file. The output is a CTM file.
 * <p>
 * A CTL file contains a list of utterances to decode. The format is
 * <p>
 * &lt;utterance file&gt; &lt;start offset&gt; &lt;end offset&gt; &lt;utterance
 * name&gt;
 * <p>
 * The &lt;utterance file&gt; is a base to which the property "dataDirectory" is
 * prepended, and ".raw" is appended. The utterance file should be raw PCM that
 * agrees with the "bitsPerSample", "channelCount", "samplesPerSecond", and
 * "framesPerSecond" properties.
 * <p>
 * The &lt;start offset&gt; and &lt;end offset&gt; are specified in frames,
 * where
 * <p>
 * bytesPerFrame =
 * (bitsPerSample/8)*channelCount*samplesPerSecond/framesPerSecond
 * <p>
 ...
 * <p>
 * The output is a "processed" CTM file. It is used by the NIST tools to compute
 * the performance on the copus. The format is not documented because it is
 * currently a hack to get the Dry Run going. We need to think more about it. If
...
public class BatchNISTRecognizer extends BatchModeRecognizer {
	...
	/**
	 * The property that specifies the file containing the corpus utterance
	 * audio
	 */
	...
	/**
	 * The property that specifies the file containing the transcripts of the
	 * corpus
	 */
	...
	/**
	 * The property that specifies the the directory where the output XXX files
	 * should be placed
	 */
	...
	/**
	 * The sphinx properties that specify the format of the PCM audio in the
	 * data file
	 */
	...
	public BatchNISTRecognizer(Recognizer recognizer, List<DataProcessor> inputDataProcessors, String ctlFile, String dataDir,
			String refFile, String ctmFile, int bitsPerSample, int samplesPerSecond, int framesPerSecond, int channelCount) {
	...
	public class CTLUtterance {
		...
		CTLUtterance(String ctl, String ref) throws CTLException {
			/*
			 * example line: 20040422_150000_NTDTV.80Hz-6400Hz 64155 65103
			 * 20040422_150000_NTDTV_64155-65103_spk8
			 */
			this.ref = ref;
	...
	private long dumpBestPath(DataOutputStream out, CTLUtterance utt, Token token) throws IOException {
		...
		if (token.isWord()) {
			...
			if (!spelling.startsWith("<")) {
				...
				out.write((names[0] + '_' + names[1] + '_' + names[2] + " 1 " + (utt.startOffset + startFrame) / 100.0 + ' '
						+ (endFrame - startFrame) / 100.0 + ' ').getBytes());
