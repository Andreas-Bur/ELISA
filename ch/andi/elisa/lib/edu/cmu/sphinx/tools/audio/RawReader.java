...
/**
 * Reads a raw audio file (i.e., a file that is just plain raw samples and
 * nothing else) and converts it to signed data.
 */
public class RawReader {

	/**
	 * Reads raw bytes from the given audioStream and returns samples based on
	 * the audioFormat.
	 *
	 * @param audioStream
	 *            the stream containing the raw bytes
	 * @param audioFormat
	...
	public static short[] readAudioData(InputStream audioStream, AudioFormat audioFormat) throws IOException {
		...
		/*
		 * Now read in the data, saving the samples in an array list. Along the
		 * way, convert each sample to little endian signed data.
		 */
		...
		while (!done) {
			...
			if (!done) {
				...
				/*
				 * We'll always give signed data. So, if the input is unsigned,
