 ...
 * <p>
 * Given a CTL file that specifies a series of audio and coresponding correct
 * transcripts, this utility creates a trivial grammar from the transcript, and
 * runs the recognizer on the utterance. The output is words with beginning and
 * end times.
 * <p>
 * See BatchNISTRecognizer for more information about the format of CTL and
 * audio files.
 */
...
public class BatchForcedAlignerRecognizer extends BatchNISTRecognizer {
	...
	public BatchForcedAlignerRecognizer(BatchForcedAlignerGrammar bfaGrammar, Recognizer recognizer,
			List<DataProcessor> inputDataProcessors, String ctlFile, String dataDir, String refFile, String ctmFile,
			int bitsPerSample, int samplesPerSecond, int framesPerSecond, int channelCount) {
		super(recognizer, inputDataProcessors, ctlFile, dataDir, refFile, ctmFile, bitsPerSample, samplesPerSecond,
				framesPerSecond, channelCount);
