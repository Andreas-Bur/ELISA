...
public class LiveModeRecognizer implements Configurable {
	...
	/**
	 * The property specifying the number of files to decode before alignment is
	 * performed.
	 */
	...
	public LiveModeRecognizer(Recognizer recognizer, ConcatFileDataSource dataSource, int skip, boolean showGapInsertions,
			String hypothesisFile, int alignInterval) {
	 ...
	 *
	 * @param argv
	 *            argv[0] : config file argv[1] : a file listing all the audio
	 *            files to decode
	 */
	...
	// LiveModeRecognizer liveRecognizer = (LiveModeRecognizer)
	// ConfigurationManager.getInstance(LiveModeRecognizer.class, props);
	//
	// }
}
