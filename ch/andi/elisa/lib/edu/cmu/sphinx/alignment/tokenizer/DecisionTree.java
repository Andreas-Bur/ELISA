 ...
 * final value. The decision nodes in the CART trees operate on an Item and have
 ...
 *
 * <p>
 * Where <code>feat</code> is an string that represents a feature to pass to the
 * <code>findFeature</code> method of an item.
 *
 * <p>
...
public class DecisionTree {
	...
	/**
	 * Entry in file represents a node. The format should be "NODE feat op val
	 * f" where 'feat' represents a feature, op represents an operand, val is
	 * the value, and f is the index of the node to go to is there isn't a
	 * match.
	 */
	 ...
	 * <a href="http://www.graphviz.org/">http://www.graphviz.org/</a>. If
	...
	public void dumpDot(PrintWriter out) {
		...
		for (Node n : cart) {
			out.println("\t\"node" + n.hashCode() + "\" [ label=\"" + n.toString() + "\", color=" + dumpDotNodeColor(n)
			...
			if (n instanceof DecisionNode) {
				...
				if (dn.qtrue < cart.length && cart[dn.qtrue] != null) {
					out.write("\t\"node" + n.hashCode() + "\" -> \"node" + cart[dn.qtrue].hashCode() + "\" [ label=" + "TRUE"
							+ " ]\n");
				}
				if (dn.qfalse < cart.length && cart[dn.qfalse] != null) {
					out.write("\t\"node" + n.hashCode() + "\" -> \"node" + cart[dn.qfalse].hashCode() + "\" [ label=" + "FALSE"
							+ " ]\n");
				}
			}
		}
	 ...
	 *
	 * @param type
	 *            <code>NODE</code> or <code>LEAF</code>
	 * @param tokenizer
	 *            the StringTokenizer containing the data to get
	 * @param currentNode
	...
	static class ComparisonNode extends DecisionNode {
		 ...
		 * IMPLEMENTATION NOTE: LESS_THAN and GREATER_THAN, the Node's value and
		 * the value passed in are converted to floating point values. For
		 * EQUAL, the Node's value and the value passed in are treated as String
		 * compares. This is the way of Flite, so be it Flite.
		 *
		 * @param val
	...
	static class MatchingNode extends DecisionNode {
		 ...
		 *
		 * @param val
		 *            the value to compare -- this must be a String
		 */
		public int getNextNode(Object val) {
