...
public class CharTokenizer implements Iterator<Token> {
	 ...
	 * @param buffer
	 *            the place to append characters of type charClass
	 *
	 ...
	 *
	 * @param charClass
	 *            the string of characters you want included or excluded in your
	 *            return
	 * @param containThisCharClass
	 *            determines if you want characters in charClass in the returned
	 *            string or not
	 *
	 * @return a string of characters from the current position until it
	 *         encounters characters in endingCharClass
	...
	private String getTokenByCharClass(String charClass, boolean containThisCharClass) {
		...
		// false.
		while ((charClass.indexOf(currentChar) != -1) == containThisCharClass && singleCharSymbols.indexOf(currentChar) == -1
				&& currentChar != EOF) {
	 ...
	 * those postpunctuation characters to the class variable 'postpunctuation'.
	 */
	...
	/**
	 * if hasErrors returns <code>true</code>, this will return a description of
	 * the error encountered, otherwise it will return <code>null</code>
	 *
		...
		if (lastToken == null || token == null) {
			...
			/* next word starts with a capital */
					...
					!(Character.isUpperCase(lastWord.charAt(lastWordLength - 1))
							|| (lastWordLength < 4 && Character.isUpperCase(lastWord.charAt(0))))) {
				return true;
			}
		}
		return false;
