...
public class BaumWelchLearner implements Learner {
	...
	private TrainerScore[][] prepareScore() {
		...
		for (initialNode.startOutgoingEdgeIterator(); initialNode.hasMoreOutgoingEdges();) {
		...
		for (finalNode.startIncomingEdgeIterator(); finalNode.hasMoreIncomingEdges();) {
	...
	public TrainerScore[] getScore() {
		...
		if (currentFeatureIndex >= 0) {
			...
			if (currentFeatureIndex == lastFeatureIndex - 1) {
				...
				if (Math.abs(totalLogScore - logScore) > Math.abs(totalLogScore)) {
					System.out.println("WARNING: log probabilities differ: " + totalLogScore + " and " + logScore);
				}
			}
			return score;
		} else {
	...
	private void forwardPass(TrainerScore[] score) {
		...
		for (int indexNode = 0; indexNode < graph.size(); indexNode++) {
			...
			for (node.startIncomingEdgeIterator(); node.hasMoreIncomingEdges();) {
				...
				probCurrentFrame[indexNode] = logMath.addAsLinear(probCurrentFrame[indexNode],
						probPreviousFrame[indexPreviousNode] + logTransitionProbability);
		...
		for (int indexNode = 0; indexNode < graph.size(); indexNode++) {
			...
			for (node.startIncomingEdgeIterator(); node.hasMoreIncomingEdges();) {
				...
				// state into the current alpha
				probCurrentFrame[indexNode] = logMath.addAsLinear(probCurrentFrame[indexNode],
						probCurrentFrame[indexPreviousNode] + logTransitionProbability);
				// System.out.println("State= " + indexNode + " curr "
				// + probCurrentFrame[indexNode] + " prev " +
				// probPreviousFrame[indexNode] + " trans " +
				// logTransitionProbability);
	...
	private void backwardPass(TrainerScore[] score) {
		...
		for (int indexNode = 0; indexNode < graph.size(); indexNode++) {
			...
			for (node.startOutgoingEdgeIterator(); node.hasMoreOutgoingEdges();) {
				...
				probCurrentFrame[indexNode] = logMath.addAsLinear(probCurrentFrame[indexNode],
						probNextFrame[indexNextNode] + logTransitionProbability + outputProbs[indexNextNode]);
			}
		...
		for (int indexNode = graph.size() - 1; indexNode >= 0; indexNode--) {
			...
			for (node.startOutgoingEdgeIterator(); node.hasMoreOutgoingEdges();) {
				...
				// from the next state into the current beta
				probCurrentFrame[indexNode] = logMath.addAsLinear(probCurrentFrame[indexNode],
						probCurrentFrame[indexNextNode] + logTransitionProbability);
			}
			// System.out.println("State= " + indexNode + " beta= " +
			// probCurrentFrame[indexNode]);
			score[indexNode].setBeta(probCurrentFrame[indexNode]);
	 ...
	 * = new FastActiveList();
	 * 
	 * // At this point we have only emitting states. We score // and prune them
	 ...
	 * transition.transitionprob; // transition.posterior =
	 * alpha*transition*beta / // totalprobability; double
	 * localtransitionbetascore = logtransition + logbeta +
	 * transition.destination.logscore; double transition.posterior =
	 ...
	 * 
	 * // add link to the appropriate lists for source and // destination tokens
