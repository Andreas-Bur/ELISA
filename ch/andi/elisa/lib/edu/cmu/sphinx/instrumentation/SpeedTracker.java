...
public class SpeedTracker extends ConfigurableAdapter
		implements ResultListener, Resetable, StateListener, SignalListener, Monitor {
	...
	/**
	 * The property that defines whether summary accuracy information is
	 * displayed
	 */
	...
	/**
	 * The property that defines whether detailed accuracy information is
	 * displayed
	 */
	@S4Boolean(defaultValue = true)
	...
	/**
	 * The property that defines whether detailed response information is
	 * displayed
	 */
	...
	public SpeedTracker(Recognizer recognizer, FrontEnd frontEnd, boolean showSummary, boolean showDetails,
			boolean showResponseTime, boolean showTimers) {
		initLogger();
	 ...
	 *
	 * @see
	 * edu.cmu.sphinx.decoder.ResultListener#newResult(edu.cmu.sphinx.result.
	 * Result)
	 */
	...
	protected void showAudioUsage() {
		logger.info("   This  Time Audio: " + timeFormat.format(audioTime) + "s" + "  Proc: " + timeFormat.format(processingTime)
				+ "s" + "  Speed: " + timeFormat.format(getSpeed()) + " X real time");
		showAudioSummary();
	}

	/** Shows the audio summary data */
	protected void showAudioSummary() {
		logger.info("   Total Time Audio: " + timeFormat.format(totalAudioTime) + "s" + "  Proc: "
				+ timeFormat.format(totalProcessingTime) + "s " + timeFormat.format(getCumulativeSpeed()) + " X real time");

		if (showResponseTime) {
			float avgResponseTime = (float) totalResponseTime / (numUtteranceStart * 1000);
			logger.info("   Response Time:  Avg: " + avgResponseTime + 's' + "  Max: " + ((float) maxResponseTime / 1000)
	 ...
	 *
	 * @see
	 * edu.cmu.sphinx.frontend.SignalListener#signalOccurred(edu.cmu.sphinx.
	 * frontend.Signal)
	 */
