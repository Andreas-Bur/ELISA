 ...
 * way that the filter bank mimics the critical band, which represents different
 * perceptual effect at different frequency bands. Additionally, the edges are
 * placed so that they coincide with the center frequencies in adjacent filters.
 * Pictorially, the filter bank looks like:
 * <p>
 * <img alt="Filterbank" src="doc-files/melfilterbank.jpg"> <br>
 * <center><b>Figure 1: A Mel-filter bank. </b> </center>
 * <p>
 * As you might notice in the above figure, the distance at the base from the
 * center to the left edge is different from the center to the right edge. Since
 * the center frequencies follow the mel-frequency scale, which is a non-linear
 * scale that models the non-linear human hearing behavior, the mel filter bank
 * corresponds to a warping of the frequency axis. As can be inferred from the
 * figure, filtering with the mel scale emphasizes the lower frequencies. A
 * common model for the relation between frequencies in mel and linear scales is
 * as follows:
 * <p>
 ...
 * 6800Hz that can be used for improving separation between models. Particularly
 * for very noisy channels, maximum frequency of around 5000Hz may help cut off
 * the noise.
 * <p>
 * Typical values for the constants defining the filter bank are:
 * <table summary="Filterbank Values" width="80%" border="1">
 * <tr>
 ...
 * </table>
 * <p>
 * Davis and Mermelstein showed that Mel-frequency cepstral coefficients present
 * robust characteristics that are good for speech recognition. For details, see
 * Davis and Mermelstein, <i>Comparison of Parametric Representations for
...
public class MelFrequencyFilterBank extends BaseDataProcessor {
	...
	private void buildFilterbank(int numberFftPoints, int numberFilters, double minFreq, double maxFreq)
		 ...
		 * the other half would be symmetrical for a real sequence -, and these
		 * points cover up to the Nyquist frequency, which is half of the
		 * sampling rate. The two "divide by 2" get canceled out.
		 */
		 ...
		 * that the center frequency of a filter is the right edge of the filter
		 * to its left, and the left edge of the filter to its right.
		 */
		if (numberFilters < 1) {
	 ...
	 * frame. Signals are returned unmodified.
	 *
