...
/**
 * Computes the PLP cepstrum from a given PLP Spectrum. The power spectrum has
 * the amplitude compressed by computing the cubed root of the PLP spectrum.
 * This operation is an approximation to the power law of hearing and simulates
 * the non-linear relationship between sound intensity and perceived loudness.
 * Computationally, this operation is used to reduce the spectral amplitude of
 * the critical band to enable all-pole modeling with relatively low order AR
 * filters. The inverse discrete cosine transform (IDCT) is then applied to the
 * autocorrelation coefficients. A linear prediction filter is then estimated
 * from the autocorrelation values, and the linear prediction cepstrum (LPC
 * cepstrum) is finally computed from the LP filter.
 *
...
public class PLPCepstrumProducer extends BaseDataProcessor {
	...
	/**
	 * Applies the intensity loudness power law. This operation is an
	 * approximation to the power law of hearing and simulates the non-linear
	 * relationship between sound intensity and percieved loudness.
	 * Computationally, this operation is used to reduce the spectral amplitude
	 * of the critical band to enable all-pole modeling with relatively low
	 * order AR filters.
	 * 
	 * @param inspectrum
	 */
	...
	/**
	 * Returns the next Data object, which is the PLP cepstrum of the input
	 * frame. However, it can also be other Data objects like a EndPointSignal.
	 *
	...
	private Data process(DoubleData input) throws IllegalArgumentException {
		...
		if (plpspectrum.length != numberPLPFilters) {
			throw new IllegalArgumentException("PLPSpectrum size is incorrect: plpspectrum.length == " + plpspectrum.length
					+ ", numberPLPFilters == " + numberPLPFilters);
		}
