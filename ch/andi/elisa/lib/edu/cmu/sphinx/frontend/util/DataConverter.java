...
/**
 * A simple converter which converts <code>DoubleData</code> to
 * <code>FloatData</code> and vv (depending on its configuration). All remaining
 * <code>Data</code>s will pass this processor unchanged.
 *
...
public class DataConverter extends BaseDataProcessor {
	...
	public Data getData() throws DataProcessingException {
		...
		if (d instanceof DoubleData && convMode.equals(CONVERT_D2F)) {
			...
			d = new FloatData(MatrixUtils.double2float(dd.getValues()), dd.getSampleRate(), dd.getFirstSampleNumber());
		} else if (d instanceof FloatData && convMode.equals(CONVERT_F2D)) {
			FloatData fd = (FloatData) d;
			d = new DoubleData(MatrixUtils.float2double(fd.getValues()), fd.getSampleRate(), fd.getFirstSampleNumber());
		}
