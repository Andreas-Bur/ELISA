...
/**
 * Applies cepstral mean normalization (CMN), sometimes called channel mean
 * normalization, to incoming cepstral data.
 *
 * Its goal is to reduce the distortion caused by the transmission channel. The
 * output is mean normalized cepstral data.
 * <p>
 * The CMN processing subtracts the mean from all the {@link Data} objects
 * between a {@link edu.cmu.sphinx.frontend.DataStartSignal} and a
 * {@link DataEndSignal} or between a
 * {@link edu.cmu.sphinx.frontend.endpoint.SpeechStartSignal} and a
 * {@link SpeechEndSignal}. BatchCMN will read in all the {@link Data} objects,
 * calculate the mean, and subtract this mean from all the {@link Data} objects.
 * For a given utterance, it will only produce an output after reading all the
 * incoming data for the utterance. As a result, this process can introduce a
 * significant processing delay, which is acceptable for batch processing, but
 * not for live mode. In the latter case, one should use the {@link LiveCMN}.
 * <p>
 * CMN is a technique used to reduce distortions that are introduced by the
 * transfer function of the transmission channel (e.g., the microphone). Using a
 * transmission channel to transmit the input speech translates to multiplying
 * the spectrum of the input speech with the transfer function of the channel
 * (the distortion). Since the cepstrum is the Fourier Transform of the log
 * spectrum, the logarithm turns the multiplication into a summation. Averaging
 * over time, the mean is an estimate of the channel, which remains roughly
 * constant. The channel is thus removed from the cepstrum by subtracting the
 * mean cepstral vector. Intuitively, the mean cepstral vector approximately
 * describes the spectral characteristics of the transmission channel (e.g.,
 * microphone).
 *
 * @see LiveCMN
 */
				...
				if (input instanceof DoubleData) {
					...
					if (sums == null) {
						...
						if (sums.length != cepstrumData.length) {
							throw new Error(
									"Inconsistent cepstrum lengths: sums: " + sums.length + ", cepstrum: " + cepstrumData.length);
						}
					}
					if (cepstrumData[0] >= 0) {
