...
/**
 * Drops certain feature frames, usually to speed up decoding. For example, if
 * you 'dropEveryNthFrame' is set to 2, it will drop every other feature frame.
 * If you set 'replaceNthWithPrevious' to 3, then you replace with 3rd frame
 * with the 2nd frame, the 6th frame with the 5th frame, etc..
 */
public class FrameDropper extends BaseDataProcessor {

	/**
	 * The property that specifies dropping one in every Nth frame. If N=2, we
	 * drop every other frame. If N=3, we drop every third frame, etc..
	 */
	@S4Integer(defaultValue = -1)
	public static final String PROP_DROP_EVERY_NTH_FRAME = "dropEveryNthFrame";

	/**
	 * The property that specifies whether to replace the Nth frame with the
	 * previous frame.
	 */
	 ...
	 *            replace each nth frame with previous
	 */
	...
	protected void initVars(int dropEveryNthFrame, boolean replaceNthWithPrevious) {
		...
		if (dropEveryNthFrame <= 1) {
			throw new IllegalArgumentException(PROP_DROP_EVERY_NTH_FRAME + "must be greater than one");
		}
	...
	/**
	 * Returns the next Data object from this FrameDropper. The Data objects
	 * belonging to a single Utterance should be preceded by a DataStartSignal
	 * and ended by a DataEndSignal.
	 *
				...
				if ((id % dropEveryNthFrame) == (dropEveryNthFrame - 1)) {
					...
					if (replaceNthWithPrevious) {
						...
						if (feature instanceof FloatData) {
							FloatData floatLastFeature = (FloatData) lastFeature;
							feature = new FloatData(floatLastFeature.getValues(), floatLastFeature.getSampleRate(),
									floatLastFeature.getFirstSampleNumber());
						} else {
							DoubleData doubleLastFeature = (DoubleData) lastFeature;
							feature = new DoubleData(doubleLastFeature.getValues(), doubleLastFeature.getSampleRate(),
	...
	/**
	 * Read a Data object from the predecessor DataProcessor, and increment the
	 * ID count appropriately.
	 *
