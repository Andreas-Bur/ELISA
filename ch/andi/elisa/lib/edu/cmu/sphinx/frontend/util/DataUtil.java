...
public class DataUtil {
	...
	/**
	 * The number format to be used by *ArrayToString() methods. The default is
	 * scientific.
	 */
	 ...
	 * short array will be <code>(byteArray.length - 1)/2</code>, i.e., the last
	 * byte is discarded.
	 *
	 * @param byteArray
	 *            a byte array
	...
	/**
	 * Converts a big-endian byte array into an array of doubles. Each
	 * consecutive bytes in the byte array are converted into a double, and
	 * becomes the next element in the double array. The size of the returned
	 * array is (length/bytesPerValue). Currently, only 1 byte (8-bit) or 2
	 * bytes (16-bit) samples are supported.
	 *
	 * @param byteArray
	 *            a byte array
	 * @param offset
	...
	/**
	 * Converts a little-endian byte array into an array of doubles. Each
	 * consecutive bytes of a float are converted into a double, and becomes the
	 * next element in the double array. The number of bytes in the double is
	 * specified as an argument. The size of the returned array is
	 * (data.length/bytesPerValue).
	 *
	 * @param data
	 *            a byte array
	 * @param offset
	 ...
	 *
	 */
	public static double[] littleEndianBytesToValues(byte[] data, int offset, int length, int bytesPerValue, boolean signedData)
			throws ArrayIndexOutOfBoundsException {
	...
	public static short bytesToShort(byte[] byteArray, int offset) throws ArrayIndexOutOfBoundsException {
		short result = (short) ((byteArray[offset++] << 8) | (0x000000FF & byteArray[offset]));
		return result;
	}

	/**
	 * Returns the string representation of the given short array. The string
	 * will be in the form:
	 * 
	 * <pre>
	...
	/**
	 * Returns the given double array as a string. The string will be in the
	 * form:
	 * 
	 * <pre>
	 * data.length data[0] data[1] ... data[data.length-1]
	 ...
	 * <p>
	 * The doubles can be written as decimal, hexadecimal, or scientific
	 * notation. In decimal notation, it is formatted by the method
	 * <code>Util.formatDouble(data[i], 10, 5)</code>. Use the System property
	 * <code>"frontend.util.dumpformat"</code> to control the dump format
	 * (permitted values are "decimal", "hexadecimal", and "scientific".
	 *
	 * @param data
	 ...
	 * </pre>
	 * 
	 * where <code>data[i]</code> is formatted by the method
	 * <code>Util.formatDouble(data[i], 10, 5)</code>.
	 *
	 * @param data
	 *            the double array to dump
	 * @param format
	 *            either HEXADECIMAL, SCIENTIFIC or DECIMAL
	 * @return a string representation of the double array
	 */
	private static String doubleArrayToString(double[] data, int format) {
		...
		for (double val : data) {
			if (format == DECIMAL) {
				dump.append(' ').append(formatDouble(val, decimalIntegerDigits, decimalFractionDigits));
	 ...
	 * </pre>
	 * <p>
	 * The floats can be written as decimal, hexadecimal, or scientific
	 * notation. In decimal notation, it is formatted by the method
	 * <code>Util.formatDouble(data[i], 10, 5)</code>. Use the System property
	 * <code>"frontend.util.dumpformat"</code> to control the dump format
	 * (permitted values are "decimal", "hexadecimal", and "scientific".
	 ...
	 *            the float array to dump
	 * @param format
	 *            either DECIMAL, HEXADECIMAL or SCIENTIFIC
	 * @return a string of the given float array
	 */
	private static String floatArrayToString(float[] data, int format) {
		...
		for (float val : data) {
			if (format == DECIMAL) {
				...
				dump.append(' ').append(Utilities.doubleToScientificString(val, floatScientificFractionDigits));
			}
		}
		return dump.toString();
	}
	...
	/**
	 * Returns a formatted string of the given number, with the given numbers of
	 * digit space for the integer and fraction parts. If the integer part has
	 * less than <code>integerDigits</code> digits, spaces will be prepended to
	 * it. If the fraction part has less than <code>fractionDigits</code>,
	 * spaces will be appended to it. Therefore,
	 * <code>formatDouble(12345.6789, 6, 6)</code> will give the string
	 * 
	 * <pre>
	 * " 12345.6789  "
	 * </pre>
	...
	/**
	 * Returns the number of samples per window given the sample rate (in Hertz)
	 * and window size (in milliseconds).
	 *
	 * @param sampleRate
	 *            the sample rate in Hertz (i.e., frequency per seconds)
	 * @param windowSizeInMs
	...
	/**
	 * Returns the number of samples in a window shift given the sample rate (in
	 * Hertz) and the window shift (in milliseconds).
	 *
	 * @param sampleRate
	 *            the sample rate in Hertz (i.e., frequency per seconds)
	 * @param windowShiftInMs
	...
	/**
	 * Returns a native audio format that has the same encoding, endianness and
	 * sample size as the given format, and a sample rate that is larger than
	...
	/**
	 * Returns a native audio format that has the same encoding, endianness and
	 * sample size as the given format, and a sample rate that is greater than
	 * or equal to the given sample rate.
	 *
	 * @param format
	 *            the desired format
	 * @param mixer
	 *            if non-null, use this Mixer; otherwise use AudioSystem
	 * @return a suitable native audio format
	 */
	public static AudioFormat getNativeAudioFormat(AudioFormat format, Mixer mixer) {
		...
		for (Line.Info info : lineInfos) {
			...
			for (AudioFormat thisFormat : formats) {
				...
				if (thisFormat.getEncoding() == format.getEncoding() && thisFormat.isBigEndian() == format.isBigEndian()
						&& thisFormat.getSampleSizeInBits() == format.getSampleSizeInBits()
