...
/**
 * Produces Mel-cepstrum data from an InputStream. To set the input stream with
 * cepstral data, use the {@link #setInputStream(InputStream) setInputStream}
 * method, and then call {@link #getData} to obtain the Data objects that have
 * cepstrum data in it.
 */
public class StreamHTKCepstrum extends BaseDataProcessor {
	...
	/**
	 * The property name for frame shift in milliseconds, which has a default
	 * value of 10F.
	 */
	...
	public void initialize() {
		...
		curPoint = -1;
		firstSampleNumber = 0;
		// we don't want any more that endianness be set at the upper level, as
		// BatchReco
		// utilizes an ad-hoc procedure to decide on endianness.. We want it to
		// be fixed in the config file
		// bigEndian = true;
	}
	...
	/**
	 * Returns the next Data object, which is the mel cepstrum of the input
	 * frame. However, it can also be other Data objects like DataStartSignal.
	 *
