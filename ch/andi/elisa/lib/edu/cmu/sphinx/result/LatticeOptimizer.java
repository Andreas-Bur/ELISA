 ...
 * Lattices that is deterministic (no Node has Edges to two or more equivalent
 * Nodes), and minimal (no Node has Edge from two or more equivalent Nodes).
 */
...
public class LatticeOptimizer {
	...
	/**
	 * Code for optimizing Lattices. An optimal lattice has all the same paths
	 * as the original, but with fewer nodes and edges
	 * <p>
	 * Note that these methods are all in Lattice so that it is easy to change
	 * the definition of "equivalent" nodes and edges. For example, an
	 * equivalent node might have the same word, but start or end at a different
	 * time.
	 * <p>
	...
	/**
	 * Make the Lattice deterministic, so that no node has multiple outgoing
	 * edges to equivalent nodes.
	 * <p>
	 * Given two edges from the same node to two equivalent nodes, replace with
	 * one edge to one node with outgoing edges that are a union of the outgoing
	 * edges of the old two nodes.
	 * <p>
	 * A --&gt; B --&gt; C \--&gt; B' --&gt; Y
	 * <p>
	 ...
	 * <p>
	 * Note that equivalent nodes must have the same incomming edges. For
	 * example
	 * <p>
	 * A --&gt; B \ \ X --&gt; B'
	 * <p>
	 * B and B' would not be equivalent because the incomming edges are
	 * different
	 */
	protected void optimizeForward() {
		...
		while (moreChanges) {
			...
			// note that we use getCopyOfNodes to avoid concurrent changes to
			// nodes
	...
	/**
	 * Look for 2 "to" edges to equivalent nodes. Replace the edges with one
	 * edge to one node that is a merge of the equivalent nodes
	 * <p>
	 * nodes are equivalent if they have equivalent from edges, and the same
	 * label
	 * <p>
	...
	protected boolean optimizeNodeForward(Node n) {
		...
		for (int j = 0; j < leavingEdges.size(); j++) {
			...
			for (int k = j + 1; k < leavingEdges.size(); k++) {
				...
				/*
				 * If these are not the same edge, and they point to equivalent
				 * nodes, we have a hit, return true
				 */
				assert e != e2;
	 ...
	 * <p>
	 * merge e1 and e2, that is, merge the scores of e1 and e2 create n' that is
	 * a merge of n1 and n2 add n' add edge e' from n to n'
	 * <p>
	...
	protected void mergeNodesAndEdgesForward(Edge e1, Edge e2) {
		...
		for (Edge edge : n2.getEnteringEdges()) {
			...
			anotherEdge.setAcousticScore(mergeAcousticScores(edge.getAcousticScore(), anotherEdge.getAcousticScore()));
			anotherEdge.setLMScore(mergeLanguageScores(edge.getLMScore(), anotherEdge.getLMScore()));
		}
		...
		for (Edge edge : n2.getLeavingEdges()) {
			...
			if (anotherEdge == null) {
				lattice.addEdge(n1, edge.getToNode(), edge.getAcousticScore(), edge.getLMScore());
			} else {
	...
	/**
	 * Minimize the Lattice deterministic, so that no node has multiple incoming
	 * edges from equivalent nodes.
	 * <p>
	 * Given two edges from equivalent nodes to a single nodes, replace with one
	 * edge from one node with incoming edges that are a union of the incoming
	 * edges of the old two nodes.
	 * <p>
	 * A --&gt; B --&gt; C X --&gt; B' --/
	 * <p>
	...
	/**
	 * Look for 2 entering edges from equivalent nodes. Replace the edges with
	 * one edge to one new node that is a merge of the equivalent nodes Nodes
	 * are equivalent if they have equivalent to edges, and the same label.
	 * Merged nodes have a union of entering and leaving edges
	 *
	 * @param n
	 *            node
	...
	protected boolean equivalentNodeLabels(Node n1, Node n2) {
		return (n1.getWord().equals(n2.getWord())
				&& (n1.getBeginTime() == n2.getBeginTime() && n1.getEndTime() == n2.getEndTime()));
	}
	...
	protected void mergeNodesAndEdgesBackward(Edge e1, Edge e2) {
		...
		for (Edge edge : n2.getEnteringEdges()) {
			Edge anotherEdge = n1.getEdgeFromNode(edge.getFromNode());
			if (anotherEdge == null) {
				lattice.addEdge(edge.getFromNode(), n1, edge.getAcousticScore(), edge.getLMScore());
			} else {
		...
		for (Node n : lattice.getCopyOfNodes()) {
			if (lattice.hasNode(n)) {
				if (n == lattice.getInitialNode()) {
					...
					if (n.getLeavingEdges().isEmpty() || n.getEnteringEdges().isEmpty()) {
	...
	/**
	 * Provides a single method to merge acoustic scores, so that changes to how
	 * acoustic score are merged can be made at one point only.
	 *
	 * @param score1
	 *            the first acoustic score
	 * @param score2
	 *            the second acoustic score
	...
	/**
	 * Provides a single method to merge language scores, so that changes to how
	 * language score are merged can be made at one point only.
	 *
	 * @param score1
	 *            the first language score
	 * @param score2
	 *            the second language score
