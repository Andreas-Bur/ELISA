...
public class Edge {
	...
	/**
	 * Create an Edge from fromNode to toNode with acoustic and Language Model
	 * scores.
	 *
	 * @param fromNode
	 *            from node
	...
	void dumpAISee(FileWriter f) throws IOException {
		f.write("edge: { sourcename: \"" + fromNode.getId() + "\" targetname: \"" + toNode.getId() + "\" label: \""
	...
	public void dumpDot(FileWriter f) throws IOException {
		...
		f.write("\tnode" + fromNode.getId() + " -> node" + toNode.getId() + " [ label=\"" + label + "\" ]\n");
	}
	...
	/**
	 * Returns true if the given edge is equivalent to this edge. Two edges are
	 * equivalent only if they have their 'fromNode' and 'toNode' are
	 * equivalent, and that their acoustic and language scores are the same.
	 *
	 * @param other
	...
	public boolean isEquivalent(Edge other) {
		/*
		 * TODO: Figure out why there would be minute differences in the
		 * acousticScore. Therefore, the equality of the acoustic score is judge
		 * based on whether the difference is bigger than 1.
		 */
		double diff = Math.abs(acousticScore) * 0.00001;
		return ((Math.abs(acousticScore - other.getAcousticScore()) <= diff && lmScore == other.getLMScore())
				&& (fromNode.isEquivalent(other.getFromNode()) && toNode.isEquivalent(other.getToNode())));
	}
}
