 ...
 * elements are potentially shared by a number of {@link GaussianMixture
 * GaussianMixtures}, these elements should not be written to. The
 * GaussianMixture defines a single probability density function along with a
...
// there some potential opportunities to reduce the number of computations in
// scoring
...
public class MixtureComponent implements Cloneable, Serializable {
	...
	public static final float DEFAULT_VAR_FLOOR = 0.0001f; // this also seems to
															// be the default of
															// SphinxTrain
	 ...
	 */
	public MixtureComponent(float[] mean, float[][] meanTransformationMatrix, float[] meanTransformationVector, float[] variance,
			float[][] varianceTransformationMatrix, float[] varianceTransformationVector) {
		this(mean, meanTransformationMatrix, meanTransformationVector, variance, varianceTransformationMatrix,
				varianceTransformationVector, DEFAULT_DIST_FLOOR, DEFAULT_VAR_FLOOR);
	}
	 ...
	 */
	public MixtureComponent(float[] mean, float[][] meanTransformationMatrix, float[] meanTransformationVector, float[] variance,
			float[][] varianceTransformationMatrix, float[] varianceTransformationVector, float distFloor, float varianceFloor) {
	 ...
	 * <p>
	 * Note: The support of <code>DoubleData</code>-features would require an
	 * array conversion to float[]. Because getScore might be invoked with very
	 * high frequency, features are restricted to be <code>FloatData</code>s.
	 *
	 * @param feature
	 ...
	 * respectively, <b>Var</b> is the Covariance matrix, <b>det()</b> is the
	 * determinant of a matrix, <b>inv()</b> is its inverse, <b>exp</b> is the
	 * exponential operator, <b>x'</b> is the transposed vector of <b>x</b> and
	 * <b>N</b> is the dimension of the vectors <b>x</b> and <b>m</b>.
	 *
	 * @param feature
	 *            the feature to score
	 * @return the score, in log, for the given feature
	 ...
	 * distance computation can be carried out in advance. Specifically, the
	...
	public float precomputeDistance() {
		...
		for (int i = 0; i < variance.length; i++) {
			logPreComputedGaussianFactor += Math.log(precisionTransformed[i] * -2);
		...
		// factor in log scale.
		logPreComputedGaussianFactor = Math.log(2.0 * Math.PI) * variance.length - logPreComputedGaussianFactor;
	...
	public MixtureComponent clone() throws CloneNotSupportedException {
		...
		mixComp.varianceTransformationVector = this.varianceTransformationVector != null
				? this.varianceTransformationVector.clone()
				: null;
