...
/**
 * A linguist processor that dumps out the search space in a simple format. This
 * processor is designed so that it can be easily extended by replacing the
 * dumpNode and the dumpEdge methods.
 */
public class LinguistDumper extends LinguistProcessor {
	...
	public void run() {
		try {
			...
			SearchState firstState = getLinguist().getSearchGraph().getInitialState();
	 ...
	 *
	 * @param depthFirst
	 *            if true traversal is depth first, otherwise the traversal is
	 *            breadth first
	 */
...
class StateLevel {
	...
	public String toString() {
		return String.valueOf(level) + ' ' + state.getSignature() + " 1 " + getTypeLabel(state);
	}
