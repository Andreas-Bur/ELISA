...
public class ModelInitializerLoader implements Loader {
	...
	private void addModelToSenonePool(Pool<Senone> pool, int[] stateID, float distFloor, float varianceFloor) {
		...
		/*
		 * int numMeans = meansPool.size(); int numVariances =
		 * variancePool.size(); int numSenones =
		 * mixtureWeightsPool.getFeature(NUM_SENONES, 0); int whichGaussian = 0;
		 * 
		 * logger.fine("NG " + numGaussiansPerSenone); logger.fine("NS " +
		 * numSenones); logger.fine("NMIX " + numMixtureWeights);
		...
		for (int state : stateID) {
			...
			for (int j = 0; j < numGaussiansPerSenone; j++) {
				...
				mixtureComponents[j] = new MixtureComponent(meansPool.get(whichGaussian), meanTransformationMatrixPool.get(0),
						meanTransformationVectorPool.get(0), variancePool.get(whichGaussian),
						varianceTransformationMatrixPool.get(0), varianceTransformationVectorPool.get(0), distFloor,
						varianceFloor);
			}
	...
	/**
	 * Loads the phone list, which possibly contains the sizes (number of
	 * states) of models.
	 *
	 * @param ps
	 * @param useCDUnits
	 ...
	 * @param path
	 *            the path to a density file @throws FileNotFoundException if a
	 ...
	 *             if an error occurs while loading the data
	 */
	private void addModelToTransitionMatrixPool(Pool<float[][]> pool, int hmmId, int numEmittingStates, float floor, boolean skip)
			throws IOException {
