...
/**
 * Classes that implement this interface create grammars. A grammar is
 * represented internally as a graph of {@link GrammarNode GrammarNodes} linked
 * together by {@link GrammarArc GrammarArcs}. Calling {@link #getInitialNode()
 * getInitialNode} will return the first node of the grammar graph. To traverse
 * the grammar graph, one should call GrammarNode.getSuccessors, which will
 * return an array of GrammarArcs, from which you can reach the neighboring
 * GrammarNodes.
 * <p>
...
public abstract class Grammar implements Configurable, GrammarInterface {
	...
	private final Random randomizer = new Random(56); // use fixed initial to
														// make get
	...
	public Grammar(boolean showGrammar, boolean optimizeGrammar, boolean addSilenceWords, boolean addFillerWords,
	...
	public void dumpStatistics() {
		if (logger.isLoggable(Level.INFO)) {
			...
			logger.info("Avg arcs  : " + ((float) successorCount / getNumNodes()));
		}
	}
	 ...
	 * in the dictionary, an empty node is created. The grammar id is
	...
	private void addSilenceWords() {
		...
		for (GrammarNode g : nodes) {
			if (!g.isEmpty() && !g.getWord().isFiller()) {
				GrammarNode silNode = createGrammarNode(maxIdentity + 1, dictionary.getSilenceWord().getSpelling());
	...
	/**
	 * Adds an optional filler word loop after every non-filler word in the
	 * grammar
	 */
	...
	/**
	 * Gets the set of fillers after filtering out fillers that don't go between
	 * words.
	 *
	...
	private Word[] getInterWordFillers() {
		...
		for (Word fillerWord : fillers) {
			if (fillerWord != dictionary.getSentenceStartWord() && fillerWord != dictionary.getSentenceEndWord()) {
