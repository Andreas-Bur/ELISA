...
/**
 * Represents a hidden-markov-model. An HMM consists of a unit (context
 * dependent or independent), a transition matrix from state to state, and a
 * sequence of senones associated with each state. This representation of an HMM
 * is a specialized left-to-right markov model. No backward transitions are
 * allowed.
 */
public class SenoneHMM implements HMM {
	 ...
	 *
	 * @param unit
	 *            the unit for this HMM
	 * @param senoneSequence
	 *            the sequence of senones for this HMM
	 * @param transitionMatrix
	 *            the state transition matrix
	 * @param position
	 *            the position associated with this HMM
	 */
	 ...
	 *
	 * @return the sequence of senones associated with this HMM. The length of
	 *         the sequence is N, where N is the order of the HMM. Note that
	 *         senone sequences may be shared among HMMs.
	 */
	...
	/**
	 * Returns the transition matrix that determines the state transition
	 * probabilities for the matrix. Each entry in the transition matrix defines
	 * the probability of transitioning from one state to the next. For example,
	 * the probability of transitioning from state 1 to state 2 can be
	 * determined by accessing transition matrix element[1][2].
	 *
	 * @return the transition matrix (in log domain) of size NxN where N is the
	 *         order of the HMM
	 */
	...
	/**
	 * Determines if this HMM represents a filler unit. A filler unit is speech
	 * that is not meaningful such as a cough, 'um' , 'er', or silence.
	 *
	 ...
	 *
	 * @return the set of arcs that transition to the initial states for this
	 *         HMM
	 */
