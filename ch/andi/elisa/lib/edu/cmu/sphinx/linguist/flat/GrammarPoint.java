...
/**
 * Manages a particular point in a grammar. The GrammarPoint is used to manage
 * the look-ahead for generating right-contexts. Since we haven't built the HMM
 * tree yet, looking ahead can be difficult. The GrammarPoint class points to a
 * particular unit within a pronunciation/word/grammar. From a particular
 * grammar point, it is possible to get the set of next grammar points.
 */
...
public class GrammarPoint {
	...
	/**
	 * Creates a grammar point that points to the given unit of the given
	 * pronunciation state.
	 *
	 * @param state
	 *            the pronunciation of interest
	 */
	...
	/**
	 * Creates a grammar node that points to the first unit of the first
	 * pronunciation of the first word of the given grammar node
	 *
	 * @param node
	...
	/**
	 * Creates a GrammarPoint that corresponds to the given unit of the given
	 * pronunciation
	 *
	 * @param state
	 ...
	 *            the index of alternative
	 * @param wordIndex
	 *            the index of the word in the node
	 * @param pronunciationIndex
	 *            the index of the pronunciation in the word.
	 * @param unitIndex
	 *            the index of the unit in the pronunciation
	 */
	 ...
	 *
	 * @return the unit, or null if there is no unit associated with this point
	 *         in the grammar
	 */
	private Unit getUnit() {
		...
		if (alternativeIndex != -1 && alternativeIndex < alternatives.length) {
			...
			if (wordIndex < words.length) {
				Pronunciation[] pronunciations = words[wordIndex].getPronunciations();
				if (pronunciationIndex < pronunciations.length) {
					Unit[] units = pronunciations[pronunciationIndex].getUnits();
	...
	/**
	 * Gets the unit associated with this point in the grammar. If there is no
	 * unit, return filler
	 *
	...
	/**
	 * Gets all of the right contexts for this grammar point. The contexts
	 * returned are guaranteed to be 'size' units in length, The number of
	 * contexts returned depends upon the perplexity of the grammar downstream
	 * from this GrammarPoint
	 *
	 * @param size
	 *            the size of each context returned
	 * @param startWithCurrent
	 *            include the current state in the context
	 * @param maxContexts
	 *            the maxium number of right contexts to return
	...
	/**
	 * Add a context to a list of contexts after ensuring that no identical
	 * contexts exist on the list. When a right context is collected it may
	 * contain duplicates in certain cases (when this unit is the last unit in a
	 * grammar node, and there is a branch to multiple words in subsequent
	 * nodes, for instance)
	 *
	 * @param contexts
	 *            the list of contexts to add the new units to
	 * @param units
	 *            the units to add to the context
	 */
	...
	/**
	 * Returns a list of next GrammarPoints for this GrammarPoint. If there are
	 * no more downstream grammar points with words, an empty list is returned.
	 *
	 * @param startWithCurrent
	 *            include the current state in the context
	...
	private List<GrammarPoint> getNextGrammarPoints(boolean startWithCurrent) {
		...
		// them up, expand them and return that set.
			...
			Pronunciation[] pronunciations = node.getAlternatives()[alternativeIndex][wordIndex].getPronunciations();
	...
	/**
	 * Given a GrammarNode return a list of successors GrammarNodes that contain
	 * words
	 *
	 * @param node
	 *            successors are gathered from this node
	 ...
	 *            the grammar node
	 * @param nextPoints
	 *            where the grammar points should be added
	 */
	 ...
	 *
	 * @param state
	 *            if true searches for context will not cross grammar nodes.
	 */
