 ...
 * Context Independent units and context dependent units can be converted to an
 * ID. IDs can be used to quickly retrieve a unit or an hmm associated with the
 * unit. This class operates under the constraint that context sizes are exactly
 * one, which is generally only valid for large vocabulary tasks.
 */
...
public class HMMPool {
	...
	/**
	 * Given a unit ID, generate a full context dependent unit that will allow
	 * us to look for a suitable hmm
	 *
	 * @param id
	 *            the unit id
	...
	private Unit synthesizeUnit(int id) {
		...
		if (logger.isLoggable(Level.FINER)) {
			logger.finer("Missing " + getUnitNameFromID(id) + " returning " + unit);
		}
		return unit;
	}
	...
	public int getID(Unit unit) {
		if (unit.isContextDependent()) {
			...
			return buildID(getSimpleUnitID(unit), getSimpleUnitID(context.getLeftContext()[0]),
	 ...
	 * @param leftID
	 *            the id of the left context unit
	 * @param rightID
	...
	public int buildID(int unitID, int leftID, int rightID) {
		...
		if (unitTable[unitID].isFiller()) {
			...
			id = unitID * (numCIUnits * numCIUnits) + (leftID * numCIUnits) + rightID;
		}
	...
	private String getUnitNameFromID(int id) {
		...
		String cs = unitTable[centralID] == null ? "(" + centralID + ')' : unitTable[centralID].toString();
		String ls = unitTable[leftID] == null ? ("(" + leftID + ')') : unitTable[leftID].toString();
		String rs = unitTable[rightID] == null ? "(" + rightID + ')' : unitTable[rightID].toString();
	...
	/**
	 * A quick and dirty benchmark to get an idea how long the HMM lookups will
	 * take. This experiment shows that on a 1GHZ sparc system, the lookup takes
	 * a little less than 1uSec. This is probably fast enough.
	 */

	static final HMMPosition[] pos = { HMMPosition.BEGIN, HMMPosition.END, HMMPosition.SINGLE, HMMPosition.INTERNAL };
