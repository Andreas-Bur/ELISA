...
/**
 * Represents a single state in the recognition trellis. Subclasses of a token
 * are used to represent the various emitting state.
 * <p>
...
public class Token implements Scoreable {
	...
	/**
	 * Internal constructor for a token. Used by classes Token, CombineToken,
	 * ParallelToken
	 *
	 * @param predecessor
	 *            the predecessor for this token
	 * @param state
	 ...
	 */
	public Token(Token predecessor, SearchState state, float logTotalScore, float logInsertionScore, float logLanguageScore,
			long collectTime) {
	 ...
	 *
	 * @param state
	 *            the SearchState associated with this token
	 * @param collectTime
	 *            collection time of this token
	 */
	 ...
	 * @param logTotalScore
	 *            total score
	 * @param logAcousticScore
	 *            the log acoustic score
	 * @param logInsertionScore
	 *            the log insertion score
	 * @param logLanguageScore
	 *            the log language score
	 */
	public Token(Token predecessor, float logTotalScore, float logAcousticScore, float logInsertionScore,
	...
	/**
	 * Returns the predecessor for this token, or null if this token has no
	 * predecessors
	 *
	...
	/**
	 * Collect time is different from frame number because some frames might be
	 * skipped in silence detector
	 * 
	...
	/**
	 * Returns the score for the token. The score is a combination of language
	 * and acoustic scores
	 *
	 ...
	 * with get score. The token will keep a reference to the scored
	 * feature-vector.
	 *
	 * @param feature
	 ...
	 *
	 * @param logScore
	 *            the new score for the token (in logMath log base)
	 */
	...
	/**
	 * Returns the insertion score associated with this token. Insertion score
	 * is the score of the transition between states. It might be transition
	 * score from the acoustic model, phone insertion score or word insertion
	 * probability from the linguist.
	 *
	 * @return the language score (in logMath log base)
	 */
	...
	/**
	 * Returns the acoustic score for this token (in logMath log base). Acoustic
	 * score is a sum of frame GMM.
	 *
	 * @return score
	 */
	...
	/**
	 * Determines if this token is associated with an emitting state. An
	 * emitting state is a state that can be scored acoustically.
	 *
	 * @return <code>true</code> if this token is associated with an emitting
	 *         state
	 */
	...
	public String toString() {
		return numFmt.format(getCollectTime()) + ' ' + scoreFmt.format(getScore()) + ' ' + scoreFmt.format(getAcousticScore())
				+ ' ' + scoreFmt.format(getLanguageScore()) + ' ' + getSearchState();
	}
	 ...
	 * dumps a branch of tokens
	 *
	 * @param includeHMMStates
	 *            if true include all sentence hmm states
	 */
	public void dumpTokenPath(boolean includeHMMStates) {
		...
		for (int i = list.size() - 1; i >= 0; i--) {
			token = list.get(i);
			if (includeHMMStates || (!(token.getSearchState() instanceof HMMSearchState))) {
	...
	public String getWordPath(boolean wantFiller, boolean wantPronunciations) {
		...
		while (token != null) {
			if (token.isWord()) {
				...
				// System.out.println(token.getFrameNumber() + " " + word + " "
				// + token.logLanguageScore + " " + token.logAcousticScore);
	...
	/**
	 * Returns the string of words and units for this token, with embedded
	 * silences.
	 *
	...
	/**
	 * Returns the word of this Token, the search state is a WordSearchState. If
	 * the search state is not a WordSearchState, return null.
	 *
	...
	public void update(Token predecessor, SearchState nextState, float logEntryScore, float insertionProbability,
