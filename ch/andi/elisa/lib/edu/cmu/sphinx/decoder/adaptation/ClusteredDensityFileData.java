...
public class ClusteredDensityFileData {
	...
	private void kMeansClustering(Loader loader, int maxIterations) {
		...
		ArrayList<float[]> oldCentroids = new ArrayList<float[]>(numberOfClusters);
		...
		while (!converged && nrOfIterations > 0) {
			...
			for (int i = 0; i < initialData.size(); i++) {
				...
				for (int k = 1; k < numberOfClusters; k++) {
					distance = this.euclidianDistance(oldCentroids.get(k), currentValue);
			...
			for (int i = 0; i < numberOfClusters; i++) {
				converged = converged && (this.isEqual(centroids.get(i), oldCentroids.get(i)));
			}
