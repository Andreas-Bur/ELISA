 ...
 *
 * The logarithmic base can be set by the property:
 * <code>edu.cmu.sphinx.util.LogMath.logBase</code>
 */
public final class LogMath {
	...
	private LogMath() {
		...
		if (useTable) {
			...
			if (entriesInTheAddTable <= verySmallNumberOfEntries) {
				throw new IllegalArgumentException("The log base " + logBase + " yields a very small addTable. "
						+ "Either choose not to use the addTable, " + "or choose a logBase closer to 1.0");
			}
	 ...
	 * </p>
	 * <p>
	 * Important to notice that <code>subtractAsLinear(a, b)</code> is *not* the
	 * same as <code>addAsLinear(a, -b)</code>, since we're in the log domain,
	 ...
	 * function can be constructed as a table, if table lookup is faster than
	 * actual computation.
	 *
	 * @param index
	 ...
	 * the current function returns the second term of the right hand side of
	 * the equality above, generalized for the case of any log base. This
	 * function is constructed as a table lookup.
	 *
	 * @param index
	 *            the index into the addTable
	 * @return the value pointed to by index
	 ...
	 *
	 * @param logMinuend
	 *            value in log domain (i.e. log(minuend)) to be subtracted from
	 * @param logSubtrahend
	 ...
	 * @throws IllegalArgumentException
	 *             <p>
	 *             This is a very slow way to do this, but this method should
	 *             rarely be used.
	 *             </p>
	 */
	public final float subtractAsLinear(float logMinuend, float logSubtrahend) throws IllegalArgumentException {
		...
		if (logMinuend < logSubtrahend) {
			throw new IllegalArgumentException(
					"Subtraction results in log " + "of a negative number: " + logMinuend + " - " + logSubtrahend);
		}
		logInnerSummation = 1.0;
	 ...
	 * <code>LogMath</code>-instance for conversion.
	 * 
	 * @param vector
	 *            to convert in-place
	 */
