...
public class Utilities {
	 ...
	 * @return a string of length containing string padded with whitespace or
	 *         truncated
	 */
	...
	/**
	 * Dumps padded text. This is a simple tool for helping dump text with
	 * padding to a Writer.
	 *
	 * @param pw
	 *            the stream to send the output
	 * @param padding
	 *            the number of spaces in the string
	 * @param string
	...
	public static void dumpMemoryInfo(String msg) {
		...
		System.out.println("Memory (mb) " + " total: " + totalMemory + " reclaimed: " + reclaimedMemory + " free: " + freeMemory
				+ " Max Used: " + (maxUsed / (1024 * 1024)) + " -- " + msg);
	}

	/**
	 * Returns the string representation of the given double value in normalized
	 * scientific notation. The <code>fractionDigits</code> argument gives the
	 * number of decimal digits in the fraction portion. For example, if
	 * <code>fractionDigits</code> is 4, then the 123450 will be "1.2345e+05".
	 * There will always be two digits in the exponent portion, and a plus or
	 * minus sign before the exponent.
	 *
	 * @param number
	 *            the double to convert
	 * @param fractionDigits
	 *            the number of digits in the fraction part, e.g., 4 in
	 *            "1.2345e+05".
	...
	/**
	 * Returns true if the given binary cepstra file is in big-endian format. It
	 * assumes that the first 4 bytes of the file tells you how many 4-byte
	 * floating point cepstra values are in the file.
	 *
	 * @param filename
	...
	/**
	 * Reads the next float from the given DataInputStream, where the data is in
	 * little endian.
	 *
	 * @param dataStream
	...
	public static int readLittleEndianInt(DataInputStream dataStream) throws IOException {
		return dataStream.readUnsignedByte() | dataStream.readUnsignedByte() << 8 | dataStream.readUnsignedByte() << 16
				| dataStream.readUnsignedByte() << 24;
	}
	...
	public static int swapInteger(int integer) {
		return (((0x000000ff & integer) << 24) | ((0x0000ff00 & integer) << 8) | ((0x00ff0000 & integer) >> 8)
				| ((0xff000000 & integer) >> 24));
	}

	/**
	 * Byte-swaps the given float to the other endian. That is, if this float is
	 * big-endian, it becomes little-endian, and vice-versa.
	 *
	 * @param floatValue
	 *            the float to swap
	...
	public static float swapFloat(float floatValue) {
		return Float.intBitsToFloat(swapInteger(Float.floatToRawIntBits(floatValue)));
	}
