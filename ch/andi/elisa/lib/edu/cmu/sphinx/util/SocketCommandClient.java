...
/**
 * A client for a socketed command interpreter. Hooks up to a socketed command
 * interpreter and allows the sending of commands.
 *
...
public class SocketCommandClient {
	...
	public synchronized void open(String aHost, int aPort) throws IOException {
		...
		/*
		 * Open a client socket connection, throws IOExceptions to the calling
		 * thread. RATIONALE: the caller should handle any connection- related
		 * errors.
		 */
	...
	/**
	 * @return the SO_TIMEOUT of the Socket that this client uses. 0 returns
	 *         implies that the option is disabled (i.e., timeout of infinity).
	...
	/**
	 * sends a command, retries on error which will attempt to repair a dead
	 * socket
	 *
	 * @param command
	 *            the command
