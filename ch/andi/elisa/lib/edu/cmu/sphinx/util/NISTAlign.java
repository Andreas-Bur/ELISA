...
/**
 * Implements a portion of the NIST align/scoring algorithm to compare a
 * reference string to a hypothesis string. It only keeps track of
 * substitutions, insertions, and deletions.
 */
public class NISTAlign {

	/*
	 * Constants that help with the align. The following are used in the
	 * backtrace table and backtrace list.
	 */
	...
	/*
	 * Constants that help with the align. The following are used to create the
	 * penalty table.
	 */
	...
	/**
	 * Totals over the life of this class. These can be reset to 0 with a call
	 * to resetTotals.
	 */
	...
	/**
	 * The reference annotation; typically the name of the audio file for the
	 * reference string. This is an optional part of the rawReference string. If
	 * it is included, it is appended to the end of the string in parentheses.
	 * Updated with each call to 'align'.
	 */
	private String referenceAnnotation;

	/**
	 * Ordered list of words from rawReference after the annotation has been
	 * removed. Updated with each call to 'align'.
	 */
	private LinkedList<Object> referenceItems;

	/**
	 * Aligned list of words from rawReference. Created in alignWords. Updated
	 * with each call to 'align'.
	 */
	...
	/**
	 * Ordered list of words from rawHypothesis after the annotation has been
	 * removed. Updated with each call to 'align'.
	 */
	private LinkedList<Object> hypothesisItems;

	/**
	 * Aligned list of words from rawHypothesis. Created in alignWords. Updated
	 * with each call to 'align'.
	 */
	 ...
	 *
	 * @param showResults
	 *            true if the results should be displayed
	 */
	 ...
	 *
	 * @param showAlignedResults
	 *            true if the aligned results should be displayed
	 */
	...
	/**
	 * Reset the total insertions, deletions, and substitutions counts for this
	 * class.
	 */
	...
	/**
	 * Performs the NIST alignment on the reference and hypothesis strings. This
	 * has the side effect of updating nearly all the fields of this class.
	 *
	 * @param reference
	...
	/**
	 * Returns the reference string. This string will be filtered (all spurious
	 * whitespace removed and annotation removed) and set to all lower case.
	 *
	...
	/**
	 * Returns the hypothesis string. This string will be filtered (all spurious
	 * whitespace removed and annotation removed) and set to all lower case.
	 *
	...
	public float getTotalWordErrorRate() {
		if (totalReferenceWords == 0) {
			...
			return ((float) getTotalWordErrors()) / ((float) totalReferenceWords);
		}
	}
	...
	/**
	 * Prints the results for this sentence to System.out. If you want the
	 * output to match the NIST output, see printNISTSentenceSummary.
	 *
	...
	/**
	 * Prints the total summary for all calls. If you want the output to match
	 * the NIST output, see printNISTTotalSummary.
	 *
	 * @see #printNISTTotalSummary
	 */
	public void printTotalSummary() {
		if (totalSentences > 0) {
			System.out.print("   Accuracy: " + toPercentage("##0.000%", getTotalWordAccuracy()));
			System.out.println("    Errors: " + getTotalWordErrors() + "  (Sub: " + totalSubstitutions + "  Ins: "
					+ totalInsertions + "  Del: " + totalDeletions + ')');
			System.out.println("   Words: " + totalReferenceWords + "   Matches: " + totalWordsCorrect + "    WER: "
					+ toPercentage("##0.000%", getTotalWordErrorRate()));
			System.out.println("   Sentences: " + totalSentences + "   Matches: " + (totalSentences - totalSentencesWithErrors)
					+ "   SentenceAcc: " + toPercentage("##0.000%", getTotalSentenceAccuracy()));
		}
	}

	/**
	 * Prints the results for this sentence to System.out. This matches the
	 * output from the NIST aligner.
	 */
	public void printNISTSentenceSummary() {
		...
		if (referenceAnnotation != null) {
			System.out.println("SENTENCE " + totalSentences + "  " + referenceAnnotation);
		} else {
			System.out.println("SENTENCE " + totalSentences);
		}

		System.out.println("Correct          = " + toPercentage("##0.0%", correct, referenceItems.size()) + padLeft(5, correct)
				+ "   (" + padLeft(6, totalWordsCorrect) + ')');
		System.out.println("Errors           = " + toPercentage("##0.0%", sentenceErrors, referenceItems.size())
				+ padLeft(5, sentenceErrors) + "   (" + padLeft(6, totalSentencesWithErrors) + ')');
	...
	/**
	 * Prints the summary for all calls to align to System.out. This matches the
	 * output from the NIST aligner.
	 */
	public void printNISTTotalSummary() {
		...
		System.out.println("  correct                  " + toPercentage("##0.0%", totalSentencesCorrect, totalSentences) + " ("
				+ padLeft(4, totalSentencesCorrect) + ')');
		System.out.println("  with error(s)            " + toPercentage("##0.0%", totalSentencesWithErrors, totalSentences) + " ("
				+ padLeft(4, totalSentencesWithErrors) + ')');
		System.out.println("    with substitutions(s)  " + toPercentage("##0.0%", totalSentencesWithSubtitutions, totalSentences)
				+ " (" + padLeft(4, totalSentencesWithSubtitutions) + ')');
		System.out.println("    with insertion(s)      " + toPercentage("##0.0%", totalSentencesWithInsertions, totalSentences)
				+ " (" + padLeft(4, totalSentencesWithInsertions) + ')');
		System.out.println("    with deletions(s)      " + toPercentage("##0.0%", totalSentencesWithDeletions, totalSentences)
		...
		System.out.println("Correct           = " + toPercentage("##0.0%", totalWordsCorrect, totalReferenceWords) + " ("
				+ padLeft(6, totalWordsCorrect) + ')');
		System.out.println("Substitutions     = " + toPercentage("##0.0%", totalSubstitutions, totalReferenceWords) + " ("
				+ padLeft(6, totalSubstitutions) + ')');
		System.out.println("Deletions         = " + toPercentage("##0.0%", totalDeletions, totalReferenceWords) + " ("
				+ padLeft(6, totalDeletions) + ')');
		System.out.println("Insertions        = " + toPercentage("##0.0%", totalInsertions, totalReferenceWords) + " ("
				+ padLeft(6, totalInsertions) + ')');
		System.out.println("Errors            = " + toPercentage("##0.0%", getTotalWordErrors(), totalReferenceWords) + " ("
				+ padLeft(6, getTotalWordErrors()) + ')');

		System.out.println();

		System.out.println("Ref. words           = " + padLeft(6, totalReferenceWords));
		System.out.println("Hyp. words           = " + padLeft(6, totalHypothesisWords));
		System.out.println("Aligned words        = " + padLeft(6, totalAlignedWords));

		System.out.println();
		System.out.println("WORD ACCURACY=  " + toPercentage("##0.000%", totalWordsCorrect, totalReferenceWords) + " ("
				+ padLeft(5, totalWordsCorrect) + '/' + padLeft(5, totalReferenceWords) + ")  ERRORS= "
				+ toPercentage("##0.000%", getTotalWordErrors(), totalReferenceWords) + " (" + padLeft(5, getTotalWordErrors())
	...
	/**
	 * Creates the backtrace table. This is magic. The basic idea is that the
	 * penalty table contains a set of penalty values based on some
	 * strategically selected numbers. I'm not quite sure what they are, but
	 * they help determine the backtrace table values. The backtrace table
	 * contains information used to help determine if words matched (OK), were
	 * inserted (INSERTION), substituted (SUBSTITUTION), or deleted (DELETION).
	 *
	 * @param referenceItems
	 *            the ordered list of reference words
	...
	/**
	 * Backtraces through the penalty table. This starts at the "lower right"
	 * corner (i.e., the last word of the longer of the reference vs. hypothesis
	 * strings) and works its way backwards.
	 *
	 * @param backtraceTable
	 *            created from call to createBacktraceTable
	...
	/**
	 * Based on the backtrace information, words are aligned as appropriate with
	 * insertions and deletions causing asterisks to be placed in the word
	 * lists. This generates the alignedReferenceWords and
	 * alignedHypothesisWords lists.
	 *
	 * @param backtrace
	 *            the backtrace list created in backtrace
	 */
	 ...
	 *
	 * @param pattern
	 *            percentage pattern (ala DecimalFormat)
	 * @param numerator
	 *            the numerator
	 * @param denominator
	 *            the denominator
	...
	String toPercentage(String pattern, int numerator, int denominator) {
		...
		return padLeft(6, percentageFormat.format((double) numerator / (double) denominator));
	}
	 ...
	 *
	 * @param width
	 *            the total width of String, including spaces
	 * @param i
	 *            the integer
	...
	/**
	 * Pads a string to the left with spaces (i.e., prepends spaces to the
	 * string so it fills out the given width).
	 *
	 * @param width
	 *            the total width of String, including spaces
	 * @param string
	...
	/*
	 * Takes two filenames -- the first contains a list of reference sentences,
	 * the second contains a list of hypothesis sentences. Aligns each pair of
	 * sentences and outputs the individual and total results.
	 */
