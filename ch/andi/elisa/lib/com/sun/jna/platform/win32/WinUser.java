...
public interface WinUser extends WinDef {
	...
	public class WINDOWINFO extends Structure {
		...
		@Override
		protected List<String> getFieldOrder() {
			return Arrays.asList(new String[] { "cbSize", "rcWindow", "rcClient", "dwStyle", "dwExStyle", "dwWindowStatus",
					"cxWindowBorders", "cyWindowBorders", "atomWindowType", "wCreatorVersion" });
		}
	}
	...
	public class WINDOWPLACEMENT extends Structure {
		 ...
		 * maximized before it was minimized. This setting is only valid the
		 * next time the window is restored. It does not change the default
		 * restoration behavior.
		 *
		 * This flag is only valid when the SW_SHOWMINIMIZED value is specified
		 * for the showCmd member.
		 */
		public static final int WPF_RESTORETOMAXIMIZED = 0x2;

		/**
		 * If the calling thread and the thread that owns the window are
		 * attached to different input queues, the system posts the request to
		 * the thread that owns the window. This prevents the calling thread
		 ...
		 * more of WPF_SETMINPOSITION, WPF_RESTORETOMAXIMIZED, or
		 ...
		 * Usually largely negative. May be in workspace coordinates.
		 */
		public POINT ptMinPosition;
		/**
		 * Coordinates of the window's upper-right corner when maximized.
		 * Usually small and negative. May be in workspace coordinates.
		 */
		...
		protected List<String> getFieldOrder() {
			return Arrays
					.asList(new String[] { "length", "flags", "showCmd", "ptMinPosition", "ptMaxPosition", "rcNormalPosition" });
		}
	}
	 ...
	 * direction keys . You can turn this style on and off to change dialog box
	 ...
	 * overlapped window has a title bar and a border. Same as the WS_TILED
	 * style.
	 */
	...
	public class COPYDATASTRUCT extends Structure {
		...
		public COPYDATASTRUCT(Pointer p) {
			super(p);
			// Receiving data and read it from native memory to fill the
			// structure.
			read();
		}
	...
	public class BLENDFUNCTION extends Structure {
		public static final List<String> FIELDS = createFieldsOrder("BlendOp", "BlendFlags", "SourceConstantAlpha",
				"AlphaFormat");
	 ...
	 * HMONITOR. A valid HMONITOR is guaranteed to be non-NULL. A physical
	 * display has the same HMONITOR as long as it is part of the desktop.
	 */
	 ...
	 * </p>
	 * <p>
	 * The {@link User32#GetMonitorInfo} function stores information into a
	...
	public class MONITORINFO extends Structure {
		 ...
		 * coordinates. Note that if the monitor is not the primary display
		 * monitor, some of the rectangle's coordinates may be negative values.
		 */
		public RECT rcMonitor;

		/**
		 * Specifies the work area rectangle of the display monitor that can be
		 * used by applications, expressed in virtual-screen coordinates.
		 * Windows uses this rectangle to maximize an application on the
		 * monitor. The rest of the area in rcMonitor contains system windows
		 * such as the task bar and side bars. Note that if the monitor is not
		 * the primary display monitor, some of the rectangle's coordinates may
		 * be negative values.
		 */
		public RECT rcWork;

		/**
		 * The attributes of the display monitor. This member can be the
		 * following value.
		 * <ul>
	 ...
	 * The MONITORINFOEX structure adds a string member to contain a name for
	 * the display monitor.
	 */
	public class MONITORINFOEX extends Structure {
		 ...
		 * Most applications have no use for a display monitor name, and so can
	 ...
	 * {@link User32#EnumDisplayMonitors} function.
	 * <p>
	 * You can use the EnumDisplayMonitors function to enumerate the set of
	 * display monitors that intersect the visible region of a specified device
	 * context and, optionally, a clipping rectangle. To do this, set the hdc
	 * parameter to a non-NULL value, and set the lprcClip parameter as needed.
	 * </p>
	 * <p>
	 * You can also use the EnumDisplayMonitors function to enumerate one or
	 * more of the display monitors on the desktop, without supplying a device
	 * context. To do this, set the hdc parameter of EnumDisplayMonitors to NULL
	 * and set the lprcClip parameter as needed.
	 * </p>
	 * In all cases, EnumDisplayMonitors calls a specified MonitorEnumProc
	 * function once for each display monitor in the calculated enumeration set.
	 * The MonitorEnumProc function always receives a handle to the display
	 * monitor. If the hdc parameter of EnumDisplayMonitors is non-NULL, the
	 * MonitorEnumProc function also receives a handle to a device context whose
	 * color format is appropriate for the display monitor. You can then paint
	 * into the device context in a manner that is optimal for the display
	 * monitor.
	 */
	public interface MONITORENUMPROC extends StdCallCallback {
		/**
		 * @param hMonitor
		 *            A handle to the display monitor. This value will always be
		 *            non-NULL.
		 * @param hdcMonitor
		 *            A handle to a device context. The device context has color
		 *            attributes that are appropriate for the display monitor
		 *            identified by hMonitor. The clipping area of the device
		 *            context is set to the intersection of the visible region
		 *            of the device context identified by the hdc parameter of
		 *            EnumDisplayMonitors, the rectangle pointed to by the
		 *            lprcClip parameter of EnumDisplayMonitors, and the display
		 *            monitor rectangle.
		 * @param lprcMonitor
		 *            A pointer to a RECT structure. If hdcMonitor is non-NULL,
		 *            this rectangle is the intersection of the clipping area of
		 *            the device context identified by hdcMonitor and the
		 *            display monitor rectangle. The rectangle coordinates are
		 *            device-context coordinates. If hdcMonitor is NULL, this
		 *            rectangle is the display monitor rectangle. The rectangle
		 ...
		 * @return To continue the enumeration, return TRUE. To stop the
	 ...
	 * called the ExitWindowsEx function. Then it logs the user off. This flag
	 * can be used only by processes running in an interactive user's logon
	 * session.
	 */
	int EWX_LOGOFF = 0;
	 ...
	 * the power-off feature. The calling process must have the SE_SHUTDOWN_NAME
	 * privilege. For more information, see
	 * {@link com.sun.jna.platform.win32.User32#ExitWindowsEx}.
	 */
	int EWX_POWEROFF = 0x00000008;

	/**
	 * Shuts down the system and then restarts the system. The calling process
	 * must have the SE_SHUTDOWN_NAME privilege. For more information, see
	 * {@link com.sun.jna.platform.win32.User32#ExitWindowsEx}.
	 */
	 ...
	 * that have been registered for restart using the
	 * RegisterApplicationRestart function. These application receive the
	 * WM_QUERYENDSESSION message with lParam set to the ENDSESSION_CLOSEAPP
	 * value. For more information, see Guidelines for Applications.
	 */
	 ...
	 * processes have stopped. The calling process must have the
	 * SE_SHUTDOWN_NAME privilege. For more information, see
	 * {@link com.sun.jna.platform.win32.User32#ExitWindowsEx}. Specifying this
	 * flag will not turn off the power even if the system supports the
	 * power-off feature. You must specify EWX_POWEROFF to do this.
	 *
	 ...
	 *
	 * @see <a href=
	 *      "https://msdn.microsoft.com/en-us/library/windows/desktop/ms633502(v=vs.85).aspx">MSDN</a>
	 */
	int GA_PARENT = 1;
	 ...
	 *
	 * @see <a href=
	 *      "https://msdn.microsoft.com/en-us/library/windows/desktop/ms633588(v=vs.85).aspx">MSDN</a>
	 */
	int GCL_HICON = -14;
	 ...
	 * 
	 * @see <A HREF=
	 *      "https://msdn.microsoft.com/en-us/library/windows/desktop/ms645568(v=vs.85).aspx"></A>
	 */
