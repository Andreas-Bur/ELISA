...
public interface Crypt32 extends StdCallLibrary {
	 ...
	 * 
	 * @param pDataIn
	 *            Pointer to a DATA_BLOB structure that contains the plaintext
	 *            to be encrypted.
	 * @param szDataDescr
	 *            String with a readable description of the data to be
	 *            encrypted. This description string is included with the
	 *            encrypted data. This parameter is optional and can be set to
	 *            NULL, except on Windows 2000.
	 * @param pOptionalEntropy
	 *            Pointer to a DATA_BLOB structure that contains a password or
	 *            other additional entropy used to encrypt the data. The
	 *            DATA_BLOB structure used in the encryption phase must also be
	 *            used in the decryption phase. This parameter can be set to
	 ...
	 *            information about where and when prompts are to be displayed
	 *            and what the content of those prompts should be. This
	 *            parameter can be set to NULL in both the encryption and
	 *            decryption phases.
	 * @param dwFlags
	 *            One of CRYPTPROTECT_LOCAL_MACHINE, CRYPTPROTECT_UI_FORBIDDEN,
	 *            CRYPTPROTECT_AUDIT, CRYPTPROTECT_VERIFY_PROTECTION.
	 * @param pDataOut
	 *            Pointer to a DATA_BLOB structure that receives the encrypted
	 *            data. When you have finished using the DATA_BLOB structure,
	 *            free its pbData member by calling the LocalFree function.
	 ...
	 *         call GetLastError.
	 */
	public boolean CryptProtectData(DATA_BLOB pDataIn, String szDataDescr, DATA_BLOB pOptionalEntropy, Pointer pvReserved,
	 ...
	 * 
	 * @param pDataIn
	 *            Pointer to a DATA_BLOB structure that holds the encrypted
	 *            data. The DATA_BLOB structure's cbData member holds the length
	 *            of the pbData member's byte string that contains the text to
	 *            be encrypted.
	 * @param szDataDescr
	 *            Pointer to a string-readable description of the encrypted data
	 *            included with the encrypted data. This parameter can be set to
	 *            NULL. When you have finished using ppszDataDescr, free it by
	 *            calling the LocalFree function.
	 * @param pOptionalEntropy
	 *            Pointer to a DATA_BLOB structure that contains a password or
	 *            other additional entropy used when the data was encrypted.
	 *            This parameter can be set to NULL; however, if an optional
	 *            entropy DATA_BLOB structure was used in the encryption phase,
	 *            that same DATA_BLOB structure must be used for the decryption
	 *            phase.
	 * @param pvReserved
	 ...
	 * @param dwFlags
	 *            DWORD value that specifies options for this function. This
	 *            parameter can be zero, in which case no option is set, or
	 *            CRYPTPROTECT_UI_FORBIDDEN.
	 * @param pDataOut
	 *            Pointer to a DATA_BLOB structure where the function stores the
	 *            decrypted data. When you have finished using the DATA_BLOB
	 *            structure, free its pbData member by calling the LocalFree
	 *            function.
	 ...
	 * @see <a href=
	 *      "http://msdn.microsoft.com/en-us/library/bb736347(v=vs.85).aspx">MSDN</a>
	 */
	 ...
	 *            system store, a new system store will be created and used.
	 ...
	 *            TRUE if this is to be a detached signature. Otherwise, FALSE.
	 *            If this parameter is set to TRUE, only the signed hash is
	 *            encoded in pbSignedBlob. Otherwise, both rgpbToBeSigned and
	 *            the signed hash are encoded.
	 * @param cToBeSigned
	 ...
	 *            contains the size, in bytes, of the signed and encoded
	 *            message.
	 ...
	 *
	 * @param pVerifyPara
	 *            A pointer to a CRYPT_VERIFY_MESSAGE_PARA structure that
	 *            contains verification parameters.
	 * @param signerIndex
	 *            The index of the de sired signature. There can be more than
	 *            one signature. CryptVerifyMessageSignature can be called
	 *            repeatedly, incrementing dwSignerIndex each time.
	 *
	 *            <p>
	 *            Set this pa rameter to zero for the first signer, or if there
	 *            is only one signer. If the function returns FALSE, and
	 *            GetLastError returns CRYPT_E_NO_SIGNER, the previous call
	 *            processed the last signer of the message.
	 *            </p>
	 * @param pbSignedBlob
	 *            A pointer to a buffe r that contains the signed message.
	 * @param cbSignedBlob
	 ...
	 *
	 *            <p>
	 *            This parameter can be NULL if the decoded message is not
	 *            needed for additional processing or to set the size of the
	 *            message for memory allocation purposes. For more information,
	 *            see Retrieving Data of Unknown Length.
	 *            </p>
	 *
	 * @param pcbDecoded
	 *            A pointer to a DWO RD value that specifies the size, in bytes,
	 *            of the pbDecoded buffer. When the function returns, this DWORD
	 *            contains the size, in bytes, of the decoded message. The
	 *            decoded message will not be returned if this parameter is
	 *            NULL.
	 * @param ppSignerCert
	 *            The address of a CER T_CONTEXT structure pointer that receives
	 *            the certificate of the signer. When you have finished using
	 *            this structure, free it by passing this pointer to the
	 *            CertFreeCertificateContext function. This parameter can be
	 *            NULL if the signer's certificate is not needed.
	 * @return If the function succeeds, the function returns nonzero. This does
	 *         not necessarily mean that the signature was verified. In the case
	 *         of a detached message, the variable pointe d to by pcbDecoded
	 *         will contain zero. In this case, this funct ion will return
	 *         nonzero, but the signature is not verified . To verify the
	 *         signature of a detached message, use the
	 *         CryptVerifyDetachedMessageSignature function.
	 *
	 *         <p>
	 *         If the function succeeds, the function returns nonzero. This does
	 *         not necessarily mean that the signature was verified. In the case
	 *         of a detached message, the variable pointed to by pcbDecoded will
	 *         contain zero. In this case, this function will return nonzero,
	 *         but the signature is not verified. To verify the signature of a
	 *         detached message, use t he CryptVerifyDetachedMessageSignature
	 *         function.
	 */
	 ...
	 *            If hChainEngine is NULL, the default chain engine,
	 ...
	 *            context will be the zero-index element in the first simple
	 *            chain.
	 * @param pTime
	 *            A pointer to a FILETIME variable that indicates the time for
	 *            which the chain is to be validated. Note that the time does
	 *            not affect trust list, revocation, or root store checking. The
	 *            current system time is used if NULL is passed to this
	 *            parameter. Trust in a particular certificate being a trusted
	 *            root is based on the current state of the root store and not
	 *            the state of the root store at a time passed in by this
	 *            parameter. For revocation, a certificate revocation list
	 *            (CRL), itself, must be valid at the current time. The value of
	 *            this parameter is used to determine whether a certificate
	 *            listed in a CRL has been revoked.
	 * @param hAdditionalStore
	 *            A handle to any additional store to search for supporting
	 *            certificates and certificate trust lists (CTLs). This
	 *            parameter can be NULL if no additional store is to be
	 *            searched.
	 * @param pChainPara
	 *            A pointer to a CERT_CHAIN_PARA structure that includes
	 *            chain-building parameters.
	 * @param dwFlags
	 *            Flag values that indicate special processing. This parameter
	 *            can be a combination of one or more of the following flags.
	 * @param pvReserved
	 *            This parameter is reserved and must be NULL.
	 * @param ppChainContext
	 *            The address of a pointer to the chain context created. When
	 *            you have finished using the chain context, release the chain
	 *            by calling the CertFreeCertificateChain function.
	 ...
	 * 
	 * @param pChainContext
	 *            A pointer to a CERT_CHAIN_CONTEXT certificate chain context to
	 *            be freed. If the reference count on the context reaches zero,
	 ...
	 *            default is to close the store with memory remaining allocated
	 *            for contexts that have not been freed. In this case, no check
	 *            is made to determine whether memory for contexts remains
	 *            allocated.
	 * @return If the function succeeds, the return value is TRUE. If the
	 *         function fails, the return value is FALSE.
	 */
	 ...
	 *            The certificate encoding type that was used to encode the
	 *            name. The message encoding type identifier, contained in the
	 ...
	 *            string. The size of this buffer is specified in the csz
	 *            parameter.
	 * @param csz
	 ...
	 *         terminating null character. If psz is NULL or csz is zero,
	 *         returns the required size of the destination string.
	 */
	 ...
	 *
	 * @param pszPolicyOID
	 *            Current predefined verify chain policy structures are listed
	 *            in the following table.
	 * @param pChainContext
	 *            A pointer to a CERT_CHAIN_CONTEXT structure that contains a
	 *            chain to be verified.
	 * @param pPolicyPara
	 *            A pointer to a CERT_CHAIN_POLICY_PARA structure that provides
	 *            the policy verification criteria for the chain. The dwFlags
	 *            member of that structure can be set to change the default
	 *            policy checking behavior.
	 * @param pPolicyStatus
	 *            A pointer to a CERT_CHAIN_POLICY_STATUS structure where status
	 *            information on the chain is returned. OID-specific extra
	 *            status can be returned in the pvExtraPolicyStatus member of
	 *            this structure.
	 * @return The return value indicates whether the function was able to check
	 *         for the policy, it does not indicate whether the policy check
	 *         failed or passed.
	 *
	 *         If the chain can be verified for the specified policy, TRUE is
	 *         returned and the dwError member of the pPolicyStatus is updated.
	 *         A dwError of 0 (ERROR_SUCCESS or S_OK) indicates the chain
	 *         satisfies the specified policy.
	 *
	 *         If the chain cannot be validated, the return value is TRUE and
	 *         you need to verify the pPolicyStatus parameter for the actual
	 *         error.
	 *
	 *         A value of FALSE indicates that the function wasn't able to check
	 *         for the policy.
	 */
	 ...
	 *            message encoding types must be specified by combining them
	 *            with a bitwise-OR.
	 * @param dwFindFlags
	 *            Used with some dwFindType values to modify the search
	 *            criteria. For most dwFindType values, dwFindFlags is not used
	 ...
	 *
	 *         A non-NULL CERT_CONTEXT that CertFindCertificateInStore returns
	 *         must be freed by CertFreeCertificateContext or by being passed as
	 *         the pPrevCertContext parameter on a subsequent call to
	 ...
	 * @param szPassword
	 *            A string password used to decrypt and verify the PFX packet.
	 *            Whether set to a string of length greater than zero or set to
	 *            an empty string or to NULL, this value must be exactly the
	 *            same as the value that was used to encrypt the packet.
	 *
	 *            <p>
	 *            Beginning with Windows 8 and Windows Server 2012, if the PFX
	 *            packet was created in the PFXExportCertStoreEx function by
	 *            using the PKCS12_PROTECT_TO_DOMAIN_SIDS flag, the
	 *            PFXImportCertStore function attempts to decrypt the password
	 *            by using the Active Directory (AD) principal that was used to
	 *            encrypt it. The AD principal is specified in the pvPara
	 *            parameter. If the szPassword parameter in the
	 *            PFXExportCertStoreEx function was an empty string or NULL and
	 *            the dwFlags parameter was set to
	 *            PKCS12_PROTECT_TO_DOMAIN_SIDS, that function randomly
	 *            generated a password and encrypted it to the AD principal
	 *            specified in the pvPara parameter. In that case you should set
	 *            the password to the value, empty string or NULL, that was used
	 *            when the PFX packet was created. The PFXImportCertStore
	 *            function will use the AD principal to decrypt the random
	 *            password, and the randomly generated password will be used to
	 *            decrypt the PFX certificate.
	 *            </p>
	 *
	 *            <p>
	 *            When you have finished using the password, clear it from
	 *            memory by calling the SecureZeroMemory function. For more
	 *            information about protecting passwords, see Handling
	 *            Passwords.
	 *            </p>
	 *
	 * @param dwFlags
	 ...
	 *         certificate store that contains the imported certificates,
	 *         including available private keys.
	 *
	 *         <p>
	 *         If the function fails, that is, if the password parameter does
	 *         not contain an exact match with the password used to encrypt the
	 *         exported packet or if there were any other problems decoding the
	 *         PFX BLOB, the function returns NULL, and an error code can be
	 *         found by calling the GetLastError function.
	 *         </p>
	 *
	 * @see <a href=
