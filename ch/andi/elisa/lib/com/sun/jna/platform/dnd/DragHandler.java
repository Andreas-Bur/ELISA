 ...
 * cursor to "link" (prior to 1.6, the JRE behavior is to display a "not
 * allowed" cursor, even though the action actually depends on how the drop
 * target responds).
 * <a href="http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=4869264"> The
 * bug</a> is fixed in java 1.6.
 * <li>Disallow drops to targets if the non-default (user-requested) action is
 * not supported by the target, e.g. the user requests a "copy" when the target
 * only supports "move". This is generally the responsibility of the drop
 * handler, which decides whether or not to accept a drag. The DragHandler
 * provides static modifier state information since the drop handler doesn't
 * have access to it.
 * </ul>
 * NOTE: Fundamentally, the active action is determined by the drop handler in
 * {@link DropTargetDragEvent#acceptDrag}, but often client code simply relies
 * on {@link DropTargetDragEvent#getDropAction}.
 */
// TODO: separate into the the following drag pieces:
// * default cursor changing behavior fix (fixed in jre1.6) (global DSL)
// * drag unselected item (fixed in 1.6, flag in 1.5.0.05+) bug not an issue
// w/DragHandler
// * multi-selection drag workaround (if not using swing drag gesture
// recognizer)
...
public abstract class DragHandler implements DragSourceListener, DragSourceMotionListener, DragGestureListener {
	...
	/**
	 * {@link #getModifiers} returns this value when the current modifiers state
	 * is unknown.
	 */
	public static final int UNKNOWN_MODIFIERS = -1;

	/**
	 * {@link #getTransferable} returns this value when the current
	 * {@link Transferable} is unknown.
	 */
	...
	static final int LINK_MASK = OSX ? InputEvent.ALT_DOWN_MASK | InputEvent.META_DOWN_MASK
	...
	/**
	 * Used to communicate modifier state to {@link DropHandler}. Note that this
	 * field will only be accurate when a {@link DragHandler} in the same VM
	 * started the drag. Otherwise, {@link #UNKNOWN_MODIFIERS} will be returned.
	 * 
	...
	/**
	 * Used to communicate the current {@link Transferable} during a drag, if
	 * available. Work around absence of access to the data when dragging
	 * pre-1.5.
	 * 
	 * @param e
	 *            event
	...
	/**
	 * Enable drags from the given component, supporting the actions in the
	 * given action mask.
	 * 
	 * @param dragSource
	 *            source of the drag.
	...
	/**
	 * Override to control whether a drag is started. The default implementation
	 * disallows the drag if the user is applying modifiers and the
	 * user-requested action is not supported.
	 * 
	 * @param e
	 *            event
	...
	/**
	 * Override to provide an appropriate {@link Transferable} representing the
	 * data being dragged.
	 * 
	 * @param e
	 *            event
	...
	/**
	 * Override this to provide a custom image. The {@link Icon} returned by
	 * this method by default is <code>null</code>, which results in no drag
	 * image.
	 * 
	 * @param e
	 *            event
	 * @param srcOffset
	 *            set this to be the offset from the drag source component's
	 *            upper left corner to the image's upper left corner. For
	 *            example, when dragging a row from a list, the offset would be
	 *            the row's bounding rectangle's (x,y) coordinate.
	 *            <p>
	...
	/**
	 * Called when a user drag gesture is recognized. This method is responsible
	 * for initiating the drag operation.
	 * 
	 * @param e
	 *            event
	 */
	...
	/**
	 * Change the size of the given drag icon, if appropriate. When using a
	 * differently-sized drag icon, we also need to adjust the cursor offset
	 * within the icon.
	 * 
	 * @param icon
	 *            Icon to be scaled.
	...
	protected Icon scaleDragIcon(Icon icon, Point imageOffset) {
		/*
		 * if (icon != null && maxGhostSize != null) { if (icon.getIconWidth() >
		 * maxGhostSize.width || icon.getIconHeight() > maxGhostSize.height) {
		 * Icon scaled = new ScaledIcon(icon, maxGhostSize.width,
		 * maxGhostSize.height); double scale =
		 * (double)scaled.getIconWidth()/icon.getIconWidth(); imageOffset.x *=
		 * scale; imageOffset.y *= scale; return scaled; } }
		 */
		return icon;
	}

	/**
	 * Create an image from the given icon. The image is provided to the native
	 * handler if drag images are supported natively.
	 * 
	 * @param gc
	 *            current graphics configuration.
	 * @param icon
	 *            Icon on which to base the drag image.
	...
	private void describe(String type, DragSourceEvent e) {
		if (false) {
			...
			if (e instanceof DragSourceDragEvent) {
				...
				msg += ": src=" + actionString(ds.getSourceActions()) + " usr=" + actionString(ev.getUserAction()) + " tgt="
						+ actionString(ev.getTargetActions()) + " act=" + actionString(ev.getDropAction()) + " mods="
						+ ev.getGestureModifiersEx();
			} else {
				msg += ": e=" + e;
			}
