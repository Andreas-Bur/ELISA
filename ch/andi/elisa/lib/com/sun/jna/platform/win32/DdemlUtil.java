...
/**
 * DdemlUtil defines helper classes, that help with manageing DDE
 * communications.
 */
public abstract class DdemlUtil {
	/**
	 * StandaloneDdeClient is a convenience class, that wraps a DdeClient and a
	 * {@link User32Util.MessageLoopThread}. The DdeClient needs a running
	...
	public static class StandaloneDdeClient implements IDdeClient, Closeable {
		...
		@Override
		public IDdeConnectionList connectList(Ddeml.HSZ service, Ddeml.HSZ topic, IDdeConnectionList existingList,
	...
	private static class MessageLoopWrapper implements InvocationHandler {
		...
		private <V> V wrap(V delegate, Class clazz) {
			V messageLoopHandler = (V) Proxy.newProxyInstance(StandaloneDdeClient.class.getClassLoader(), new Class[] { clazz },
					loopThread.new Handler(delegate));
			V clientDelegate = (V) Proxy.newProxyInstance(StandaloneDdeClient.class.getClassLoader(), new Class[] { clazz },
	...
	public static class DdeConnection implements IDdeConnection {
		...
		@Override
		public Ddeml.HDDEDATA clientTransaction(Pointer data, int dataLength, Ddeml.HSZ item, int wFmt, int transaction,
			...
			Ddeml.HDDEDATA returnData = Ddeml.INSTANCE.DdeClientTransaction(data, dataLength, conv, item, wFmt, transaction,
					timeout, result);
			if (returnData == null) {
				throw DdemlException.create(client.getLastError());
			}
		...
		public Ddeml.HDDEDATA clientTransaction(Pointer data, int dataLength, String item, int wFmt, int transaction, int timeout,
		...
		@Override
		public void poke(Pointer data, int dataLength, Ddeml.HSZ item, int wFmt, int timeout, WinDef.DWORDByReference result,
				DWORD_PTR userHandle) {
			clientTransaction(data, dataLength, item, wFmt, Ddeml.XTYP_POKE, timeout, result, userHandle);
		}

		public void poke(Pointer data, int dataLength, String item, int wFmt, int timeout, WinDef.DWORDByReference result,
				DWORD_PTR userHandle) {
			HSZ itemHSZ = null;
			try {
				itemHSZ = client.createStringHandle(item);
		...
		@Override
		public Ddeml.HDDEDATA request(Ddeml.HSZ item, int wFmt, int timeout, WinDef.DWORDByReference result,
				DWORD_PTR userHandle) {
	...
	public static class DdeConnectionList implements IDdeConnectionList {
		...
		public IDdeConnection queryNextServer(IDdeConnection prevConnection) {
			Ddeml.HCONV conv = Ddeml.INSTANCE.DdeQueryNextServer(convList,
	...
	public static class DdeClient implements IDdeClient {
		...
		public IDdeConnectionList connectList(Ddeml.HSZ service, Ddeml.HSZ topic, IDdeConnectionList existingList,
				Ddeml.CONVCONTEXT ctx) {
			Ddeml.HCONVLIST convlist = Ddeml.INSTANCE.DdeConnectList(idInst, service, topic,
	...
	public interface AdvstartHandler {
		 ...
		 * </p>
		 * <p>
		 * If a client requests an advise loop on a topic name, item name, and
		 * data format for an advise loop that is already established, the
		 ...
		 *            uType - The transaction type.
		 * @param dataFormat
		 *            uFmt - The data format requested by the client.
		 * @param hconv
		 *            A handle to the conversation.
		 * @param topic
		 *            hsz1 - A handle to the topic name.
		 * @param item
	...
	public interface AdvstopHandler {
		 ...
		 *            uType - The transaction type.
		 * @param dataFormat
		 *            uFmt - The data format associated with the advise loop
		 *            being ended.
		 * @param hconv
		 *            A handle to the conversation.
		 * @param topic
	...
	public interface ConnectHandler {
		 ...
		 *            hsz1 - A handle to the topic name.
		 * @param service
		 *            hsz2 - A handle to the service name.
		 * @param convcontext
		 *            dwData1 - CONVCONTEXT structure that contains context
		 *            information for the conversation. If the client is not a
		 *            DDEML application, this parameter is NULL.
		 * @param sameInstance
		 *            dwData2 - Specifies whether the client is the same
		 *            application instance as the server.
	...
	public interface AdvreqHandler {
		 ...
		 *            uType - The transaction type.
		 * @param dataFormat
		 *            uFmt - The format in which the data should be submitted to
		 *            the client.
		 * @param hconv
		 *            A handle to the conversation.
		 * @param topic
		 *            hsz1 - A handle to the topic name.
		 * @param item
		 *            hsz2 - A handle to the item name that has changed.
		 * @param count
		 *            The count of XTYP_ADVREQ transactions that remain to be
		 *            processed on the same topic, item, and format name set
		 *            within the context of the current call to the
		 *            DdePostAdvise function. The count is zero if the current
		 *            XTYP_ADVREQ transaction is the last one. A server can use
		 *            this count to determine whether to create an
		 *            HDATA_APPOWNED data handle to the advise data.
		 *
		 *            <p>
		 *            This is set to CADV_LATEACK if the DDEML issued the
		 *            XTYP_ADVREQ transaction because of a late-arriving DDE_ACK
	...
	public interface RequestHandler {
		 ...
		 *
		 * @return Data for the changed item or NULL if transaction can't be
		 *         completed. If the server returns NULL, the client will
		 *         receive a DDE_FNOTPROCESSED flag.
		 */
	...
	public interface WildconnectHandler {
		 ...
		 *            uType - The transaction type.
		 * @param topic
		 *            A handle to the topic name. If this parameter is NULL, the
		 *            client is requesting a conversation on all topic names
		 *            that the server supports.
		 * @param service
		 *            A handle to the service name. If this parameter is NULL,
		 *            the client is requesting a conversation on all service
	...
	public interface AdvdataHandler {
		 ...
		 *            uType - The transaction type.
		 * @param dataFormat
		 *            uFmt - The format atom of the data sent from the server.
		 * @param hconv
		 *            A handle to the conversation.
		 * @param topic
		 *            hsz1 - A handle to the topic name.
		 ...
		 *            item name pair. This parameter is NULL if the client
		 *            specified the XTYPF_NODATA flag when it requested the
		 *            advise loop.
		 * @return A DDE callback function should return DDE_FACK if it
		 *         processes this transaction, DDE_FBUSY if it is too busy to
		 *         process this transaction, or DDE_FNOTPROCESSED if it rejects
		 *         this transaction.
		 */
	...
	public interface ErrorHandler {
		 ...
		 *            This parameter is NULL if the error is not associated with
		 *            a conversation.
		 * @param errorCode
		 *            dwData1 - The error code in the low-order word. Currently,
		 *            only the following error code is supported.
		 *            <p>
		 *            Only Ddeml.DMLERR_LOW_MEMRORY is known: Memory is low;
		 *            advise, poke, or execute data may be lost, or the system
		 *            may fail.
		 *            </p>
		 */
	...
	public interface RegisterHandler {
		/**
		 * A Dynamic Data Exchange (DDE) callback function, DdeCallback,
		 * receives the XTYP_REGISTER transaction type whenever a Dynamic Data
		 * Exchange Management Library (DDEML) server application uses the
		 * DdeNameService function to register a service name, or whenever a
		 * non-DDEML application that supports the System topic is started.
		 * 
		 * <p>
		 * <strong>Remarks</strong>
		 * </p>
		 ...
		 *            hsz1 - A handle to the base service name being registered.
		 * @param instanceSpecificServiceName
		 *            hsz2 - A handle to the instance-specific service name
		 *            being registered.
		 */
	...
	public interface XactCompleteHandler {
		 ...
		 *            completed transaction (if applicable) or NULL if no data
		 *            was exchanged during the transaction.
		 * @param hConv
		 *            - A handle to the conversation.
		 * @param topic
		 ...
		 * @param item
		 *            hsz2 - A handle to the item name involved in the completed
		 *            transaction.
		 * @param hdata
		 *            A handle to the data involved in the completed
		 *            transaction, if applicable. If the transaction was
		 *            successful but involved no data, this parameter is TRUE.
		 *            This parameter is NULL if the transaction was
		 *            unsuccessful.
		 * @param transactionIdentifier
		 *            dwData1 - The transaction identifier of the completed
		 *            transaction.
		 * @param statusFlag
		 *            dwData2 - Any applicable DDE_ status flags in the low
		 *            word. This parameter provides support for applications
		 *            dependent on DDE_APPSTATUS bits. It is recommended that
		 *            applications no longer use these bits � they may not be
		 *            supported in future versions of the DDEML.
		 */
	...
	public interface PokeHandler {
		 ...
		 *            uType - The transaction type.
		 * @param dataFormat
		 *            uFmt - The format of the data sent from the server.
		 * @param hconv
		 *            A handle to the conversation.
		 * @param topic
		 *            hsz1 - A handle to the topic name.
		 ...
		 *         it processes this transaction, the DDE_FBUSY flag if it is
		 *         too busy to process this transaction, or the
		 *         DDE_FNOTPROCESSED flag if it rejects this transaction.
		 *
		 */
	...
	public interface MonitorHandler {
		 ...
		 *            uType - The transaction type.
		 * @param hdata
		 *            A handle to a DDE object that contains information about
		 *            the DDE event. The application should use the
		 *            DdeAccessData function to obtain a pointer to the object.
		 * @param eventCode
		 *            dwData2 - The DDE event. This parameter can be one of the
		 *            following values:
		 * 
		 *            <table>
		 *            <tr>
		 ...
		 *            <td>MF_CALLBACKS<br>
		 *            0x08000000</td>
		 *            <td>The system sent a transaction to a DDE callback
		 *            function. The DDE object contains a MONCBSTRUCT structure
		 *            that provides information about the transaction.</td>
		 *            </tr>
		 *            <tr>
		 *            <td>MF_CONV<br>
		 *            0x40000000</td>
		 *            <td>A DDE conversation was established or terminated. The
		 *            DDE object contains a MONCONVSTRUCT structure that
		 *            provides information about the conversation.</td>
		 *            </tr>
		 *            <tr>
		 *            <td>MF_ERRORS<br>
		 *            0x10000000</td>
		 *            <td>A DDE error occurred. The DDE object contains a
		 ...
		 *            <td>MF_HSZ_INFO<br>
		 *            0x01000000</td>
		 *            <td>A DDE application created, freed, or incremented the
		 *            usage count of a string handle, or a string handle was
		 *            freed as a result of a call to the DdeUninitialize
		 *            function. The DDE object contains a MONHSZSTRUCT structure
		 *            that provides information about the string handle.</td>
		 *            </tr>
		 *            <tr>
		 *            <td>MF_LINKS<br>
		 *            0x20000000</td>
		 *            <td>A DDE application started or stopped an advise loop.
		 *            The DDE object contains a MONLINKSTRUCT structure that
		 *            provides information about the advise loop.</td>
		 *            </tr>
		 *            <tr>
		 *            <td>MF_POSTMSGS<br>
		 *            0x04000000</td>
		 *            <td>The system or an application posted a DDE message. The
		 *            DDE object contains a MONMSGSTRUCT structure that provides
		 *            information about the message.</td>
		 *            </tr>
		 *            <tr>
		 *            <td>MF_SENDMSGS<br>
		 *            0x02000000</td>
		 *            <td>The system or an application sent a DDE message. The
		 *            DDE object contains a MONMSGSTRUCT structure that provides
		 *            information about the message.</td>
		 *            </tr>
		 *            </table>
	...
	public static class DdeAdapter implements Ddeml.DdeCallback {
		...
		public WinDef.PVOID ddeCallback(int wType, int wFmt, Ddeml.HCONV hConv, Ddeml.HSZ hsz1, Ddeml.HSZ hsz2,
				Ddeml.HDDEDATA hData, BaseTSD.ULONG_PTR lData1, BaseTSD.ULONG_PTR lData2) {
			...
			try {
				...
				case Ddeml.XTYP_WILDCONNECT:
					...
					Ddeml.HSZPAIR[] hszPairs = onWildconnect(wType, hsz1, hsz2, convcontext,
							lData2 != null && lData2.intValue() != 0);
				...
				default:
					LOG.log(Level.FINE,
							String.format("Not implemented Operation - Transaction type: 0x%X (%s)", wType, transactionTypeName));
				}
		...
		private boolean onConnect(int transactionType, Ddeml.HSZ topic, Ddeml.HSZ service, Ddeml.CONVCONTEXT convcontext,
		...
		private Ddeml.HDDEDATA onAdvreq(int transactionType, int dataFormat, Ddeml.HCONV hconv, Ddeml.HSZ topic, Ddeml.HSZ item,
				int count) {
		...
		private Ddeml.HDDEDATA onRequest(int transactionType, int dataFormat, Ddeml.HCONV hconv, Ddeml.HSZ topic,
				Ddeml.HSZ item) {
		...
		private Ddeml.HSZPAIR[] onWildconnect(int transactionType, HSZ topic, HSZ service, CONVCONTEXT convcontext,
				boolean sameInstance) {
		...
		private void onXactComplete(int transactionType, int dataFormat, HCONV hConv, HSZ topic, HSZ item, HDDEDATA hdata,
				ULONG_PTR transactionIdentifier, ULONG_PTR statusFlag) {
	...
	public interface IDdeConnection extends Closeable {
		 ...
		 * 
		 * @param executeString
		 *            The string passed to the server for execution
		 * @param timeout
		 *            The maximum amount of time, in milliseconds, that the
		 *            client will wait for a response from the server
		 *            application in a synchronous transaction. This parameter
		 *            should be TIMEOUT_ASYNC for asynchronous transactions.
		 * @param result
		 *            A pointer to a variable that receives the result of the
		 *            transaction. An application that does not check the result
		 *            can use NULL for this value. For synchronous transactions,
		 *            the low-order word of this variable contains any
		 *            applicable DDE_ flags resulting from the transaction. This
		 *            provides support for applications dependent on
		 *            DDE_APPSTATUS bits. It is, however, recommended that
		 *            applications no longer use these bits because they may not
		 *            be supported in future versions of the Dynamic Data
		 *            Exchange Management Library (DDEML). For asynchronous
		 *            transactions, this variable is filled with a unique
		 *            transaction identifier for use with the
		 ...
		 * 
		 *            <p>
		 *            If an error occurs, a DdemlException is raised with the
		 ...
		 *            (HDDEDATA) to pass to the server and in that case the
		 *            cbData parameter should be set to -1. This parameter is
		 *            required only if the wType parameter is XTYP_EXECUTE or
		 ...
		 *            DdeCreateDataHandle function, employing the same data
		 *            format specified in the wFmt parameter.
		 *            </p>
		 *
		 * @param dataLength
		 *            The length, in bytes, of the data pointed to by the pData
		 *            parameter, including the terminating NULL, if the data is
		 *            a string. A value of -1 indicates that pData is a data
		 *            handle that identifies the data being sent.
		 *
		 * @param item
		 *            A handle to the data item for which data is being
		 *            exchanged during the transaction. This handle must have
		 *            been created by a previous call to the
		 *            DdeCreateStringHandle function. This parameter is ignored
		 *            (and should be set to 0L) if the wType parameter is
		 *            XTYP_EXECUTE.
		 *
		 * @param wFmt
		 ...
		 *
		 *            <p>
		 *            If the transaction specified by the wType parameter does
		 *            not pass data or is XTYP_EXECUTE, this parameter should be
		 *            zero.
		 *            </p>
		 *
		 *            <p>
		 ...
		 *            XTYP_ADVSTART, XTYP_ADVSTOP, XTYP_REQUEST), the wFmt value
		 *            must be either a valid predefined (CF_) DDE format or a
		 ...
		 *
		 * @param item
		 *            The data item for which data is being exchanged during the
		 *            transaction. This parameter is ignored (and should be set
		 *            to NULL) if the wType parameter is XTYP_EXECUTE.
		 *
		 * @param wFmt
		 *            The standard clipboard format in which the data item is
		 *            being submitted or requested.
		 ...
		 *         that identifies the data for successful synchronous
		 *         transactions in which the client expects data from the
		 *         server. The return value is nonzero for successful
		 *         asynchronous transactions and for synchronous transactions in
		 *         which the client does not expect data. The return value is
		 *         zero for all unsuccessful transactions.
		 *
		 *         <p>
		 *         If an error occurs, a DdemlException is raised with the
		 *         appropriate error code:
		 ...
		 *            <tr>
		 *            <td>XTYP_ADVSTART</td>
		 *            <td>Begins an advise loop. Any number of distinct advise
		 *            loops can exist within a conversation. An application can
		 *            alter the advise loop type by combining the XTYP_ADVSTART
		 *            transaction type with one or more of the following flags:
		 *            <dl>
		 *            <dt>XTYPF_NODATA.</dt>
		 *            <dd>Instructs the server to notify the client of any data
		 *            changes without actually sending the data. This flag gives
		 *            the client the option of ignoring the notification or
		 *            requesting the changed data from the server.</dd>
		 *            <dt>XTYPF_ACKREQ.</dt>
		 *            <dd>Instructs the server to wait until the client
		 *            acknowledges that it received the previous data item
		 *            before sending the next data item. This flag prevents a
		 *            fast server from sending data faster than the client can
		 *            process it.</dd>
		 *            </dl>
		 *            </td>
		 *            </tr>
		 ...
		 * 
		 * <p>
		 * Note: This wraps the DdeDisconnect function and aligns the name with
		 * the Closable-wording.
		 * </p>
		 * 
		 * <p>
		 * If the method fails a DdeException will be raised with the
		 ...
		 *
		 *            <p>
		 *            A server application can disable the following
		 *            transactions:
		 *            </p>
		 *            <ul>
		 *            <li>XTYP_ADVSTART</li>
		 ...
		 *            </ul>
		 *            <p>
		 *            A client application can disable the following
		 *            transactions:
		 *            </p>
		 *            <ul>
		 *            <li>XTYP_ADVDATA</li>
		 ...
		 *            </tr>
		 *            <tr>
		 *            <td>EC_QUERYWAITING</td>
		 *            <td>Determines whether any transactions are in the queue
		 *            for the specified conversation.</td>
		 *            </tr>
		 *            </table>
		 *
		 ...
		 *         transaction queue contains one or more unprocessed
		 *         transactions that are not being processed, the return value
		 ...
		 *            specified by the hUser parameter. An application should
		 *            set this parameter to QID_SYNC to associate hUser with the
		 *            conversation identified by the hConv parameter.
		 * @param hUser
		 ...
		 *            parameter should be a transaction identifier returned by
		 *            the DdeClientTransaction function. For synchronous
	...
	/**
	 * The IDdeClient defines functions that wrap a ddeml instance. and are not
	 * tied to conversation.
	 */
	public interface IDdeClient extends Closeable {
		 ...
		 *            provide special instructions to DdeInitialize. The CBF_
		 *            flags specify filters that prevent specific types of
		 *            transactions from reaching the callback function. The MF_
		 *            flags specify the types of DDE activity that a DDE
		 *            monitoring application monitors. Using these flags
		 *            enhances the performance of a DDE application by
		 ...
		 *            <td>
		 *            <p>
		 *            Makes it possible for the application to monitor DDE
		 *            activity in the system. This flag is for use by DDE
		 *            monitoring applications. The application specifies the
		 *            types of DDE activity to monitor by combining one or more
		 *            monitor flags with the APPCLASS_MONITOR flag. For details,
		 ...
		 *            <td>
		 *            <p>
		 *            Registers the application as a standard (nonmonitoring)
		 *            DDEML application.
		 *            </p>
		 *            </td>
		 *            </tr>
		 ...
		 *            conversation. The application can only be a client. This
		 *            flag reduces consumption of resources by the DDEML. It
		 *            includes the functionality of the CBF_FAIL_ALLSVRXACTIONS
		 *            flag.
		 *            </p>
		 *            </td>
		 *            </tr>
		 ...
		 *            application has created its string handles and registered
		 *            its service names or has turned off filtering by a
		 *            subsequent call to the DdeNameService or DdeInitialize
		 *            function. This flag is always in effect when an
		 *            application calls DdeInitialize for the first time,
		 *            regardless of whether the application specifies the flag.
		 *            On subsequent calls to DdeInitialize, not specifying this
		 *            flag turns off the application's service-name filters, but
		 *            specifying it turns on the application's service name
		 *            filters.
		 *            </p>
		 *            </td>
		 *            </tr>
		 ...
		 *            transactions. The system returns DDE_FNOTPROCESSED to each
		 *            client that sends a transaction to this application. This
		 ...
		 *            XTYP_ADVSTART and XTYP_ADVSTOP transactions. The system
		 *            returns DDE_FNOTPROCESSED to each client that sends an
		 ...
		 *            and XTYP_WILDCONNECT transactions.
		 *            </p>
		 *            </td>
		 *            </tr>
		 *            <tr>
		 ...
		 *            transactions. The system returns DDE_FNOTPROCESSED to a
		 *            client that sends an XTYP_EXECUTE transaction to the
		 *            server.
		 *            </p>
		 *            </td>
		 *            </tr>
		 ...
		 *            transactions. The system returns DDE_FNOTPROCESSED to a
		 *            client that sends an XTYP_REQUEST transaction to the
		 *            server.
		 *            </p>
		 *            </td>
		 *            </tr>
		 ...
		 *            <p>
		 *            Prevents the callback function from receiving XTYP_CONNECT
		 *            transactions from the application's own instance. This
		 *            flag prevents an application from establishing a DDE
		 *            conversation with its own instance. An application should
		 *            use this flag if it needs to communicate with other
		 ...
		 *            handle or whenever a string handle is freed as a result of
		 *            a call to the DdeUninitialize function.
		 *            </p>
		 *            </td>
		 *            </tr>
		 *            <tr>
		 ...
		 *
		 *            <p>
		 *            If the function failsa DdemlException is raised with one
		 *            of the following errroCodes:
		 *            </p>
		 *            <ul>
		 ...
		 *
		 * @param value
		 *            The string for which a handle is to be created. This
		 *            string can be up to 255 characters. The reason for this
		 *            limit is that DDEML string management functions are
		 ...
		 *            createStringHandle function. A NULL value will be silently
		 *            ignored.
		 * 
		 ...
		 *            server is registering or unregistering. An application
		 *            that is unregistering all of its service names should set
		 *            this parameter to 0L.
		 * @param afCmd
		 ...
		 *            </tr>
		 *            <tr>
		 *            <td>DNS_UNREGISTER</td>
		 *            <td>Unregisters the error code service name. If the hsz1
		 *            parameter is 0L, all service names registered by the
		 *            server will be unregistered.</td>
		 *            </tr>
		 *            <tr>
		 *            <td>DNS_FILTERON</td>
		 *            <td>Turns on service name initiation filtering. The filter
		 *            prevents a server from receiving XTYP_CONNECT transactions
		 *            for service names it has not registered. This is the
		 *            default setting for this filter. <br>
		 * 			<br>
		 *            If a server application does not register any service
		 *            names, the application cannot receive XTYP_WILDCONNECT
		 *            transactions.</td>
		 *            </tr>
		 *            <tr>
		 *            <td>DNS_FILTEROFF</td>
		 *            <td>Turns off service name initiation filtering. If this
		 *            flag is specified, the server receives an XTYP_CONNECT
		 *            transaction whenever another DDE application calls the
		 *            DdeConnect function, regardless of the service name.</td>
		 *            </tr>
		 *            </table>
		 *            <p>
		 *            If the function fails, a DdemlException is raised with the
		 ...
		 *            registering or unregistering. An application that is
		 ...
		 *
		 * @param service
		 *            A handle to the string that specifies the service name of
		 *            the server application with which a conversation is to be
		 *            established. This handle must have been created by a
		 *            previous call to the DdeCreateStringHandle function. If
		 *            this parameter is 0L, a conversation is established with
		 *            any available server.
		 *
		 * @param topic
		 *            A handle to the string that specifies the name of the
		 *            topic on which a conversation is to be established. This
		 *            handle must have been created by a previous call to
		 *            DdeCreateStringHandle. If this parameter is 0L, a
		 *            conversation on any topic supported by the selected server
		 *            is established.
		 *
		 * @param convcontext
		 ...
		 *            structure during the XTYP_CONNECT or XTYP_WILDCONNECT
		 *            transaction.
		 *
		 ...
		 *
		 * @param service
		 *            The service name of the server application with which a
		 *            conversation is to be established. If this parameter is
		 *            NULL, a conversation is established with any available
		 *            server.
		 *
		 * @param topic
		 *            The name of the topic on which a conversation is to be
		 *            established. If this parameter is NULL, a conversation on
		 *            any topic supported by the selected server is established.
		 *
		 * @param convcontext
		 *            A pointer to the CONVCONTEXT structure that contains
		 *            conversation context information. If this parameter is
		 ...
		 *            pointed to by pSrc. (include the terminating NULL, if the
		 *            data is a string). If this parameter is zero, the pSrc
		 *            parameter is ignored.
		 *
		 * @param cbOff
		 *            An offset, in bytes, from the beginning of the buffer
		 *            pointed to by the pSrc parameter. The data beginning at
		 *            this offset is copied from the buffer to the DDE object.
		 *
		 * @param hszItem
		 *            A handle to the string that specifies the data item
		 *            corresponding to the DDE object. This handle must have
		 *            been created by a previous call to the
		 ...
		 *            function creates. This flag enables the application to
		 *            share the data handle with other DDEML applications rather
		 *            than creating a separate handle to pass to each
		 *            application. If this flag is specified, the application
		 *            must eventually free the shared memory object associated
		 *            with the handle by using the DdeFreeDataHandle function.
		 *            If this flag is not specified, the handle becomes invalid
		 *            in the application that created the handle after the data
		 *            handle is returned by the application's DDE callback
		 *            function or is used as a parameter in another DDEML
		 *            function.
		 *
		 * @return If the function succeeds, the return value is a data handle.
		 *
		 ...
		 *            The additional data is copied to the object beginning at
		 *            this offset.
		 *
		 * @return If the function succeeds, the return value is a new handle to
		 *         the DDE object. The new handle is used in all references to
		 *         the object.
		 *
		 *         <p>
		 *         If the function fails a DdeException is raised with the
		 ...
		 *            parameter is NULL, the DdeGetData function returns the
		 *            amount of data, in bytes, that would be copied to the
		 *            buffer.
		 *
		 * @param cbMax
		 *            The maximum amount of data, in bytes, to copy to the
		 *            buffer pointed to by the pDst parameter. Typically, this
		 *            parameter specifies the length of the buffer pointed to by
		 *            pDst.
		 *
		 * @param cbOff
		 ...
		 *         the size, in bytes, of the memory object associated with the
		 *         data handle or the size specified in the cbMax parameter,
		 *         whichever is lower.
		 *
		 *         <p>
		 *         If the function fails a DdeException is raised with the
		 ...
		 *            of the DDE object identified by the hData parameter. If
		 ...
		 *            send notifications for all topics with active advise
		 *            loops, an application can set this parameter to 0L.
		 *
		 * @param hszItem
		...
		/**
		 * Abandons all asynchronous transaction and releases all resources
		 ...
		 *            A handle to the string that specifies the service name of
		 *            the server application with which a conversation is to be
		 *            established. If this parameter is 0L, the system attempts
		 *            to establish conversations with all available servers that
		 ...
		 *            DdeCreateStringHandle function. If this parameter is 0L,
		 *            the system will attempt to establish conversations on all
		 *            topics supported by the selected server (or servers).
		 * 
		 * @param existingList
		 *            An existinct conversation list to be enumerated. This
		 *            parameter should be NULL if a new conversation list is to
		 *            be established.
		 *
		 * @param ctx
		 *            A pointer to the CONVCONTEXT structure that contains
		 ...
		 *            application with which a conversation is to be
		 *            established. If this parameter is NULL, the system
		 *            attempts to establish conversations with all available
		 ...
		 *            on all topics supported by the selected server (or
		 *            servers).
		 * 
		 * @param existingList
		 *            An existinct conversation list to be enumerated. This
	...
	public interface IDdeConnectionList extends Closeable {
		 ...
		 *            this function. If this parameter is NULL, the function
		 *            returns the first conversation handle in the list.
		 *
		 * @return If the list contains any more conversation handles, the
		 *         return value is the next conversation IDdeConnection in the
		 ...
		 *
		 * <p>
		 * If the function fails a DdeException is raised with the appropriate
		 * errorCode:
		 * </p>
		 * 
		 * <ul>
