...
public abstract class Netapi32Util {
	...
	/**
	 * Returns the name of the primary domain controller (PDC) on the current
	 * computer.
	 * 
	 ...
	 * @return Name of the primary domain controller.
	 */
	...
	public static LocalGroup[] getLocalGroups(String serverName) {
		...
		try {
			int rc = Netapi32.INSTANCE.NetLocalGroupEnum(serverName, 1, bufptr, LMCons.MAX_PREFERRED_LENGTH, entriesRead,
					totalEntries, null);
	...
	public static Group[] getGlobalGroups(String serverName) {
		...
		IntByReference totalEntries = new IntByReference();
		try {
			int rc = Netapi32.INSTANCE.NetGroupEnum(serverName, 1, bufptr, LMCons.MAX_PREFERRED_LENGTH, entriesRead, totalEntries,
					null);
			if (LMErr.NERR_Success != rc || bufptr.getValue() == Pointer.NULL) {
				throw new Win32Exception(rc);
			}
	...
	public static Group[] getUserLocalGroups(String userName, String serverName) {
		...
		IntByReference totalentries = new IntByReference();
		try {
			int rc = Netapi32.INSTANCE.NetUserGetLocalGroups(serverName, userName, 0, 0, bufptr, LMCons.MAX_PREFERRED_LENGTH,
	...
	public static Group[] getUserGroups(String userName, String serverName) {
		...
		IntByReference totalentries = new IntByReference();
		try {
			int rc = Netapi32.INSTANCE.NetUserGetGroups(serverName, userName, 0, bufptr, LMCons.MAX_PREFERRED_LENGTH, entriesread,
					totalentries);
			if (rc != LMErr.NERR_Success) {
				throw new Win32Exception(rc);
			}
	 ...
	 * 
	 * @return Domain controller information.
	 */
	...
	public static class DomainTrust {
		...
		/**
		 * Contains the string representation of the security identifier of the
		...
		public GUID DomainGuid;
		/**
		 * Contains the string representation of the GUID of the domain
		...
		/**
		 * The domain represented by this structure is a member of the same
		 * forest as the server specified in the ServerName parameter of the
		...
		/**
		 * The domain represented by this structure is directly trusted by the
		 * domain that the server specified in the ServerName parameter of the
		...
		/**
		 * The domain represented by this structure is the root of a tree and a
		 * member of the same forest as the server specified in the ServerName
		 * parameter of the DsEnumerateDomainTrusts function.
		 * 
		 * @return True or false.
		 */
		...
		/**
		 * The domain represented by this structure is the primary domain of the
		 * server specified in the ServerName parameter of the
		 * DsEnumerateDomainTrusts function.
		 * 
		 * @return True or false.
		 */
		...
		/**
		 * The domain represented by this structure is running in the Windows
		 * 2000 native mode.
		 * 
		 * @return True or false.
		 */
		...
		/**
		 * The domain represented by this structure directly trusts the domain
	 ...
	 * 
	 * @return An array of domain trusts.
	 */
	...
	public static DomainTrust[] getDomainTrusts(String serverName) {
		...
		try {
			...
			DS_DOMAIN_TRUSTS[] domainTrusts = (DS_DOMAIN_TRUSTS[]) domainTrustRefs
					.toArray(new DS_DOMAIN_TRUSTS[domainTrustCount.getValue()]);
