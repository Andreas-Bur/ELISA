...
public class W32FileMonitor extends FileMonitor {
	...
	protected synchronized void watch(File file, int eventMask, boolean recursive) throws IOException {
		...
		HANDLE handle = klib.CreateFile(file.getAbsolutePath(), WinNT.FILE_LIST_DIRECTORY, mask, null, WinNT.OPEN_EXISTING, flags,
				null);
	...
	protected synchronized void unwatch(File file) {
		...
		if (finfo != null) {
			...
			klib.CloseHandle(finfo.handle); // TODO check error code if failed
											// to close
		}
	}
