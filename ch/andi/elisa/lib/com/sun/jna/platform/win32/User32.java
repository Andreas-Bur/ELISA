...
public interface User32 extends StdCallLibrary, WinUser, WinNT {
	...
	/**
	 * This function retrieves the coordinates of a window's client area. The
	 ...
	 *         function fails if hWnd is an invalid window handle or lpMsg is an
	 *         invalid pointer. To get extended error information, call
	 *         GetLastError.
	 *         </p>
	 */
	 ...
	 * 
	 * @param idThread
	 *            The identifier of the thread to which the message is to be
	 *            posted.
	 * 
	 *            <p>
	 * 			The function fails if the specified thread does not have a
	 *            message queue. The system creates a thread's message queue
	 *            when the thread makes its first call to one of the User or GDI
	 *            functions.
	 *            </p>
	 * 
	 *            <p>
	 * 			Message posting is subject to UIPI. The thread of a process
	 *            can post messages only to posted-message queues of threads in
	 *            processes of lesser or equal integrity level.
	 *            </p>
	 * 
	 *            <p>
	 * 			This thread must have the SE_TCB_NAME privilege to post a
	 *            message to a thread that belongs to a process with the same
	 *            locally unique identifier (LUID) but is in a different
	 *            desktop. Otherwise, the function fails and returns
	 *            ERROR_INVALID_THREAD_ID.
	 *            </p>
	 * 
	 *            <p>
	 * 			This thread must either belong to the same desktop as the
	 *            calling thread or to a process with the same LUID. Otherwise,
	 ...
	 *         </p>
	 *         <p>
	 * 		GetLastError returns ERROR_INVALID_THREAD_ID if idThread is not a
	 *         valid thread identifier, or if the thread specified by idThread
	 *         does not have a message queue.
	 *         </p>
	 *         <p>
	 * 		GetLastError returns ERROR_NOT_ENOUGH_QUOTA when the message
	 *         limit is hit.
	 *         </p>
	 */
	 ...
	 *         <ul>
	 *         <li>{@link WinUser#MOD_ALT} Either ALT key must be held
	 *         down.</li>
	 ...
	 *         notifications.
	 *         <p>
	 * 		<b> Windows Vista and Windows XP/2000: This flag is not
	 *         supported.</b>
	 *         </p>
	 * 		</li>
	 ...
	 *         <li>if one of the controls in the dialog template is not
	 *         registered, or its window window procedure fails WM_CREATE or
	 *         WM_NCCREATE</li>
	 *         </ul>
	 */
	...
	/**
	 * Retrieves a handle to the display monitor that contains a specified
	 * point.
	 * 
	 * @param pt
	 ...
	 *            intersect any display monitor. This parameter can be one of
	 ...
	 *            </ul>
	 * @return If the point is contained by a display monitor, the return value
	 *         is an HMONITOR handle to that display monitor. If the point is
	 *         not contained by a display monitor, the return value depends on
	 ...
	 * 
	 * @param lprc
	 *            A pointer to a RECT structure that specifies the rectangle of
	 ...
	 *            <li>MONITOR_DEFAULTTOPRIMARY</li>
	 *            </ul>
	 * @return If the rectangle intersects one or more display monitor
	 *         rectangles, the return value is an HMONITOR handle to the display
	 *         monitor that has the largest area of intersection with the
	 *         rectangle. If the rectangle does not intersect a display monitor,
	 ...
	 *            <li>MONITOR_DEFAULTTOPRIMARY</li>
	 *            </ul>
	 * @return If the window intersects one or more display monitor rectangles,
	 *         the return value is an HMONITOR handle to the display monitor
	 *         that has the largest area of intersection with the window. If the
	 *         window does not intersect a display monitor, the return value
	 ...
	 * @param lpmi
	 *            A pointer to a {@link WinUser.MONITORINFO} structure that
	 *            receives information about the specified display monitor.
	 * @return If the function succeeds, the return value is nonzero. If the
	 *         function fails, the return value is zero.
	 */
	 ...
	 *            A handle to the display monitor of interest.
	 * @param lpmi
	 *            A pointer to a {@link WinUser.MONITORINFOEX} structure that
	 *            receives information about the specified display monitor.
	 * @return If the function succeeds, the return value is nonzero. If the
	 *         function fails, the return value is zero.
	 */
	...
	/**
	 * Enumerates display monitors (including invisible pseudo-monitors
	 * associated with the mirroring drivers) that intersect a region formed by
	 * the intersection of a specified clipping rectangle and the visible region
	 * of a device context. EnumDisplayMonitors calls an application-defined
	 ...
	 *
	 * @param hdc
	 *            A handle to a display device context that defines the visible
	 *            region of interest. If this parameter is NULL, the hdcMonitor
	 *            parameter passed to the callback function will be NULL, and
	 *            the visible region of interest is the virtual screen that
	 *            encompasses all the displays on the desktop.
	 * @param lprcClip
	 *            A pointer to a RECT structure that specifies a clipping
	 *            rectangle. The region of interest is the intersection of the
	 *            clipping rectangle with the visible region specified by hdc.
	 *            If hdc is non-NULL, the coordinates of the clipping rectangle
	 *            are relative to the origin of the hdc. If hdc is NULL, the
	 *            coordinates are virtual-screen coordinates. This parameter can
	 *            be NULL if you don't want to clip the region specified by hdc.
	 * @param lpfnEnum
	 *            A pointer to an application-defined callback function.
	 * @param dwData
	 *            Application-defined data that EnumDisplayMonitors passes
	 *            directly to the lpfnEnum function.
	 * @return If the function succeeds, the return value is nonzero. If the
	 *         function fails, the return value is zero.
	 */
	 ...
	 *            A handle to the window.
	 * @param lpwndpl
	 *            A pointer to a WINDOWPLACEMENT structure that specifies the
	 ...
	 * desired client-rectangle size. The window rectangle can then be passed to
	 * the CreateWindow function to create a window whose client area is the
	 * desired size.
	 *
	 ...
	 * encloses the window, which includes the client area and the nonclient
	 * area.
	 *
	 * The AdjustWindowRect function does not add extra space when a menu bar
	 ...
	 *
	 * @param lpRect
	 *            A pointer to a RECT structure that contains the coordinates of
	 *            the top-left and bottom-right corners of the desired client
	 *            area. When the function returns, the structure contains the
	 *            coordinates of the top-left and bottom-right corners of the
	 *            window to accommodate the desired client area.
	 * @param dwStyle
	 *            The window style of the window whose required size is to be
	 *            calculated. Note that you cannot specify the WS_OVERLAPPED
	 *            style.
	 * @param bMenu
	 ...
	 * wraps to two or more rows.
	 *
	 * The AdjustWindowRectEx function does not take the WS_VSCROLL or
	 * WS_HSCROLL styles into account. To account for the scroll bars, call the
	 * GetSystemMetrics function with SM_CXVSCROLL or SM_CYHSCROLL.
	 *
	 * @param lpRect
	 *            A pointer to a RECT structure that contains the coordinates of
	 ...
	 *            Indicates whether the window has a menu.
	 * @param dwExStyle
	 *            The extended window style of the window whose required size is
	 *            to be calculated.
	 * @return The number of characters copied to the specified buffer indicates
	 *         success. Zero indicates failure. To get extended error
	 *         information, call GetLastError.
	 ...
	 *
	 * When this function is called, the caller must specify whether or not
	 ...
	 * forces the application to close. During this period, the shutdown may not
	 * be aborted except by the console user, and another shutdown may not be
	 * initiated.
	 *
	 * Calling this function with the value of the uFlags parameter set to
	 * EWX_FORCE avoids this situation. Remember that doing this may result in
	 * loss of data.
	 *
	 * To set a shutdown priority for an application relative to other
	 * applications in the system, use the SetProcessShutdownParameters
	 * function.
	 *
	 * During a shutdown or log-off operation, running applications are allowed
	 * a specific amount of time to respond to the shutdown request. If this
	 * time expires before all applications have stopped, the system displays a
	 * user interface that allows the user to forcibly shut down the system or
	 * to cancel the shutdown request. If the EWX_FORCE value is specified, the
	 * system forces running applications to stop when the time expires.
	 *
	 * If the EWX_FORCEIFHUNG value is specified, the system forces hung
	 ...
	 * application cannot assume that the console notification messages have
	 * been handled when a call to ExitWindowsEx returns.
	 *
	 * To shut down or restart the system, the calling process must use the
	 * {@link com.sun.jna.platform.win32.Advapi32#AdjustTokenPrivileges}
	 * function to enable the SE_SHUTDOWN_NAME privilege. For more information,
	 * see Running with Special Privileges.
	 *
	 * @param uFlags
	 *            The shutdown type. This parameter must include one of
	 *            EWX_HYBRID_SHUTDOWN, EWX_LOGOFF, EWX_POWEROFF, EWX_REBOOT,
	 *            EWX_RESTARTAPPS, or EWX_SHUTDOWN. This parameter can
	 *            optionally include one of EWX_FORCE or EWX_FORCEIFHUNG.
	 * @param dReason
	 *            The reason for initiating the shutdown. This parameter must be
	 *            one of the system shutdown reason codes. If this parameter is
	 *            zero, the SHTDN_REASON_FLAG_PLANNED reason code will not be
	 *            set and therefore the default action is an undefined shutdown
	 *            that is logged as "No title for this reason could be found".
	 *            By default, it is also an unplanned shutdown. Depending on how
	 *            the system is configured, an unplanned shutdown triggers the
	 *            creation of a file that contains the system state information,
	 *            which can delay shutdown. Therefore, do not use zero for this
	 *            parameter.
	 * @return If the function succeeds, the return value is nonzero. Because
	 *         the function executes asynchronously, a nonzero return value
	 *         indicates that the shutdown has been initiated. It does not
	 *         indicate whether the shutdown will succeed. It is possible that
	 *         the system, the user, or another application will abort the
	 *         shutdown. If the function fails, the return value is zero. To get
	 *         extended error information, call GetLastError.
	 */
	 ...
	 * unauthorized use. The LockWorkStation function is callable only by
	 * processes running on the interactive desktop. In addition, the user must
	 * be logged on, and the workstation cannot already be locked.
	 *
	 * @return If the function succeeds, the return value is nonzero. Because
	 *         the function executes asynchronously, a nonzero return value
	 *         indicates that the operation has been initiated. It does not
	 *         indicate whether the workstation has been successfully locked. If
	 ...
	 *            An array of {@link WinUser.RAWINPUTDEVICELIST} structures for
	 *            the devices attached to the system. If (@code null}, the
	 *            number of devices is returned in <tt>puiNumDevices</tt>
	 * @param puiNumDevices
	 *            If <tt>pRawInputDeviceList</tt> is {@code null}, the function
	 *            populates this variable with the number of devices attached to
	 *            the system; otherwise, this variable specifies the number of
	 *            {@link WinUser.RAWINPUTDEVICELIST} structures that can be
	 *            contained in the buffer to which <tt>pRawInputDeviceList</tt>
	 *            points. If this value is less than the number of devices
	 *            attached to the system, the function returns the actual number
	 *            of devices in this variable and fails with
	 *            ERROR_INSUFFICIENT_BUFFER.
	 * @param cbSize
	 *            The size of a {@link WinUser.RAWINPUTDEVICELIST} structure, in
	 *            bytes.
	 * @return If the function is successful, the return value is the number of
	 *         devices stored in the buffer pointed to by
	 *         <tt>pRawInputDeviceList</tt>. On any other error, the function
	 *         returns -1 and {@code GetLastError} returns the error indication.
	 * @see WinUser.RAWINPUTDEVICELIST#sizeof()
	 * @see <A HREF=
	 *      "https://msdn.microsoft.com/en-us/library/windows/desktop/ms645598(v=vs.85).aspx">GetRawInputDeviceList</A>
	 */
	 ...
	 */
	HANDLE SetWinEventHook(int eventMin, int eventMax, HMODULE hmodWinEventProc, WinEventProc winEventProc, int processID,
			int threadID, int flags);
	 ...
	 *         error information, call GetLastError.
	 * @see <a href=
	 *      "https://msdn.microsoft.com/en-us/library/ms648058(S.85).aspx">MSDN</a>
	 */
	 ...
	 *            specify one of the following values.<br>
	 *            GCW_ATOM: -32<br>
	 ...
	 * 
	 * @return Type: HWND The return value is the handle to the active window
	 ...
	 * and return immediately, use the PostMessage or PostThreadMessage
	 * function.
	 * 
	 * @param hWnd
	 *            A handle to the window whose window procedure will receive the
	 ...
	 *            child windows. Message sending is subject to UIPI. The thread
	 *            of a process can send messages only to message queues of
	 *            threads in processes of lesser or equal integrity level.
	 * 
	 * @param msg
	 *            The message to be sent. For lists of the system-provided
	 *            messages, see System-Defined Messages.
	 * @param wParam
	 *            Additional message-specific information.
	 * @param lParam
	 *            Additional message-specific information.
	 ...
	 * 
	 *         Two classic usage : - with a WM_USER+x msg value : wParam and
	 *         lParam are numeric values - with a WM_COPYDATA msg value : wParam
	 *         is the length of the structure and lParam a pointer to a
	 *         COPYDATASTRUCT
	 */
