...
public interface WinDef {
	...
	public static class HICON extends HANDLE {
		...
		/**
		 * Instantiates a new hicon from a handle - this is required since in
		 * Win32 API HANDLE and HICON are the same, whereas in Java they are
		 * not, so in order to &quot;cast&quot; the handle we need this
		 * constructor
		 *
		 * @param handle
		 *            The {@link HANDLE} to cast
		 */
	...
	/**
	 * LPVOID is simply a Windows API typedef for void* - to pointer to any type
	 * so to speak.
	 */
	...
	public static class BOOL extends IntegerType implements Comparable<BOOL> {
		...
		/**
		 * Compares 2 BOOL values - - <B>Note:</B> a {@code null} value is
		 * considered <U>greater</U> than any non-{@code null} one (i.e.,
		...
		/**
		 * Compares a BOOL value with a {@code long} one. <B>Note:</B> if the
		 * BOOL value is {@code null} then it is consider <U>greater</U> than
		 * any {@code long} value.
		 *
		 * @param v1
		 *            The {@link BOOL} value
