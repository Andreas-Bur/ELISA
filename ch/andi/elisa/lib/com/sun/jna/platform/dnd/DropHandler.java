 ...
 * </pre>
 * <ul>
 * <li>Accept drops where the action is the default (i.e. no modifiers) but the
 * intersection of source and target actions is <i>not</i> the default. Doing so
 * allows the source to adjust the cursor appropriately.
 * <li>Refuse drops where the user modifiers request an action that is not
 * supported (this works for all cases except when the drag source is not a
 * {@link DragHandler} and the user explicitly requests a MOVE operation; this
 * is indistinguishable from a drag with no modifiers unless we have access to
 * the key modifiers, which {@link DragHandler} provides).
 ...
 * {@link #paintDropTarget(DropTargetEvent, int, Point)} or by providing a
 * {@link DropTargetPainter}.
 * </ul>
 * 
 ...
 * actions based on available flavors. For instance, a text field for file paths
 * might support {@link DnDConstants#ACTION_COPY_OR_MOVE} on a plain string, but
 * {@link DnDConstants#ACTION_LINK} might be the only action supported on a
 * file.
 ...
 * </ul>
 * 
 * Override {@link #drop(DropTargetDropEvent, int)} to handle the drop. You
 * should invoke {@link DropTargetDropEvent#dropComplete} as soon as the
 * {@link Transferable} data is obtained, to avoid making the DnD operation look
 * suspended.
 * 
 * @see DragHandler
 * @author twall
...
public abstract class DropHandler implements DropTargetListener {
	...
	/**
	 * Create a handler that allows the given set of actions. If using this
	 * constructor, you will need to override {@link #isSupported} to indicate
	 * which data flavors are allowed.
	 * 
	 * @param c
	 *            target component
	...
	/**
	 * Set whether this handler (and thus its drop target) will accept any
	 * drops.
	 * 
	 * @param active
	 ...
	 * Override this method if the acceptable drop actions depend on the
	 * currently available {@link DataFlavor}. The default returns the accepted
	 * actions passed into the constructor.
	 * 
	 * @param dataFlavors
	...
	/**
	 * Calculate the effective action. The default implementation checks whether
	 * any {@link DataFlavor}s are supported, and if so, will change the current
	 * action from {@link DnDConstants#ACTION_NONE} to something in common
	 * between the source and destination. Refuse user-requested actions if they
	 * are not supported (rather than silently accepting a non-user-requested
	 * action, which is the Java's DnD default behavior). The drop action is
	 * forced to {@link DnDConstants#ACTION_NONE} if there is no supported data
	 * flavor.
	 * 
	 * @param e
	...
	/*
	 * Adjust the drop action depending on whether the current action is the
	 * default or a specific user-requested action. The default implementation
	 * will change the current action from {@link DnDConstants#ACTION_NONE} if
	 * there are actions in common between the source and destination. It will
	 * refuse user-requested actions if they are not supported (rather than
	 * silently accepting a non-user-requested action, which is the behavior of
	 * Swing's default drop handlers).<p> You can override this method if you
	 * wish to adjust the action based on the the drag location; if you wish to
	 * deny drops based on location, override {@link #canDrop} instead. If you
	 * wish to adjust the action based on the available data flavors, override
	...
	/**
	 * Returns whether there are key modifiers active , or false if they can't
	 * be determined. We use the DragHandler hint, if available, or fall back to
	 * whether the drop action is other than the default (move).
	 * 
	 * @param dropAction
	 *            requested action.
	...
	private void describe(String type, DropTargetEvent e) {
		if (false) {
			...
			if (e instanceof DropTargetDragEvent) {
				...
				DropTargetDragEvent ev = (DropTargetDragEvent) e;
				msg += ": src=" + DragHandler.actionString(ev.getSourceActions()) + " tgt="
						+ DragHandler.actionString(dt.getDefaultActions()) + " act="
						+ DragHandler.actionString(ev.getDropAction());
			} else if (e instanceof DropTargetDropEvent) {
				DropTargetContext dtc = e.getDropTargetContext();
				DropTarget dt = dtc.getDropTarget();
				DropTargetDropEvent ev = (DropTargetDropEvent) e;
	...
	/**
	 * Accept or reject the drag represented by the given event. Returns the
	 * action determined by {@link #getDropAction(DropTargetEvent)}.
	 * 
	 * @param e
	 *            event
	...
	/**
	 * Return whether any of the flavors in the given list are accepted. The
	 * list is compared against the accepted list provided in the constructor.
	 * 
	 * @param flavors
	 *            list of transfer flavors to check
	 ...
	 * {@link DragHandler#NONE}. Otherwise the decoration should be removed or
	 * hidden.
	 * <p>
	 ...
	 * {@link #getDropAction(DropTargetEvent)}. You may override this method to
	 * refuse drops on certain areas within the drop target component. The
	...
	/**
	 * Handle an incoming drop with the given action. The action passed in might
	 * be different from {@link DropTargetDropEvent#getDropAction}, for
	 * instance, if there are no modifiers and the default action is not
	 ...
	 * allows the drag source to reset the cursor and any drag images which may
	 * be in effect.
	 * 
	 * @param e
	 *            event
	 ...
	 *             dropped item has no supported flavors
	 * @throws IOException
	 *             data access failure
	 */
