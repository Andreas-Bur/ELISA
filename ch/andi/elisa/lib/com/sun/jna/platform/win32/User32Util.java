...
public final class User32Util {
	...
	public static final HWND createWindowEx(final int exStyle, final String className, final String windowName, final int style,
			final int x, final int y, final int width, final int height, final HWND parent, final HMENU menu,
			final HINSTANCE instance, final LPVOID param) {
		final HWND hWnd = User32.INSTANCE.CreateWindowEx(exStyle, className, windowName, style, x, y, width, height, parent, menu,
				instance, param);
		if (hWnd == null)
			throw new Win32Exception(Kernel32.INSTANCE.GetLastError());
		return hWnd;
	 ...
	 * 
	 * This implies, that the Callables should block the loop as short as
	 * possible.
	 */
	public static class MessageLoopThread extends Thread {
		...
		/**
		 * The method is called from the thread, that run the message
		 * dispatcher, when the call to
		 * {@link com.sun.jna.platform.win32.User32#GetMessage} fails (returns
		 * {@code -1}).
		 * 
		 * <p>
		 * If the method returns {@code true}, the MainLoop is exitted, if it
		 * returns {@code false} the mainloop is resumed.
		 * </p>
		 * 
		 * <p>
		 * Default behavior: The error code is logged to the
		...
		protected boolean getMessageFailed() {
			...
			Logger.getLogger("com.sun.jna.platform.win32.User32Util.MessageLoopThread").log(Level.WARNING,
					"Message loop was interrupted by an error. [lastError: {0}]", lastError);
			return true;
		}
	}
}
