...
public interface Advapi32 extends StdCallLibrary {
	 ...
	 */
	boolean LookupAccountSid(String lpSystemName, PSID Sid, char[] lpName, IntByReference cchName, char[] ReferencedDomainName,
	 ...
	 * @return {@code true} if the function was successful - call
	 *         {@code GetLastError()} to check failure reason
	 * @see <A HREF=
	 *      "http://msdn.microsoft.com/en-us/library/aa376399(VS.85).aspx">ConvertSidToStringSid</A>
	 */
	 ...
	 * @param Sid
	 *            Receives a pointer to the converted SID. To free the returned
	 *            buffer, call the LocalFree function.
	 * @return {@code true} if the function was successful - call
	 *         {@code GetLastError()} to check failure reason
	 * @see <A HREF=
	 *      "http://msdn.microsoft.com/en-us/library/aa376402(VS.85).aspx">ConvertStringSidToSid</A>
	 */
	...
	/**
	 * he EqualSid function tests two security identifier (SID) values for
	 * equality. Two SIDs must match exactly to be considered equal.
	 * 
	 * @param pSid1
	 *            A pointer to the first SID structure to compare. This
	 *            structure is assumed to be valid.
	 * @param pSid2
	 *            A pointer to the second SID structure to compare. This
	 *            structure is assumed to be valid.
	 * @return If the SID structures are equal, the return value is nonzero. If
	 *         the SID structures are not equal, the return value is zero. To
	 *         get extended error information, call GetLastError. If either SID
	...
	/**
	 * The InitializeSecurityDescriptor function initializes a new security
	 * descriptor.
	 * 
	 ...
	 *            initializes.
	 * @param dwRevision
	 *            The revision level to assign to the security descriptor. This
	 *            parameter must be SECURITY_DESCRIPTOR_REVISION.
	 * @return If the function succeeds, the return value is nonzero. If the
	 *         function fails, the return value is zero. For extended error
	 *         information, call GetLastError.
	 */
	...
	/**
	 * The GetSecurityDescriptorControl function retrieves a security descriptor
	 * control and revision information.
	 * 
	 * @param pSecurityDescriptor
	 *            A pointer to a SECURITY_DESCRIPTOR structure whose control and
	 *            revision information the function retrieves.
	 * @param pControl
	...
	/**
	 * The SetSecurityDescriptorControl function sets the control bits of a
	 * security descriptor. The function can set only the control bits that
	 * relate to automatic inheritance of ACEs. To set the other control bits of
	 * a security descriptor, use the functions, such as
	 * SetSecurityDescriptorDacl, for modifying the components of a security
	 * descriptor.
	 * 
	 * @param pSecurityDescriptor
	 *            A pointer to a SECURITY_DESCRIPTOR structure whose control and
	 ...
	 *            A SECURITY_DESCRIPTOR_CONTROL mask that indicates the control
	 *            bits to set.
	 * @param ControlBitsToSet
	 *            SECURITY_DESCRIPTOR_CONTROL mask that indicates the new values
	 *            for the control bits specified by the ControlBitsOfInterest
	 *            mask.
	 * @return If the function succeeds, the return value is nonzero. If the
	 *         function fails, the return value is zero. For extended error
	 *         information, call GetLastError.
	 ...
	 * 
	 * @param pSecurityDescriptor
	 *            A pointer to a SECURITY_DESCRIPTOR structure whose owner
	 *            information the function retrieves.
	 * @param pOwner
	 *            A pointer to a pointer to a security identifier (SID) that
	 *            identifies the owner when the function returns. If the
	 *            security descriptor does not contain an owner, the function
	 *            sets the pointer pointed to by pOwner to NULL and ignores the
	 *            remaining output parameter, lpbOwnerDefaulted. If the security
	 *            descriptor contains an owner, the function sets the pointer
	 *            pointed to by pOwner to the address of the security
	 *            descriptor's owner SID and provides a valid value for the
	 *            variable pointed to by lpbOwnerDefaulted.
	 * @param lpbOwnerDefaulted
	 *            A pointer to a flag that is set to the value of the
	 *            SE_OWNER_DEFAULTED flag in the SECURITY_DESCRIPTOR_CONTROL
	 *            structure when the function returns. If the value stored in
	 *            the variable pointed to by the pOwner parameter is NULL, no
	 *            value is set.
	 * @return If the function succeeds, the return value is nonzero. If the
	 *         function fails, the return value is zero. For extended error
	 *         information, call GetLastError.
	...
	/**
	 * The SetSecurityDescriptorOwner function sets the owner information of an
	 * absolute-format security descriptor. It replaces any owner information
	 * already present in the security descriptor.
	 * 
	 * @param pSecurityDescriptor
	 *            A pointer to the SECURITY_DESCRIPTOR structure whose owner is
	 *            set by this function. The function replaces any existing owner
	 *            with the new owner.
	 * @param pOwner
	 *            A pointer to a SID structure for the security descriptor's new
	 *            primary owner. The SID structure is referenced by, not copied
	 *            into, the security descriptor. If this parameter is NULL, the
	 *            function clears the security descriptor's owner information.
	 ...
	 * 
	 * @param pSecurityDescriptor
	 *            A pointer to a SECURITY_DESCRIPTOR structure whose primary
	 *            group information the function retrieves.
	 * @param pGroup
	 *            A pointer to a pointer to a security identifier (SID) that
	 *            identifies the primary group when the function returns. If the
	 *            security descriptor does not contain a primary group, the
	 *            function sets the pointer pointed to by pGroup to NULL and
	 *            ignores the remaining output parameter, lpbGroupDefaulted. If
	 *            the security descriptor contains a primary group, the function
	 *            sets the pointer pointed to by pGroup to the address of the
	 *            security descriptor's group SID and provides a valid value for
	 *            the variable pointed to by lpbGroupDefaulted.
	 * @param lpbGroupDefaulted
	 *            A pointer to a flag that is set to the value of the
	 *            SE_GROUP_DEFAULTED flag in the SECURITY_DESCRIPTOR_CONTROL
	 *            structure when the function returns. If the value stored in
	 *            the variable pointed to by the pGroup parameter is NULL, no
	 *            value is set.
	 * @return If the function succeeds, the return value is nonzero. If the
	 *         function fails, the return value is zero. For extended error
	 *         information, call GetLastError.
	...
	/**
	 * The SetSecurityDescriptorGroup function sets the primary group
	 * information of an absolute-format security descriptor, replacing any
	 * primary group information already present in the security descriptor.
	 * 
	 * @param pSecurityDescriptor
	 *            A pointer to the SECURITY_DESCRIPTOR structure whose primary
	 *            group is set by this function. The function replaces any
	 *            existing primary group with the new primary group.
	 * @param pGroup
	 *            A pointer to a SID structure for the security descriptor's new
	 ...
	 *            Indicates whether the primary group information was derived
	 *            from a default mechanism. If this value is TRUE, it is default
	 *            information, and the function stores this value as the
	 *            SE_GROUP_DEFAULTED flag in the SECURITY_DESCRIPTOR_CONTROL
	 *            structure. If this parameter is zero, the SE_GROUP_DEFAULTED
	 *            flag is cleared.
	 * @return If the function succeeds, the return value is nonzero. If the
	 *         function fails, the return value is zero. For extended error
	 *         information, call GetLastError.
	 ...
	 * discretionary access control list (DACL) in a specified security
	 * descriptor.
	 * 
	 * @param pSecurityDescriptor
	 *            A pointer to the SECURITY_DESCRIPTOR structure that contains
	 *            the DACL. The function retrieves a pointer to it.
	 * @param bDaclPresent
	 *            A pointer to a value that indicates the presence of a DACL in
	 *            the specified security descriptor. If lpbDaclPresent is TRUE,
	 *            the security descriptor contains a DACL, and the remaining
	 *            output parameters in this function receive valid values. If
	 *            lpbDaclPresent is FALSE, the security descriptor does not
	 *            contain a DACL, and the remaining output parameters do not
	 *            receive valid values. A value of TRUE for lpbDaclPresent does
	 *            not mean that pDacl is not NULL. That is, lpbDaclPresent can
	 *            be TRUE while pDacl is NULL, meaning that a NULL DACL is in
	 *            effect. A NULL DACL implicitly allows all access to an object
	 *            and is not the same as an empty DACL. An empty DACL permits no
	 *            access to an object. For information about creating a proper
	 *            DACL, see Creating a DACL.
	 * @param pDacl
	 *            A pointer to a pointer to an access control list (ACL). If a
	 *            DACL exists, the function sets the pointer pointed to by pDacl
	 *            to the address of the security descriptor's DACL. If a DACL
	 *            does not exist, no value is stored. If the function stores a
	 *            NULL value in the pointer pointed to by pDacl, the security
	 *            descriptor has a NULL DACL. A NULL DACL implicitly allows all
	 *            access to an object. If an application expects a non-NULL DACL
	 *            but encounters a NULL DACL, the application should fail
	 *            securely and not allow access.
	 * @param bDaclDefaulted
	 *            A pointer to a flag set to the value of the SE_DACL_DEFAULTED
	 *            flag in the SECURITY_DESCRIPTOR_CONTROL structure if a DACL
	 *            exists for the security descriptor. If this flag is TRUE, the
	 *            DACL was retrieved by a default mechanism; if FALSE, the DACL
	 *            was explicitly specified by a user.
	 * @return If the function succeeds, the return value is nonzero. If the
	 *         function fails, the return value is zero. For extended error
	 *         information, call GetLastError.
	 */
	...
	/**
	 * The SetSecurityDescriptorDacl function sets information in a
	 * discretionary access control list (DACL). If a DACL is already present in
	 * the security descriptor, the DACL is replaced.
	 * 
	 * @param pSecurityDescriptor
	 *            A pointer to the SECURITY_DESCRIPTOR structure to which the
	 *            function adds the DACL. This security descriptor must be in
	 *            absolute format, meaning that its members must be pointers to
	 ...
	 *            parameters. If this parameter is FALSE, the function clears
	 *            the SE_DACL_PRESENT flag, and pDacl and bDaclDefaulted are
	 *            ignored.
	 * @param pDacl
	 *            A pointer to an ACL structure that specifies the DACL for the
	 *            security descriptor. If this parameter is NULL, a NULL DACL is
	 *            assigned to the security descriptor, which allows all access
	 *            to the object. The DACL is referenced by, not copied into, the
	 *            security descriptor.
	 * @param bDaclDefaulted
	 *            A flag that indicates the source of the DACL. If this flag is
	 *            TRUE, the DACL has been retrieved by some default mechanism.
	 *            If FALSE, the DACL has been explicitly specified by a user.
	 *            The function stores this value in the SE_DACL_DEFAULTED flag
	 *            of the SECURITY_DESCRIPTOR_CONTROL structure. If this
	 *            parameter is not specified, the SE_DACL_DEFAULTED flag is
	 *            cleared.
	 * @return If the function succeeds, the return value is nonzero. If the
	 *         function fails, the return value is zero. For extended error
	 *         information, call GetLastError.
	 ...
	 *            parameter. This value must be large enough to contain the ACL
	 *            header and all of the access control entries (ACEs) to be
	 *            stored in the ACL. In addition, this value must be
	 *            DWORD-aligned. For more information about calculating the size
	 *            of an ACL, see Remarks.
	 * @param dwAclRevision
	...
	/**
	 * The AddAce function adds one or more access control entries (ACEs) to a
	 ...
	 *            value can be ACL_REVISION or ACL_REVISION_DS. Use
	 *            ACL_REVISION_DS if the ACL contains object-specific ACEs. This
	 *            value must be compatible with the AceType field of all ACEs in
	 *            pAceList. Otherwise, the function will fail and set the last
	 *            error to ERROR_INVALID_PARAMETER.
	 ...
	 *            beginning of the list. A value of MAXDWORD appends the ACEs to
	 *            the end of the list.
	 * @param pAceList
	 ...
	 * 
	 * @param pAcl
	 *            A pointer to an ACL. This function adds an access-allowed ACE
	 *            to the end of this ACL. The ACE is in the form of an
	 *            ACCESS_ALLOWED_ACE structure.
	 * @param dwAceRevision
	 *            Specifies the revision level of the ACL being modified. This
	 *            value can be ACL_REVISION or ACL_REVISION_DS. Use
	 *            ACL_REVISION_DS if the ACL contains object-specific ACEs.
	 * @param AccessMask
	 *            Specifies the mask of access rights to be granted to the
	 *            specified SID.
	 * @param pSid
	 *            A pointer to the SID representing a user, group, or logon
	 ...
	 *            ACL_REVISION_DS if the ACL contains object-specific ACEs.
	 * @param AceFlags
	 *            A set of bit flags that control ACE inheritance. The function
	 *            sets these flags in the AceFlags member of the ACE_HEADER
	 *            structure of the new ACE. This parameter can be a combination
	 *            of the following values: CONTAINER_INHERIT_ACE,
	 *            INHERIT_ONLY_ACE, INHERITED_ACE, NO_PROPAGATE_INHERIT_ACE, and
	 *            OBJECT_INHERIT_ACE
	 * @param AccessMask
	 *            Specifies the mask of access rights to be granted to the
	 *            specified SID.
	...
	/**
	 * The GetAce function obtains a pointer to an access control entry (ACE) in
	 ...
	 *            corresponds to the first ACE in the ACL, a value of one to the
	 *            second ACE, and so on.
	 * @param pAce
	 *            A pointer to a pointer that the function sets to the address
	 *            of the ACE.
	 * @return If the function succeeds, the return value is nonzero. If the
	 *         function fails, the return value is zero. For extended error
	 *         information, call GetLastError.
	 ...
	 * 
	 * @param ThreadHandle
	 *            [in, optional] A pointer to a handle to the thread to which
	 *            the function assigns the impersonation token. If ThreadHandle
	 *            is NULL, the function assigns the impersonation token to the
	 *            calling thread.
	 * @param TokenHandle
	 *            [in, optional] A handle to the impersonation token to assign
	 *            to the thread. This handle must have been opened with
	 *            TOKEN_IMPERSONATE access rights. For more information, see
	 *            Access Rights for Access-Token Objects. If Token is NULL, the
	 *            function causes the thread to stop using an impersonation
	 *            token.
	 * @return If the function succeeds, the return value is nonzero. If the
	 *         function fails, the return value is zero. To get extended error
	 *         information, call GetLastError.
	 ...
	 *         information, call GetLastError.
	 */
	boolean DuplicateTokenEx(HANDLE hExistingToken, int dwDesiredAccess, WinBase.SECURITY_ATTRIBUTES lpTokenAttributes,
	 ...
	 *         defined in Winerror.h.
	 */
	int RegEnumKeyEx(HKEY hKey, int dwIndex, char[] lpName, IntByReference lpcName, IntByReference reserved, char[] lpClass,
	 ...
	 *         defined in Winerror.h.
	 */
	int RegQueryInfoKey(HKEY hKey, char[] lpClass, IntByReference lpcClass, IntByReference lpReserved, IntByReference lpcSubKeys,
			IntByReference lpcMaxSubKeyLen, IntByReference lpcMaxClassLen, IntByReference lpcValues,
			IntByReference lpcMaxValueNameLen, IntByReference lpcMaxValueLen, IntByReference lpcbSecurityDescriptor,
	 ...
	 *         information, call GetLastError.
	 */
	boolean ReadEventLog(HANDLE hEventLog, int dwReadFlags, int dwRecordOffset, Pointer lpBuffer, int nNumberOfBytesToRead,
	 ...
	 *            SERVICE_CHANGE_CONFIG access right. For more information, see
	 *            <a href=
	 *            "http://msdn.microsoft.com/en-us/library/ms685981.aspx"
	 *            >Service Security and Access Rights</a>. If the service
	 *            controller handles the SC_ACTION_RESTART action, hService must
	 ...
	 *            the dwInfoLevel parameter. If this value is NULL, the
	 ...
	 *            A handle to the service. This handle is returned by the
	 *            OpenService or CreateService function and must have the
	 *            SERVICE_QUERY_CONFIG access right. For more information, see
	 *            Service Security and Access Rights.
	 * @param dwInfoLevel
	 *            The configuration information to be queried.
	 * @param lpBuffer
	 *            A pointer to the buffer that receives the service
	 *            configuration information. The format of this data depends on
	 *            the value of the dwInfoLevel parameter. The maximum size of
	 *            this array is 8K bytes. To determine the required size,
	 *            specify NULL for this parameter and 0 for the cbBufSize
	 *            parameter. The function fails and GetLastError returns
	 *            ERROR_INSUFFICIENT_BUFFER. The pcbBytesNeeded parameter
	 *            receives the needed size.
	 * @param cbBufSize
	 *            The size of the structure pointed to by the lpBuffer
	 *            parameter, in bytes.
	 * @param pcbBytesNeeded
	 *            A pointer to a variable that receives the number of bytes
	 *            required to store the configuration information, if the
	 ...
	 *            right. For more information, see <a href=
	 *            "http://msdn.microsoft.com/en-us/library/ms685981.aspx"
	...
	boolean CreateProcessAsUser(HANDLE hToken, String lpApplicationName, String lpCommandLine,
			...
			int dwCreationFlags, String lpEnvironment, String lpCurrentDirectory, WinBase.STARTUPINFO lpStartupInfo,
	 ...
	 * object. This function is obsolete. Use the SetNamedSecurityInfo function
	 * instead.
	 *
	 * @param lpFileName
	 *            A pointer to a null-terminated string that specifies the file
	 *            or directory for which security is set. Note that security
	 *            applied to a directory is not inherited by its children.
	 * @param SecurityInformation
	 *            Specifies a SECURITY_INFORMATION structure that identifies the
	 *            contents of the security descriptor pointed to by the
	 *            pSecurityDescriptor parameter.
	 * @param pSecurityDescriptor
	...
	/**
	 * The GetSecurityInfo function retrieves a copy of the security descriptor
	 * for an object specified by a handle.
	 *
	 * @param handle
	 *            [in] A handle to the object from which to retrieve security
	 *            information.
	 * @param ObjectType
	 *            [in] SE_OBJECT_TYPE enumeration value that indicates the type
	 *            of object.
	 * @param SecurityInfo
	 *            [in] A set of bit flags that indicate the type of security
	 *            information to retrieve. See WinNT *_SECURITY_INFORMATION
	 * @param ppsidOwner
	 *            [out, optional] A pointer to a variable that receives a
	 *            pointer to the owner SID in the security descriptor returned
	 *            in ppSecurityDescriptor. The returned pointer is valid only if
	 *            you set the OWNER_SECURITY_INFORMATION flag. This parameter
	 *            can be NULL if you do not need the owner SID.
	 * @param ppsidGroup
	 *            [in, optional] A pointer to a variable that receives a pointer
	 *            to the primary group SID in the returned security descriptor.
	 *            The returned pointer is valid only if you set the
	 *            GROUP_SECURITY_INFORMATION flag. This parameter can be NULL if
	 *            you do not need the group SID.
	 * @param ppDacl
	 *            [in, optional] A pointer to a variable that receives a pointer
	 *            to the DACL in the returned security descriptor. The returned
	 *            pointer is valid only if you set the DACL_SECURITY_INFORMATION
	 *            flag. This parameter can be NULL if you do not need the DACL.
	 * @param ppSacl
	 *            [in, optional] A pointer to a variable that receives a pointer
	 *            to the SACL in the returned security descriptor. The returned
	 ...
	 *            security descriptor of the object. When you have finished
	 *            using the pointer, free the returned buffer by calling the
	 *            LocalFree function. This parameter is required if any one of
	 *            the ppsidOwner, ppsidGroup, ppDacl, or ppSacl parameters is
	 *            not NULL.
	 ...
	 *         </p>
	 *         <p>
	 * 		1. If the ppsidOwner, ppsidGroup, ppDacl, and ppSacl parameters
	 *         are non-NULL, and the SecurityInfo parameter specifies that they
	 *         be retrieved from the object, those parameters will point to the
	 *         corresponding parameters in the security descriptor returned in
	 *         ppSecurityDescriptor.
	 *         </p>
	 *         <p>
	 * 		2. To read the owner, group, or DACL from the object's security
	 *         descriptor, the calling process must have been granted
	 *         READ_CONTROL access when the handle was opened. To get
	 *         READ_CONTROL access, the caller must be the owner of the object
	 *         or the object's DACL must grant the access.
	 *         </p>
	 *         <p>
	 * 		3. To read the SACL from the security descriptor, the calling
	 *         process must have been granted ACCESS_SYSTEM_SECURITY access when
	 *         the handle was opened. The proper way to get this access is to
	 *         enable the SE_SECURITY_NAME privilege in the caller's current
	 *         token, open the handle for ACCESS_SYSTEM_SECURITY access, and
	 *         then disable the privilege.
	 *         </p>
	 *         <p>
	 * 		4. If the supplied handle was opened with an ACCESS_MASK value of
	 *         MAXIMUM_ALLOWED, then the SetSecurityInfo function will not
	 *         propagate ACEs to children.
	 *         </p>
	 */
	...
	/**
	 * The SetSecurityInfo function sets specified security information in the
	 * security descriptor of a specified object. The caller identifies the
	 * object by a handle.
	 *
	 * @param handle
	 *            [in] A handle to the object for which to set security
	 *            information.
	 * @param ObjectType
	 *            [in] A value of the SE_OBJECT_TYPE enumeration that indicates
	 *            the type of object named by the pObjectName parameter.
	 * @param SecurityInfo
	 *            [in] A set of bit flags that indicate the type of security
	 *            information to set. See WinNT *_SECURITY_INFORMATION
	 * @param ppsidOwner
	 *            [in, optional] A pointer to a SID structure that identifies
	 *            the owner of the object. If the caller does not have the
	 *            SeRestorePrivilege constant (see Privilege Constants), this
	 *            SID must be contained in the caller's token, and must have the
	 *            SE_GROUP_OWNER permission enabled. The SecurityInfo parameter
	 *            must include the OWNER_SECURITY_INFORMATION flag. To set the
	 *            owner, the caller must have WRITE_OWNER access to the object
	 *            or have the SE_TAKE_OWNERSHIP_NAME privilege enabled. If you
	 *            are not setting the owner SID, this parameter can be NULL.
	 * @param ppsidGroup
	 *            [in, optional] A pointer to a SID that identifies the primary
	 *            group of the object. The SecurityInfo parameter must include
	 *            the GROUP_SECURITY_INFORMATION flag. If you are not setting
	 *            the primary group SID, this parameter can be NULL.
	 * @param ppDacl
	 *            [in, optional] A pointer to the new DACL for the object. The
	 *            SecurityInfo parameter must include the
	 *            DACL_SECURITY_INFORMATION flag. The caller must have WRITE_DAC
	 *            access to the object or be the owner of the object. If you are
	 *            not setting the DACL, this parameter can be NULL.
	 * @param ppSacl
	 *            [in, optional] A pointer to the new SACL for the object. The
	 *            SecurityInfo parameter must include any of the following
	 *            flags: SACL_SECURITY_INFORMATION, LABEL_SECURITY_INFORMATION,
	 *            ATTRIBUTE_SECURITY_INFORMATION, SCOPE_SECURITY_INFORMATION, or
	 *            BACKUP_SECURITY_INFORMATION. If setting
	 *            SACL_SECURITY_INFORMATION or SCOPE_SECURITY_INFORMATION, the
	 *            caller must have the SE_SECURITY_NAME privilege enabled. If
	 ...
	 *         </p>
	 *         <p>
	 * 		1. If you are setting the discretionary access control list
	 *         (DACL) or any elements in the system access control list (SACL)
	 *         of an object, the system automatically propagates any inheritable
	 *         access control entries (ACEs) to existing child objects,
	 *         according to the ACE inheritance rules.
	 *         </p>
	 *         <p>
	 * 		2. The SetSecurityInfo function does not reorder access-allowed
	 *         or access-denied ACEs based on the preferred order. When
	 *         propagating inheritable ACEs to existing child objects,
	 *         SetSecurityInfo puts inherited ACEs in order after all of the
	 *         noninherited ACEs in the DACLs of the child objects.
	 *         </p>
	 *         <p>
	 * 		3. If share access to the children of the object is not
	 *         available, this function will not propagate unprotected ACEs to
	 *         the children. For example, if a directory is opened with
	 *         exclusive access, the operating system will not propagate
	 *         unprotected ACEs to the subdirectories or files of that directory
	 *         when the security on the directory is changed.
	 *         </p>
	 *         <p>
	 * 		4. If the supplied handle was opened with an ACCESS_MASK value of
	 *         MAXIMUM_ALLOWED, then the SetSecurityInfo function will not
	 ...
	 *         </p>
	 */
	int SetSecurityInfo(HANDLE handle, int ObjectType, int SecurityInfo, Pointer ppsidOwner, Pointer ppsidGroup, Pointer ppDacl,
			Pointer ppSacl);
	 ...
	 *            which to retrieve security information. For descriptions of
	 *            the string formats for the different object types, see
	 *            SE_OBJECT_TYPE.
	 * @param ObjectType
	 *            Specifies a value from the SE_OBJECT_TYPE enumeration that
	 *            indicates the type of object named by the pObjectName
	 *            parameter.
	 * @param SecurityInfo
	 ...
	 *            [out, optional] A pointer to a variable that receives a
	 *            pointer to the owner SID in the security descriptor returned
	 *            in ppSecurityDescriptor or NULL if the security descriptor has
	 *            no owner SID. The returned pointer is valid only if you set
	 *            the OWNER_SECURITY_INFORMATION flag. Also, this parameter can
	 *            be NULL if you do not need the owner SID.
	 * @param ppsidGroup
	 *            [out, optional] A pointer to a variable that receives a
	 *            pointer to the primary group SID in the returned security
	 *            descriptor or NULL if the security descriptor has no group
	 *            SID. The returned pointer is valid only if you set the
	 *            GROUP_SECURITY_INFORMATION flag. Also, this parameter can be
	 *            NULL if you do not need the group SID.
	 * @param ppDacl
	 *            [out, optional] A pointer to a variable that receives a
	 *            pointer to the DACL in the returned security descriptor or
	 *            NULL if the security descriptor has no DACL. The returned
	 *            pointer is valid only if you set the DACL_SECURITY_INFORMATION
	 *            flag. Also, this parameter can be NULL if you do not need the
	 *            DACL.
	 * @param ppSacl
	 *            [out, optional] A pointer to a variable that receives a
	 *            pointer to the SACL in the returned security descriptor or
	 *            NULL if the security descriptor has no SACL. The returned
	 *            pointer is valid only if you set the SACL_SECURITY_INFORMATION
	 *            flag. Also, this parameter can be NULL if you do not need the
	 *            SACL.
	 * @param ppSecurityDescriptor
	 ...
	 * @return whether the call succeeded. A nonzero return is a failure.
	 *
	 *         NOTES: 1. To read the owner, group, or DACL from the object's
	 *         security descriptor, the object's DACL must grant READ_CONTROL
	 *         access to the caller, or the caller must be the owner of the
	 *         object. 2. To read the system access control list of the object,
	 *         the SE_SECURITY_NAME privilege must be enabled for the calling
	 *         process. For information about the security implications of
	 ...
	 *
	 * @param pObjectName
	 *            [in] A pointer to a string that specifies the name of the
	 *            object for which to set security information. This can be the
	 *            name of a local or remote file or directory on an NTFS file
	 *            system, network share, registry key, semaphore, event, mutex,
	 *            file mapping, or waitable timer. * For descriptions of the
	 *            string formats for the different object types, see
	 *            SE_OBJECT_TYPE.
	 * @param ObjectType
	 *            [in] A value of the SE_OBJECT_TYPE enumeration that indicates
	 *            the type of object named by the pObjectName parameter.
	 ...
	 * @return whether the call succeeded. A nonzero return is a failure.
	 *
	 *         NOTES: 1. The SetNamedSecurityInfo function does not reorder
	 *         access-allowed or access-denied ACEs based on the preferred
	 *         order. When propagating inheritable ACEs to existing child
	 *         objects, SetNamedSecurityInfo puts inherited ACEs in order after
	 *         all of the noninherited ACEs in the DACLs of the child objects.
	 *         2. This function transfers information in plaintext. The
	 *         information transferred by this function is signed unless signing
	 *         has been turned off for the system, but no encryption is
	 *         performed. 3. When you update access rights of a folder indicated
	 *         by an UNC path, for example \\Test\TestFolder, the original
	 *         inherited ACE is removed and the full volume path is not
	 *         included.
	 */
	int SetNamedSecurityInfo(String pObjectName, int ObjectType, int SecurityInfo, Pointer ppsidOwner, Pointer ppsidGroup,
			Pointer ppDacl, Pointer ppSacl);

	/**
	 * The GetSecurityDescriptorLength function returns the length, in bytes, of
	 * a structurally valid security descriptor. The length includes the length
	 * of all associated structures.
	 *
	 * @param ppSecurityDescriptor
	 *            A pointer to the SECURITY_DESCRIPTOR structure whose length
	 *            the function returns. The pointer is assumed to be valid.
	 * @return If the function succeeds, the function returns the length, in
	 *         bytes, of the SECURITY_DESCRIPTOR structure. If the
	 *         SECURITY_DESCRIPTOR structure is not valid, the return value is
	 *         undefined.
	 */
	int GetSecurityDescriptorLength(Pointer ppSecurityDescriptor);

	/**
	 * The IsValidSecurityDescriptor function determines whether the components
	 * of a security descriptor are valid.
	 *
	 * @param ppSecurityDescriptor
	 *            [in] A pointer to a SECURITY_DESCRIPTOR structure that the
	 *            function validates.
	 * @return If the components of the security descriptor are valid, the
	 *         return value is nonzero.
	 */
	boolean IsValidSecurityDescriptor(Pointer ppSecurityDescriptor);

	/**
	 * A pointer to a SECURITY_DESCRIPTOR structure in absolute format. The
	 * function creates a version of this security descriptor in self-relative
	 * format without modifying the original.
	 * 
	 * @param pAbsoluteSD
	 *            A pointer to a SECURITY_DESCRIPTOR structure in absolute
	 *            format. The function creates a version of this security
	 *            descriptor in self-relative format without modifying the
	 *            original.
	 * @param pSelfRelativeSD
	 *            A pointer to a buffer the function fills with a security
	 *            descriptor in self-relative format.
	 * @param lpdwBufferLength
	 *            A pointer to a variable specifying the size of the buffer
	 *            pointed to by the pSelfRelativeSD parameter. If the buffer is
	 *            not large enough for the security descriptor, the function
	 *            fails and sets this variable to the minimum required size.
	 * @return If the function succeeds, the function returns nonzero. If the
	 *         function fails, it returns zero. To get extended error
	 *         information, call GetLastError. Possible return codes include,
	 *         but are not limited to, the following: ERROR_INSUFFICIENT_BUFFER
	 ...
	 * format by using a security descriptor in self-relative format as a
	 * template.
	 * 
	 * @param pSelfRelativeSD
	 *            A pointer to a SECURITY_DESCRIPTOR structure in self-relative
	 *            format. The function creates an absolute-format version of
	 *            this security descriptor without modifying the original
	 *            security descriptor.
	 * @param pAbsoluteSD
	 *            A pointer to a buffer that the function fills with the main
	 *            body of an absolute-format security descriptor. This
	 *            information is formatted as a SECURITY_DESCRIPTOR structure.
	 * @param lpdwAbsoluteSDSize
	 *            A pointer to a variable that specifies the size of the buffer
	 *            pointed to by the pAbsoluteSD parameter. If the buffer is not
	 *            large enough for the security descriptor, the function fails
	 *            and sets this variable to the minimum required size.
	 * @param pDacl
	 *            A pointer to a buffer the function fills with the
	 *            discretionary access control list (DACL) of the
	 *            absolute-format security descriptor. The main body of the
	 *            absolute-format security descriptor references this pointer.
	 * @param lpdwDaclSize
	 *            A pointer to a variable that specifies the size of the buffer
	 *            pointed to by the pDacl parameter. If the buffer is not large
	 *            enough for the access control list (ACL), the function fails
	 *            and sets this variable to the minimum required size.
	 * @param pSacl
	 *            A pointer to a buffer the function fills with the system
	 *            access control list (SACL) of the absolute-format security
	 *            descriptor. The main body of the absolute-format security
	 *            descriptor references this pointer.
	 * @param lpdwSaclSize
	 *            A pointer to a variable that specifies the size of the buffer
	 *            pointed to by the pSacl parameter. If the buffer is not large
	 *            enough for the ACL, the function fails and sets this variable
	 *            to the minimum required size.
	 * @param pOwner
	 *            A pointer to a buffer the function fills with the security
	 *            identifier (SID) of the owner of the absolute-format security
	 *            descriptor. The main body of the absolute-format security
	 *            descriptor references this pointer.
	 * @param lpdwOwnerSize
	 *            A pointer to a variable that specifies the size of the buffer
	 *            pointed to by the pOwner parameter. If the buffer is not large
	 *            enough for the SID, the function fails and sets this variable
	 *            to the minimum required size.
	 * @param pPrimaryGroup
	 *            A pointer to a buffer the function fills with the SID of the
	 *            absolute-format security descriptor's primary group. The main
	 *            body of the absolute-format security descriptor references
	 *            this pointer.
	 * @param lpdwPrimaryGroupSize
	 *            A pointer to a variable that specifies the size of the buffer
	 *            pointed to by the pPrimaryGroup parameter. If the buffer is
	 *            not large enough for the SID, the function fails and sets this
	...
	boolean MakeAbsoluteSD(SECURITY_DESCRIPTOR_RELATIVE pSelfRelativeSD, SECURITY_DESCRIPTOR pAbsoluteSD,
			IntByReference lpdwAbsoluteSDSize, ACL pDacl, IntByReference lpdwDaclSize, ACL pSacl, IntByReference lpdwSaclSize,
			PSID pOwner, IntByReference lpdwOwnerSize, PSID pPrimaryGroup, IntByReference lpdwPrimaryGroupSize);

	/**
	 * The IsValidAcl function validates an access control list (ACL).
	 *
	 * @param pAcl
	 *            [in] A pointer to an ACL structure validated by this function.
	 ...
	 *
	 *         This function checks the revision level of the ACL and verifies
	 *         that the number of access control entries (ACEs) specified in the
	 *         AceCount member of the ACL structure fits the space specified by
	 *         the AclSize member of the ACL structure.If pAcl is NULL, the
	 *         application will fail with an access violation.
	 */
	boolean IsValidAcl(Pointer pAcl);

	/**
	 * Applies the given mapping of generic access rights to the given access
	 * mask.
	 * 
	 * @param AccessMask
	 *            [in, out] A pointer to an access mask.
	 * @param GenericMapping
	 *            [in] A pointer to a GENERIC_MAPPING structure specifying a
	 *            mapping of generic access types to specific and standard
	 *            access types.
	 */
	void MapGenericMask(DWORDByReference AccessMask, GENERIC_MAPPING GenericMapping);

	/**
	 * Check if the if the security descriptor grants access to the given client
	 * token.
	 *
	 * @param pSecurityDescriptor
	 *            [in] A pointer to a SECURITY_DESCRIPTOR structure against
	 *            which access is checked.
	 * @param ClientToken
	 *            [in] A handle to an impersonation token that represents the
	 ...
	 * @param DesiredAccess
	 *            [in] Access mask that specifies the access rights to check.
	 *            This mask must have been mapped by the MapGenericMask function
	 *            to contain no generic access rights.<br>
	 *            If this parameter is MAXIMUM_ALLOWED, the function sets the
	 *            GrantedAccess access mask to indicate the maximum access
	 *            rights the security descriptor allows the client.
	 * @param GenericMapping
	 *            [in] A pointer to the GENERIC_MAPPING structure associated
	 *            with the object for which access is being checked.
	 * @param PrivilegeSet
	 *            [out, optional] A pointer to a PRIVILEGE_SET structure that
	 *            receives the privileges used to perform the access validation.
	 *            If no privileges were used, the function sets the
	 *            PrivilegeCount member to zero.
	 * @param PrivilegeSetLength
	 *            [in, out] Specifies the size, in bytes, of the buffer pointed
	 *            to by the PrivilegeSet parameter.
	 * @param GrantedAccess
	 *            [out] A pointer to an access mask that receives the granted
	 *            access rights. If AccessStatus is set to FALSE, the function
	 *            sets the access mask to zero. If the function fails, it does
	 *            not set the access mask.
	 * @param AccessStatus
	 *            [out] A pointer to a variable that receives the results of the
	 ...
	 *            information.
	 * @return true on success; false on failure (use GetLastError to get
	 *         extended error information)
	 */
	boolean AccessCheck(Pointer pSecurityDescriptor, HANDLE ClientToken, DWORD DesiredAccess, GENERIC_MAPPING GenericMapping,
			PRIVILEGE_SET PrivilegeSet, DWORDByReference PrivilegeSetLength, DWORDByReference GrantedAccess,
			BOOLByReference AccessStatus);
	 ...
	 * @param lpStatus
	 *            A pointer to a variable that receives the encryption status of
	 *            the file.
	 * @return If the function succeeds, the return value is nonzero. If the
	 *         function fails, the return value is zero. To get extended error
	 *         information, call GetLastError.
	 ...
	 * in it. It does not affect encryption of subdirectories below the
	 * indicated directory.
	 *
	 * @param DirPath
	 ...
	 * the file. This is one of a group of Encrypted File System (EFS) functions
	 * that is intended to implement backup and restore functionality, while
	 ...
	 * @param pvContext
	 *            The address of a context block that must be presented in
	 *            subsequent calls to ReadEncryptedFileRaw,
	 *            WriteEncryptedFileRaw, or CloseEncryptedFileRaw. Do not modify
	 *            it.
	 ...
	 *         FORMAT_MESSAGE_FROM_SYSTEM flag to get a generic text description
	 *         of the error.
	 */
	int OpenEncryptedFileRaw(String lpFileName, ULONG ulFlags, PointerByReference pvContext);

	/**
	 * Backs up (export) encrypted files. This is one of a group of Encrypted
	 * File System (EFS) functions that is intended to implement backup and
	 * restore functionality, while maintaining files in their encrypted state.
	 *
	 * @param pfExportCallback
	 *            A pointer to the export callback function. The system calls
	 *            the callback function multiple times, each time passing a
	 *            block of the file's data to the callback function until the
	 *            entire file has been read. For more information, see
	 *            ExportCallback.
	 * @param pvCallbackContext
	 *            A pointer to an application-defined and allocated context
	 *            block. The system passes this pointer to the callback function
	 *            as a parameter so that the callback function can have access
	 *            to application-specific data. This can be a structure and can
	 *            contain any data the application needs, such as the handle to
	 *            the file that will contain the backup copy of the encrypted
	 *            file.
	 * @param pvContext
	 *            A pointer to a system-defined context block. The context block
	 *            is returned by the OpenEncryptedFileRaw function. Do not
	 *            modify it.
	 * @return If the function succeeds, the return value is ERROR_SUCCESS. If
	 *         the function fails, it returns a nonzero error code defined in
	 *         WinError.h. You can use FormatMessage with the
	 *         FORMAT_MESSAGE_FROM_SYSTEM flag to get a generic text description
	 *         of the error.
	 */
	...
	/**
	 * Restores (import) encrypted files. This is one of a group of Encrypted
	 * File System (EFS) functions that is intended to implement backup and
	 * restore functionality, while maintaining files in.
	 *
	 * @param pfImportCallback
	 *            A pointer to the import callback function. The system calls
	 *            the callback function multiple times, each time passing a
	 *            buffer that will be filled by the callback function with a
	 *            portion of backed-up file's data. When the callback function
	 *            signals that the entire file has been processed, it tells the
	 *            system that the restore operation is finished. For more
	 *            information, see ImportCallback.
	 * @param pvCallbackContext
	 *            A pointer to an application-defined and allocated context
	 *            block. The system passes this pointer to the callback function
	 *            as a parameter so that the callback function can have access
	 ...
	 *         GetExitCodeProcess.
	 * @see <a href=
	 *      "http://msdn.microsoft.com/en-us/library/windows/desktop/ms682431%28v=vs.85%29.aspx">MSDN</a>
	 */
	boolean CreateProcessWithLogonW(String lpUsername, String lpDomain, String lpPassword, int dwLogonFlags,
			String lpApplicationName, String lpCommandLine, int dwCreationFlags, Pointer lpEnvironment, String lpCurrentDirectory,
			STARTUPINFO lpStartupInfo, PROCESS_INFORMATION lpProcessInfo);
	 ...
	 *
	 * @param lpServiceTable
	 *            A pointer to an array of SERVICE_TABLE_ENTRY structures
	 *            containing one entry for each service that can execute in the
	 *            calling process. The members of the last entry in the table
	 *            must have NULL values to designate the end of the table.
	 * 
	 * @return true if function succeeds. To get extended error information,
	 *         call GetLastError. Possible error codes:
	 *         <table>
	 *         <tr>
	 *         <th>Return code</th>
	 *         <th>Description</th>
	 *         </tr>
	 *         <tr>
	 *         <td>ERROR_FAILED_SERVICE_CONTROLLER_CONNECT</td>
	 *         <td>This error is returned if the program is being run as a
	 *         console application rather than as a service. If the program will
	 *         be run as a console application for debugging purposes, structure
	 *         it such that service-specific code is not called when this error
	 *         is returned.</td>
	 *         </tr>
	 *         <tr>
	 *         <td>ERROR_INVALID_DATA</td>
	 *         <td>The specified dispatch table contains entries that are not in
	 *         the proper format.</td>
	 *         </tr>
	 *         <tr>
	 *         <td>ERROR_SERVICE_ALREADY_RUNNING</td>
	 *         <td>The process has already called StartServiceCtrlDispatcher.
	 *         Each process can call StartServiceCtrlDispatcher only one
	 *         time.</td>
	 *         </tr>
	 *         </table>
	 */
	 ...
	 *
	 * @param lpServiceName
	 *            The name of the service run by the calling thread. This is the
	 *            service name that the service control program specified in the
	 *            CreateService function when creating the service.
	 *
	 *            <p>
	 * 			If the service type is SERVICE_WIN32_OWN_PROCESS, the function
	 *            does not verify that the specified name is valid, because
	 *            there is only one registered service in the process.
	 *            </p>
	 *
	 * @param lpHandlerProc
	 *            A pointer to the handler function to be registered. For more
	 *            information, see
	 *            {@link com.sun.jna.platform.win32.Winsvc.Handler
	 *            WinSvc.Handler}.
	 *
	 * @return A service status handle, NULL on error. Call GetLastError to get
	 *         extended error condition. Possible error codes:
	 *         <table>
	 *         <tr>
	 *         <th>Return code</th>
	 *         <th>Description</th>
	 ...
	 *         <td>Not enough memory is available to convert an ANSI string
	 *         parameter to Unicode. This error does not occur for Unicode
	 *         string parameters.</td>
	 *         </tr>
	 *         <tr>
	 *         <td>ERROR_SERVICE_NOT_IN_EXE</td>
	 *         <td>The service entry was specified incorrectly when the process
	 *         called the {@link #StartServiceCtrlDispatcher} function.</td>
	 *         </tr>
	 *         </table>
	 */
	 ...
	 *            CreateService function when creating the service.
	 * @param lpHandlerProc
	 *            The handler function to be registered. For more information,
	 *            see HandlerEx.
	 * @param lpContext
	 ...
	 *
	 * @return A service status handle on success, NULL on error. Call
	 *         GetLastError to get extended information. Possible error codes:
	 *         <table>
	 *         <tr>
	 *         <th>Return code</th>
	 *         <th>Description</th>
	 ...
	 *         </table>
	 */
	public SERVICE_STATUS_HANDLE RegisterServiceCtrlHandlerEx(String lpServiceName, HandlerEx lpHandlerProc, Pointer lpContext);
	 ...
	 *            A handle to the status information structure for the current
	 *            service. This handle is returned by the
	 *            RegisterServiceCtrlHandlerEx function.
	 * @param lpServiceStatus
	 *            A pointer to the SERVICE_STATUS structure the contains the
	 *            latest status information for the calling service.
	 * 
	 * @return true if function succeeds. To get extended error information,
	 *         call GetLastError. Possible error codes:
	 *         <table>
	 ...
	 *         <tr>
	 *         <td>ERROR_INVALID_DATA</td>
	 *         <td>The specified service status structure is invalid.</td>
	 *         </tr>
	 *         <tr>
	 *         <td>ERROR_INVALID_HANDLE</td>
	 *         <td>The specified handle is invalid.</td>
	 *         </tr>
	 *         </table>
	 */
	 ...
	 *
	 * @param hSCManager
	 *            [in] A handle to the service control manager database. This
	 *            handle is returned by the OpenSCManager function and must have
	 *            the SC_MANAGER_CREATE_SERVICE access right. For more
	 *            information, see Service Security and Access Rights.
	 * @param lpServiceName
	 *            [in] The name of the service to install. The maximum string
	 *            length is 256 characters. The service control manager database
	 *            preserves the case of the characters, but service name
	 *            comparisons are always case insensitive. Forward-slash (/) and
	 *            backslash (\) are not valid service name characters.
	 * @param lpDisplayName
	 *            [in, optional] The display name to be used by user interface
	 *            programs to identify the service. This string has a maximum
	 *            length of 256 characters. The name is case-preserved in the
	 *            service control manager. Display name comparisons are always
	 *            case-insensitive.
	 * @param dwDesiredAccess
	 *            [in] The access to the service. Before granting the requested
	 *            access, the system checks the access token of the calling
	 *            process. For a list of values, see Service Security and Access
	 *            Rights.
	 * @param dwServiceType
	 *            [in] The service type. This parameter can be one of the
	 *            following values.
	 *            <table>
	 *            <tr>
	 *            <th>Value</th>
	 ...
	 *            </tr>
	 *            <tr>
	 *            <td>SERVICE_ADAPTER<br>
	 * 			0x00000004</td>
	 *            <td>Reserved.</td>
	 *            </tr>
	 *            <tr>
	 *            <td>SERVICE_FILE_SYSTEM_DRIVER<br>
	 * 			0x00000002</td>
	 *            <td>File system driver service.</td>
	 *            </tr>
	 *            <tr>
	 *            <td>SERVICE_KERNEL_DRIVER<br>
	 * 			0x00000001</td>
	 *            <td>Driver service.</td>
	 *            </tr>
	 *            <tr>
	 *            <td>SERVICE_RECOGNIZER_DRIVER<br>
	 * 			0x00000008</td>
	 *            <td>Reserved.</td>
	 *            </tr>
	 *            <tr>
	 *            <td>SERVICE_WIN32_OWN_PROCESS<br>
	 * 			0x00000010</td>
	 *            <td>Service that runs in its own process.</td>
	 *            </tr>
	 *            <tr>
	 *            <td>SERVICE_WIN32_SHARE_PROCESS<br>
	 * 			0x00000020</td>
	 *            <td>Service that shares a process with one or more other
	 *            services. For more information, see Service Programs.</td>
	 *            </tr>
	 *            </table>
	 * 
	 *            <p>
	 ...
	 *            </tr>
	 *            <tr>
	 *            <td>SERVICE_INTERACTIVE_PROCESS<br>
	 * 			0x00000100</td>
	 ...
	 * 
	 * @param dwStartType
	 *            [in] The service start options. This parameter can be one of
	 ...
	 *            </tr>
	 *            <tr>
	 *            <td>SERVICE_AUTO_START<br>
	 * 			0x00000002</td>
	 *            <td>A service started automatically by the service control
	 *            manager during system startup.</td>
	 *            </tr>
	 *            <tr>
	 *            <td>SERVICE_BOOT_START<br>
	 * 			0x00000000</td>
	 *            <td>A device driver started by the system loader. This value
	 *            is valid only for driver services.</td>
	 *            </tr>
	 *            <tr>
	 *            <td>SERVICE_DEMAND_START<br>
	 * 			0x00000003</td>
	 *            <td>A service started by the service control manager when a
	 *            process calls the StartService function.</td>
	 *            </tr>
	 *            <tr>
	 *            <td>SERVICE_DISABLED<br>
	 * 			0x00000004</td>
	 *            <td>A service that cannot be started. Attempts to start the
	 *            service result in the error code ERROR_SERVICE_DISABLED.</td>
	 *            </tr>
	 *            <tr>
	 *            <td>SERVICE_SYSTEM_START<br>
	 * 			0x00000001</td>
	 *            <td>A device driver started by the IoInitSystem function. This
	 *            value is valid only for driver services.</td>
	 *            </tr>
	 *            </table>
	 *
	 * @param dwErrorControl
	 *            [in] The severity of the error, and action taken, if this
	 *            service fails to start. This parameter can be one of the
	 *            following values.
	 * 
	 *            <table>
	 *            <tr>
	 ...
	 *            </tr>
	 *            <tr>
	 *            <td>SERVICE_ERROR_CRITICAL<br>
	 * 			0x00000003</td>
	 *            <td>The startup program logs the error in the event log, if
	 *            possible. If the last-known-good configuration is being
	 *            started, the startup operation fails. Otherwise, the system is
	 *            restarted with the last-known good configuration.</td>
	 *            </tr>
	 *            <tr>
	 *            <td>SERVICE_ERROR_IGNORE<br>
	 * 			0x00000000</td>
	 *            <td>The startup program ignores the error and continues the
	 *            startup operation.</td>
	 *            </tr>
	 *            <tr>
	 *            <td>SERVICE_ERROR_NORMAL<br>
	 * 			0x00000001</td>
	 *            <td>The startup program logs the error in the event log but
	 *            continues the startup operation.</td>
	 *            </tr>
	 *            <tr>
	 *            <td>SERVICE_ERROR_SEVERE<br>
	 * 			0x00000002</td>
	 *            <td>The startup program logs the error in the event log. If
	 *            the last-known-good configuration is being started, the
	 *            startup operation continues. Otherwise, the system is
	 *            restarted with the last-known-good configuration.</td>
	 *            </tr>
	 *            </table>
	 *
	 * @param lpBinaryPathName
	 *            [in, optional] The fully qualified path to the service binary
	 *            file. If the path contains a space, it must be quoted so that
	 *            it is correctly interpreted. For example, "d:\\my
	 *            share\\myservice.exe" should be specified as "\"d:\\my
	 *            share\\myservice.exe\"".
	 *
	 *            <p>
	 * 			The path can also include arguments for an auto-start service.
	 *            For example, "d:\\myshare\\myservice.exe arg1 arg2". These
	 *            passed to the service entry point (typically the main
	 *            function).
	 *            </p>
	 *
	 *            <p>
	 * 			If you specify a path on another computer, the share must be
	 *            accessible by the computer account of the local computer
	 *            because this is the security context used in the remote call.
	 *            However, this requirement allows any potential vulnerabilities
	 *            in the remote computer to affect the local computer.
	 *            Therefore, it is best to use a local file.
	 *            </p>
	 * 
	 * @param lpLoadOrderGroup
	 *            [in, optional] The names of the load ordering group of which
	 *            this service is a member. Specify NULL or an empty string if
	 *            the service does not belong to a group.
	 *
	 *            <p>
	 * 			The startup program uses load ordering groups to load groups
	 *            of services in a specified order with respect to the other
	 *            groups. The list of load ordering groups is contained in the
	 ...
	 *            </p>
	 * @param lpdwTagId
	 *            [out, optional] A pointer to a variable that receives a tag
	 *            value that is unique in the group specified in the
	 *            lpLoadOrderGroup parameter. Specify NULL if you are not
	 *            changing the existing tag.
	 *
	 *            <p>
	 * 			You can use a tag for ordering service startup within a load
	 *            ordering group by specifying a tag order vector in the
	 *            following registry value:
	 *            </p>
	 *
	 *            <p>
	 ...
	 *            </p>
	 * @param lpDependencies
	 *            [in, optional] A pointer to a double null-terminated array of
	 *            null-separated names of services or load ordering groups that
	 *            the system must start before this service. Specify NULL or an
	 *            empty string if the service has no dependencies. Dependency on
	 *            a group means that this service can run if at least one member
	 *            of the group is running after an attempt to start all members
	 *            of the group.
	 *
	 *            <p>
	 * 			You must prefix group names with SC_GROUP_IDENTIFIER so that
	 *            they can be distinguished from a service name, because
	 *            services and service groups share the same name space.
	 *            </p>
	 * @param lpServiceStartName
	 *            [in, optional] The name of the account under which the service
	 *            should run. If the service type is SERVICE_WIN32_OWN_PROCESS,
	 *            use an account name in the form DomainName\UserName. The
	 *            service process will be logged on as this user. If the account
	 *            belongs to the built-in domain, you can specify .\UserName.
	 *
	 *            <p>
	 * 			If this parameter is NULL, CreateService uses the LocalSystem
	 *            account. If the service type specifies
	 *            SERVICE_INTERACTIVE_PROCESS, the service must run in the
	 *            LocalSystem account.
	 *            </p>
	 *
	 *            <p>
	 * 			If this parameter is NT AUTHORITY\LocalService, CreateService
	 *            uses the LocalService account. If the parameter is NT
	 *            AUTHORITY\NetworkService, CreateService uses the
	 ...
	 *            that the system uses to load the device driver. Specify NULL
	 *            if the driver is to use a default object name created by the
	 *            I/O system.
	 *            </p>
	 *
	 *            <p>
	 * 			A service can be configured to use a managed account or a
	 *            virtual account. If the service is configured to use a managed
	 *            service account, the name is the managed service account name.
	 *            If the service is configured to use a virtual account, specify
	 *            the name as NT SERVICE\ServiceName. For more information about
	 *            managed service accounts and virtual accounts, see the Service
	 *            Accounts Step-by-Step Guide.
	 *
	 *            <p>
	 * 			<strong>Windows Server 2008, Windows Vista, Windows Server
	 *            2003 and Windows XP:</strong> Managed service accounts and
	 *            virtual accounts are not supported until Windows 7 and Windows
	 *            Server 2008 R2.
	 *            </p>
	 * @param lpPassword
	 *            [in, optional] The password to the account name specified by
	 *            the lpServiceStartName parameter. Specify an empty string if
	 *            the account has no password or if the service runs in the
	 *            LocalService, NetworkService, or LocalSystem account. For more
	 *            information, see Service Record List.
	 *
	 *            <p>
	 * 			If the account name specified by the lpServiceStartName
	 *            parameter is the name of a managed service account or virtual
	 *            account name, the lpPassword parameter must be NULL.
	 *            </p>
	 *
	 *            <p>
	 ...
	 *            </p>
	 *
	 * @return SC_HANDLE on success, NULL on error. Call GetLastError to get
	 *         extended error condition. Possible error codes:
	 * 
	 *         <table>
	 *         <tr>
	 ...
	 *         <td>The handle to the SCM database does not have the
	 *         SC_MANAGER_CREATE_SERVICE access right.</td>
	 *         </tr>
	 *         <tr>
	 *         <td>ERROR_CIRCULAR_DEPENDENCY</td>
	 *         <td>A circular service dependency was specified.</td>
	 *         </tr>
	 *         <tr>
	 *         <td>ERROR_DUPLICATE_SERVICE_NAME</td>
	 *         <td>The display name already exists in the service control
	 *         manager database either as a service name or as another display
	 *         name.</td>
	 *         </tr>
	 *         <tr>
	 *         <td>ERROR_INVALID_HANDLE</td>
	 *         <td>The handle to the specified service control manager database
	 *         is invalid.</td>
	 *         </tr>
	 *         <tr>
	 *         <td>ERROR_INVALID_NAME</td>
	 ...
	 *         </tr>
	 *         <tr>
	 *         <td>ERROR_INVALID_SERVICE_ACCOUNT</td>
	 *         <td>The user account name specified in the lpServiceStartName
	 *         parameter does not exist.</td>
	 *         </tr>
	 *         <tr>
	 *         <td>ERROR_SERVICE_EXISTS</td>
	 *         <td>The specified service already exists in this database.</td>
	 *         </tr>
	 *         <tr>
	 *         <td>ERROR_SERVICE_MARKED_FOR_DELETE</td>
	 *         <td>The specified service already exists in this database and has
	 *         been marked for deletion.</td>
	 *         </tr>
	 *         </table>
	 */
	public SC_HANDLE CreateService(SC_HANDLE hSCManager, String lpServiceName, String lpDisplayName, int dwDesiredAccess,
			int dwServiceType, int dwStartType, int dwErrorControl, String lpBinaryPathName, String lpLoadOrderGroup,
			IntByReference lpdwTagId, String lpDependencies, String lpServiceStartName, String lpPassword);
	 ...
	 * 
	 * @param hService
	 *            [in] A handle to the service. This handle is returned by the
	 *            OpenService or CreateService function, and it must have the
	 *            DELETE access right.
	 * 
	 * @return true if function succeeds. To get extended error information,
	 *         call GetLastError. Possible error codes:
	 ...
	 *         <tr>
	 *         <td>ERROR_ACCESS_DENIED</td>
	 *         <td>The handle does not have the DELETE access right.</td>
	 *         </tr>
	 *         <tr>
	 *         <td>ERROR_INVALID_HANDLE</td>
	 *         <td>The specified handle is invalid.</td>
	 ...
	 *         <tr>
	 *         <td>ERROR_SERVICE_MARKED_FOR_DELETE</td>
	 *         <td>The specified service has already been marked for
	 *         deletion.</td>
	 *         </tr>
	 *         </table>
	 */
