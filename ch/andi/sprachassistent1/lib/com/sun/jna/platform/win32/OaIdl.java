...
public interface OaIdl {
	...
	// The DATE Type is defined in localtime and the java Date type always
	// contains
	...
	public static class EXCEPINFO extends Structure {
		...
		public static final List<String> FIELDS = createFieldsOrder("wCode", "wReserved", "bstrSource", "bstrDescription",
				"bstrHelpFile", "dwHelpContext", "pvReserved", "pfnDeferredFillIn", "scode");
	...
	public static class DATE extends Structure {
		...
		public void setFromJavaDate(Date javaDate) {
			...
			dayDate.setTime(dayDate.getTime() / 1000 * 1000); // Clear
																// milliseconds

			double integralPart = Math.floor(daysAsFract);
			double fractionalPart = Math.signum(daysAsFract)
					* ((javaDate.getTime() - dayDate.getTime()) / (24d * 60 * 60 * 1000));
	 ...
	 * </p>
	 * 
	 * <blockquote>A 32-bit signed integer. The range is �2147483648 through
	 * 2147483647 decimal.</blockquote>
	 */
	public static class SAFEARRAY extends Structure implements Closeable {
		...
		public static final List<String> FIELDS = createFieldsOrder("cDims", "fFeatures", "cbElements", "cLocks", "pvData",
				"rgsabound");
		 ...
		 *
		 * @param vartype
		 *            type of array contents (see Variant.VT_* constants)
		 * @param size
		 *            array of dimension size
		 * @return SAFEARRAYWrapper or {@code NULL} if creation fails.
		 */
		 ...
		 * longer needed, it needs to be freed if not handled automatically.
		 * </p>
		 *
		 * @param indices
		 ...
		 *
		 * <p>
		 * The function creates a copy of the value. The values are allocated
		 * with native functions and need to be freed accordingly.
		 * </p>
		 *
		 * @param indices
		 *            the index, order follows java/C convention
		 ...
		 *
		 * <p>
		 * As in the all the accessor functions, that index is converted to java
		 * conventions.
		 * </p>
		 * 
		 * @param dimension
	...
	public static class TLIBATTR extends Structure {
		...
		public static final List<String> FIELDS = createFieldsOrder("guid", "lcid", "syskind", "wMajorVerNum", "wMinorVerNum",
				"wLibFlags");
	...
	public static class FUNCDESC extends Structure {
		...
		public static final List<String> FIELDS = createFieldsOrder("memid", "lprgscode", "lprgelemdescParam", "funckind",
				"invkind", "callconv", "cParams", "cParamsOpt", "oVft", "cScodes", "elemdescFunc", "wFuncFlags");
	...
	public class VARDESC extends Structure {
		...
		public static final List<String> FIELDS = createFieldsOrder("memid", "lpstrSchema", "_vardesc", "elemdescVar",
				"wVarFlags", "varkind");
	...
	public class TYPEATTR extends Structure {
		...
		public static final List<String> FIELDS = createFieldsOrder("guid", "lcid", "dwReserved", "memidConstructor",
				"memidDestructor", "lpstrSchema", "cbSizeInstance", "typekind", "cFuncs", "cVars", "cImplTypes", "cbSizeVft",
				"cbAlignment", "wTypeFlags", "wMajorVerNum", "wMinorVerNum", "tdescAlias", "idldescType");
		...
		/**
		 * The type is predefined. The client application should automatically
		 * create a single instance of the object that has this attribute. The
		 * name of the variable that points to the object is the same as the
		...
		/**
		 * The type is a control from which other types will be derived, and
		...
		/**
		 * The types used in the interface are fully compatible with Automation,
		 * including VTBL binding support. Setting dual on an interface sets
		 * this flag in addition to TYPEFLAG_FDUAL. Not allowed on
		 * dispinterfaces.
		 */
		public final static int TYPEFLAGS_FOLEAUTOMATION = 0x100;

		/**
		 * Should not be accessible from macro languages. This flag is intended
		 * for system-level types or types that type browsers should not
		 * display.
		 */
		...
		/**
		 * Indicates that the interface derives from IDispatch, either directly
		 * or indirectly. This flag is computed. There is no Object Description
		...
		/**
		 * Interfaces can be marked with this flag to indicate that they will be
		 * using a proxy/stub dynamic link library. This flag specifies that the
		 * typelib proxy should not be unregistered when the typelib is
		 * unregistered.
		 */
