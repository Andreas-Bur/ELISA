...
public abstract class COMUtils {
	...
	/**
	 * Check status of HRESULT if it indicates a failed call a
	 * COMInvokeException is reaised.
	 * 
	 * <p>
	 * The string members of the pExcepInfo are freed in this call and can't be
	 * used afterwards. The structure is not freeed, as it is expected, that is
	 * allocated via the Memory object of JNA.
	 * </p>
	 * 
	 * @param hr
	 *            the hr
	...
	public static void checkRC(HRESULT hr, EXCEPINFO pExcepInfo, IntByReference puArgErr) {
		...
		if (FAILED(hr)) {
			...
			throw new COMInvokeException(formatMessage.toString(), hr, errorArg, description, helpCtx, helpFile, scode, source,
					wCode);
		}
	...
	/**
	 * Check if COM was initialized correctly. The initialization status is not
	 * changed!
	 *
	 * <p>
	...
	public static boolean comIsInitialized() {
		...
		if (hr.equals(W32Errors.S_OK)) {
			...
			// in this check. According to MSDN CoUninitialize needs to be
			// called
			// in this case.
			Ole32.INSTANCE.CoUninitialize();
			return true;
		...
		// will be raised.
		COMUtils.checkRC(hr);
		// The return will not be met, as COMUtils#checkRC will raise an
		// exception
		return false;
	}
