 ...
 * <p>
 * NOTE: If you use {@link #setWindowMask(Window,Shape)} and override
 * {@link Window#paint(Graphics)} on OS X, you'll need to explicitly set the
 * clip mask on the <code>Graphics</code> object with the window mask; only the
 * content pane of the window and below have the window mask automatically
 * applied.
 * <p>
 * NOTE: On OSX, the property <code>apple.awt.draggableWindowBackground</code>
 * is set automatically when a window's background color has an alpha component.
 * That property must be set to its final value <em>before</em> the heavyweight
 * peer for the Window is created. Once {@link Component#addNotify} has been
 * called on the component, causing creation of the heavyweight peer, changing
...
public class WindowUtils {
	 ...
	 * repaint requests. The content's repaint method will be invoked whenever
	 * any part of the ancestor window is repainted.
	 */
	...
	public static abstract class NativeWindowUtils {
		protected abstract class TransparentContentPane extends JPanel implements AWTEventListener {
			...
			/**
			 * Use the contents of the given BufferedImage to paint directly on
			 * this component's ancestor window.
			 */
		...
		/**
		 * Set the window to be transparent. Only explicitly painted pixels will
		 * be non-transparent. All pixels will be composited with whatever is
		 * under the window using their alpha values.
		 */
		...
		/**
		 * Set the window mask based on the given Raster, which should be
		 * treated as a bitmap (zero/nonzero values only). A value of
		...
		/**
		 * Set the window mask based on an Icon. All non-transparent pixels will
		...
		/**
		 * Use this method to ensure heavyweight popups are used in conjunction
		 * with a given window. This prevents the window's alpha setting or mask
		 * region from being applied to the popup.
		 */
		...
		/**
		 * Detects the full file path of the process associated with the
	...
	private static class Holder {
		/**
		 * Indicates whether a window must be visible before its native handle
		 * can be obtained. This wart is caused by the Java 1.4/X11
		 * implementation.
		 */
	...
	private static class W32WindowUtils extends NativeWindowUtils {
		 ...
		 * window on any change. It also does not paint window decorations when
		...
		public String getProcessFilePath(final HWND hwnd) {
			...
			final HANDLE process = Kernel32.INSTANCE.OpenProcess(WinNT.PROCESS_QUERY_INFORMATION | WinNT.PROCESS_VM_READ, false,
					pid.getValue());
		...
		private void fixWindowDragging(Window w, String context) {
			if (w instanceof RootPaneContainer) {
				...
				if (oldDraggable == null) {
					...
					if (w.isDisplayable()) {
						System.err.println(context + "(): To avoid content dragging, " + context
								+ "() must be called before the window is realized, or " + WDRAG
								+ " must be set to Boolean.FALSE before the window is realized.  If you really want content dragging, set "
								+ WDRAG + " on the window's root pane to Boolean.TRUE before calling " + context
	 ...
	 * is not supported. The mask is treated as a bitmap and ignores
	 * transparency.
	 */
	 ...
	 * fully transparent. The alpha level is applied equally to all window
	 * pixels.
	 * <p>
	 ...
	 * <code>apple.awt.draggableWindowBackground</code> must be set to its final
	 * value <em>before</em> the heavyweight peer for the Window is created.
	 * Once {@link Component#addNotify} has been called on the component,
	 ...
	 * the Window is created. Once {@link Component#addNotify} has been called
	 * on the component, causing creation of the heavyweight peer, changing this
	 * property has no effect.
	 * 
	 * @see <a href=
	 *      "http://developer.apple.com/technotes/tn2007/tn2196.html#APPLE_AWT_DRAGGABLEWINDOWBACKGROUND">Apple
	...
	/**
	 * Obtains the set icon for the window associated with the specified window
	 * handle.
	 *
	 * @param hwnd
	 *            The concerning window handle.
