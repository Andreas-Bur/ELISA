...
public interface Ntifs extends WinDef, BaseTSD {
	...
	public static class SymbolicLinkReparseBuffer extends Structure {
		 ...
		 * array. Note that this offset must be divided by sizeof(WCHAR) to get
		 * the array index.
		 */
		 ...
		 * NULL-terminated, SubstituteNameLength does not include space for the
		 ...
		 * Note that this offset must be divided by sizeof(WCHAR) to get the
		 * array index.
		 */
		...
		/**
		 * Used to indicate if the given symbolic link is an absolute or
		 * relative symbolic link. If Flags contains SYMLINK_FLAG_RELATIVE, the
		 * symbolic link contained in the PathBuffer array (at offset
		 * SubstitueNameOffset) is processed as a relative symbolic link;
		 * otherwise, it is processed as an absolute symbolic link.
		 */
		public int Flags = 0;

		/**
		 * First character of the path string. This is followed in memory by the
		 * remainder of the string. The path string contains the substitute name
		 * string and print name string. The substitute name and print name
		 * strings can appear in any order in the PathBuffer. (To locate the
		 * substitute name and print name strings in the PathBuffer, use the
		 * SubstituteNameOffset, SubstituteNameLength, PrintNameOffset, and
		 * PrintNameLength members.) <b>NOTE: MAXIMUM_REPARSE_DATA_BUFFER_SIZE
		 * is chosen here based on documentation. Because chars are two bytes,
		 * the actual array size needs to be divided by 2</b>
		 */
		...
		protected List<String> getFieldOrder() {
			return Arrays.asList(new String[] { "SubstituteNameOffset", "SubstituteNameLength", "PrintNameOffset",
	...
	public static class GenericReparseBuffer extends Structure {
		...
		/**
		 * Microsoft-defined data for the reparse point. <b>NOTE:
		 * MAXIMUM_REPARSE_DATA_BUFFER_SIZE is chosen based on documentation</b>
		 */
	 ...
	 * Microsoft reparse point. (Third-party reparse point owners must use the
	 * REPARSE_GUID_DATA_BUFFER structure instead.)
	 */
	public static class REPARSE_DATA_BUFFER extends Structure {
		...
		/**
		 * Length, in bytes, of the unparsed portion of the file name pointed to
		 * by the FileName member of the associated file object. For more
		 * information about the FileName member, see FILE_OBJECT. This member
		 * is only valid for create operations when the I/O fails with
		 * STATUS_REPARSE. For all other purposes, such as setting or querying a
		 * reparse point for the reparse data, this member is treated as
		 * reserved.
		 */
		...
		public void read() {
			super.read();
			// Set structure value based on ReparseTag and then re-read the
			// union.
			switch (ReparseTag) {
			default:
