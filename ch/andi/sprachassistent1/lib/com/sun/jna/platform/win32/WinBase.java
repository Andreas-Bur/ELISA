...
public interface WinBase extends WinDef, BaseTSD {
	...
	public static class FILE_BASIC_INFO extends Structure {
		...
		/**
		 * The time the file was created in FILETIME format, which is a 64-bit
		...
		/**
		 * The file attributes. For a list of attributes, see File Attribute
		 * Constants. If this is set to 0 in a FILE_BASIC_INFO structure passed
		 * to SetFileInformationByHandle then none of the attributes are
		 * changed.
		 */
		...
		protected List<String> getFieldOrder() {
			return Arrays
					.asList(new String[] { "CreationTime", "LastAccessTime", "LastWriteTime", "ChangeTime", "FileAttributes" });
		}
		...
		public FILE_BASIC_INFO(Pointer memory) {
			super(memory);
			read();
			// This is admittedly odd, but the read() doesn't properly
			// initialize the LARGE_INTEGERs via contructors, so do so here.
		...
		public FILE_BASIC_INFO(FILETIME CreationTime, FILETIME LastAccessTime, FILETIME LastWriteTime, FILETIME ChangeTime,
				int FileAttributes) {
	...
	/**
	 * Receives extended information for the file. Used for file handles. Use
	 * only when calling GetFileInformationByHandleEx.
	 */
	public static class FILE_STANDARD_INFO extends Structure {
		...
		public FILE_STANDARD_INFO(LARGE_INTEGER AllocationSize, LARGE_INTEGER EndOfFile, int NumberOfLinks, boolean DeletePending,
				boolean Directory) {
	...
	/**
	 * Indicates whether a file should be deleted. Used for any handles. Use
	...
	public static class FILE_DISPOSITION_INFO extends Structure {
		...
		/**
		 * Indicates whether the file should be deleted. Set to TRUE to delete
		 * the file. This member has no effect if the handle was opened with
	...
	public static class FILE_COMPRESSION_INFO extends Structure {
		...
		@Override
		protected List<String> getFieldOrder() {
			return Arrays.asList(new String[] { "CompressedFileSize", "CompressionFormat", "CompressionUnitShift", "ChunkShift",
		...
		public FILE_COMPRESSION_INFO(LARGE_INTEGER CompressedFileSize, short CompressionFormat, byte CompressionUnitShift,
				byte ChunkShift, byte ClusterShift) {
	...
	/**
	 * Contains identification information for a file. This structure is
	 * returned from the GetFileInformationByHandleEx function when FileIdInfo
	...
	// FINDEX_INFO_LEVELS values defines values that are used with the
	// FindFirstFileEx function to specify the information level of the returned
	// data.

	/**
	 * The FindFirstFileEx function retrieves a standard set of attribute
	 * information. The data is returned in a WIN32_FIND_DATA structure.
	 */
	 ...
	 * WIN32_FIND_DATA structure, and the cAlternateFileName member is always a
	 * NULL string.
	 */
	int FindExInfoBasic = 1;
	/**
	 * This value is used for validation. Supported values are less than this
	 * value.
	 */
	int FindExInfoMaxInfoLevel = 2;

	// FINDEX_SEARCH_OPS values defines values that are used with the
	// FindFirstFileEx function to specify the type of filtering to perform.
	/**
	 * The search for a file that matches a specified file name. The
	 * lpSearchFilter parameter of FindFirstFileEx must be NULL when this search
	 * operation is used.
	 */
	int FindExSearchNameMatch = 0;

	/**
	 * This is an advisory flag. If the file system supports directory
	 * filtering, the function searches for a file that matches the specified
	 * name and is also a directory. If the file system does not support
	 * directory filtering, this flag is silently ignored. The lpSearchFilter
	 * parameter of the FindFirstFileEx function must be NULL when this search
	 * value is used. If directory filtering is desired, this flag can be used
	 * on all file systems, but because it is an advisory flag and only affects
	 * file systems that support it, the application must examine the file
	 * attribute data stored in the lpFindFileData parameter of the
	 * FindFirstFileEx function to determine whether the function has returned a
	 * handle to a directory.
	 */
	int FindExSearchLimitToDirectories = 1;

	/**
	 * This filtering type is not available. For more information, see Device
	 * Interface Classes.
	 */
	...
	public static class WIN32_FIND_DATA extends Structure {
		...
		/**
		 * The file attributes of a file. For possible values and their
		 * descriptions, see File Attribute Constants. The
		 * FILE_ATTRIBUTE_SPARSE_FILE attribute on the file is set if any of the
		 * streams of the file have ever been sparse.
		 */
		 ...
		 * file was last read from, written to, or for executable files, run.
		 * For a directory, the structure specifies when the directory is
		 * created. If the underlying file system does not support last access
		 * time, this member is zero. On the FAT file system, the specified date
		 * for both files and directories is correct, but the time of day is
		...
		/**
		 * A FILETIME structure. For a file, the structure specifies when the
		 * file was last written to, truncated, or overwritten, for example,
		 * when WriteFile or SetEndOfFile are used. The date and time are not
		 * updated when file attributes or security descriptors are changed. For
		 * a directory, the structure specifies when the directory is created.
		 * If the underlying file system does not support last write time, this
		 * member is zero.
		 */
		public FILETIME ftLastWriteTime;

		/**
		 * The high-order DWORD value of the file size, in bytes. This value is
		 * zero unless the file size is greater than MAXDWORD. The size of the
		 * file is equal to (nFileSizeHigh * (MAXDWORD+1)) + nFileSizeLow.
		 */
		 ...
		 * reparse point tag. Otherwise, this value is undefined and should not
		 * be used. For more information see Reparse Point Tags.
		 *
		 * IO_REPARSE_TAG_CSV (0x80000009) IO_REPARSE_TAG_DEDUP (0x80000013)
		 * IO_REPARSE_TAG_DFS (0x8000000A) IO_REPARSE_TAG_DFSR (0x80000012)
		 * IO_REPARSE_TAG_HSM (0xC0000004) IO_REPARSE_TAG_HSM2 (0x80000006)
		 * IO_REPARSE_TAG_MOUNT_POINT (0xA0000003) IO_REPARSE_TAG_NFS
		 * (0x80000014) IO_REPARSE_TAG_SIS (0x80000007) IO_REPARSE_TAG_SYMLINK
		...
		/**
		 * The name of the file. <b>NOTE: When written from Native memory, this
		 * will be a null terminated string. Any characters after the null
		 * terminator are random memory. Use function getFileName to get a
		 * String with the name.</b>
		 */
		public char[] cFileName = new char[MAX_PATH];

		/**
		 * An alternative name for the file. This name is in the classic 8.3
		 * file name format. <b>NOTE: When written from Native memory, this will
		 * be a null terminated string. Any characters after the null terminator
		 * are random memory. Use function getAlternateFileName to get a String
		 * with the alternate name.</b>
		 */
		...
		@Override
		protected List<String> getFieldOrder() {
			return Arrays.asList(new String[] { "dwFileAttributes", "ftCreationTime", "ftLastAccessTime", "ftLastWriteTime",
		...
		public WIN32_FIND_DATA(int dwFileAttributes, FILETIME ftCreationTime, FILETIME ftLastAccessTime, FILETIME ftLastWriteTime,
				int nFileSizeHigh, int nFileSizeLow, int dwReserved0, int dwReserved1, char[] cFileName,
	...
	public static class FILETIME extends Structure {
		 ...
		 * a 64-bit unsigned integer representing the number of 100-nanosecond
	 ...
	 * 
	 * @see <A HREF=
	 *      "http://msdn.microsoft.com/en-us/library/ms724950(VS.85).aspx">SYSTEMTIME
	 *      structure</A>
	 */
	public static class SYSTEMTIME extends Structure {
		...
		public short wMonth;
		// The day of the week. The valid values for this member are 0 through
		// 6.
		public short wDayOfWeek;
		// The day of the month. The valid values for this member are 1 through
		// 31.
		public short wDay;
		...
		public void fromCalendar(Calendar cal) {
			...
			wMonth = (short) (1 + cal.get(Calendar.MONTH) - Calendar.JANUARY); // 1
																				// =
																				// January
			...
			wDayOfWeek = (short) (cal.get(Calendar.DAY_OF_WEEK) - Calendar.SUNDAY); // 0
																					// =
																					// Sunday
		}
		...
		protected List<String> getFieldOrder() {
			return Arrays.asList(
					new String[] { "wYear", "wMonth", "wDayOfWeek", "wDay", "wHour", "wMinute", "wSecond", "wMilliseconds" });
		}
	...
	public static class TIME_ZONE_INFORMATION extends Structure {
		...
		@Override
		protected List<String> getFieldOrder() {
			return Arrays.asList(new String[] { "Bias", "StandardName", "StandardDate", "StandardBias", "DaylightName",
	...
	public static class SYSTEM_INFO extends Structure {
		...
		@Override
		protected List<String> getFieldOrder() {
			return Arrays.asList(new String[] { "processorArchitecture", "dwPageSize", "lpMinimumApplicationAddress",
					"lpMaximumApplicationAddress", "dwActiveProcessorMask", "dwNumberOfProcessors", "dwProcessorType",
	...
	public static class MEMORYSTATUSEX extends Structure {
		...
		@Override
		protected List<String> getFieldOrder() {
			return Arrays.asList(new String[] { "dwLength", "dwMemoryLoad", "ullTotalPhys", "ullAvailPhys", "ullTotalPageFile",
	 ...
	 * {@link Kernel32#CreateFile}, {@link Kernel32#CreatePipe}, or
	 * {@link Advapi32#RegCreateKeyEx}.
	 */
	...
	/**
	 * Represents a thread entry point in another process. Can only be expressed
	 * as a pointer, as the location has no meaning in the Java process.
	 */
	...
	/**
	 * Specifies a type of computer name to be retrieved by the
	 * GetComputerNameEx function
	 */
	public static interface COMPUTER_NAME_FORMAT {
		/**
		 * The NetBIOS name of the local computer or the cluster associated with
		 * the local computer. This name is limited to MAX_COMPUTERNAME_LENGTH +
		 * 1 characters and may be a truncated version of the DNS host name. For
		 * example, if the DNS host name is &quot;corporate-mail-server&quot;,
		 * the NetBIOS name would be &quot;corporate-mail-"&quot;.
		 */
		...
		/**
		 * The name of the DNS domain assigned to the local computer or the
		 * cluster associated with the local computer.
		 */
		int ComputerNameDnsDomain = 2;

		/**
		 * The fully qualified DNS name that uniquely identifies the local
		 * computer or the cluster associated with the local computer. This name
		 * is a combination of the DNS host name and the DNS domain name, using
		 * the form HostName.DomainName. For example, if the DNS host name is
		 * &quot;corporate-mail-server&quot; and the DNS domain name is
		 * &quot;microsoft.com&quot;, the fully qualified DNS name is
		 * &quot;corporate-mail-server.microsoft.com&quot;.
		 */
		int ComputerNameDnsFullyQualified = 3;

		/**
		 * The NetBIOS name of the local computer. On a cluster, this is the
		 * NetBIOS name of the local node on the cluster.
		 */
		int ComputerNamePhysicalNetBIOS = 4;

		/**
		 * The DNS host name of the local computer. On a cluster, this is the
		 * DNS host name of the local node on the cluster.
		 */
		int ComputerNamePhysicalDnsHostname = 5;

		/**
		 * The name of the DNS domain assigned to the local computer. On a
		 * cluster, this is the DNS domain of the local node on the cluster.
		 */
		int ComputerNamePhysicalDnsDomain = 6;

		/**
		 * The fully qualified DNS name that uniquely identifies the computer.
		 * On a cluster, this is the fully qualified DNS name of the local node
		 * on the cluster. The fully qualified DNS name is a combination of the
		 * DNS host name and the DNS domain name, using the form
		 * HostName.DomainName.
		 */
		int ComputerNamePhysicalDnsFullyQualified = 7;

		/**
		 * Note used - serves as an upper limit in case one wants to go through
		 * all the values
		 */
	 ...
	 * The system calls ImportCallback one or more times, each time to retrieve
	 * a portion of a backup file's data. ImportCallback reads the data from a
	 ...
	 * parameters determine the behavior of {@link Kernel32#ReadFile},
	 * {@link Kernel32#WriteFile}, ReadFileEx, and WriteFileEx operations on the
	 * device. <br>
	 *
	 * <b>Remarks</b><br>
	...
	public static class COMMTIMEOUTS extends Structure {
		 ...
		 * {@link Kernel32#ReadFile} operation is completed and any buffered
		 * data is returned. A value of zero indicates that interval time-outs
		 * are not used.
		 *
	...
	public static class DCB extends Structure {
		...
		public static class DCBControllBits extends DWORD {
			...
			/**
			 * If this member is TRUE, the communications driver is sensitive to
			 * the state of the DSR signal.<br>
			...
			/**
			 * The DTR (data-terminal-ready) flow control. This member can be
			...
			/**
			 * Indicates whether bytes received with parity errors are replaced
			 * with the character specified by the ErrorChar member.<br>
			 * If this member is TRUE and the fParity member is TRUE,
			 * replacement occurs.
			 *
			 * @param fErrorChar
			 */
			 ...
			 * character is received and starts again when the XonChar character
			 * is received.
			 *
			 * @param fOutX
			 */
			...
			/**
			 * If this member is TRUE, the CTS (clear-to-send) signal is
			 * monitored for output flow control.<br>
			...
			/**
			 * If this member is TRUE, the DSR (data-set-ready) signal is
			 * monitored for output flow control.<br>
			...
			/**
			 * If this member is TRUE, parity checking is performed and errors
			 * are reported.
			 *
			 * @param fParity
			 */
			...
			/**
			 * If this member is TRUE, transmission continues after the input
			 * buffer has come within XoffLim bytes of being full and the driver
			 * has transmitted the XoffChar character to stop receiving
			 * bytes.<br>
			 * If this member is FALSE, transmission does not continue until the
			 * input buffer is within XonLim bytes of being empty and the driver
		...
		@Override
		protected List<String> getFieldOrder() {
			return Arrays.asList(new String[] { "DCBlength", "BaudRate", "controllBits", "wReserved", "XonLim", "XoffLim",
