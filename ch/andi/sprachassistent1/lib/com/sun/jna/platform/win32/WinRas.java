...
public interface WinRas {
	...
	/**
	 * The RASEAPINFO structure contains user-specific Extensible Authentication
	 * Protocol (EAP) information. Use RASEAPINFO to pass this information to
	...
	public static class RASDIALEXTENSIONS extends Structure {
		...
		@Override
		protected List<String> getFieldOrder() {
			return Arrays.asList(new String[] { "dwSize", "dwfOptions", "hwndParent", "reserved", "reserved1", "RasEapInfo",
					"fSkipPppAuth", "RasDevSpecificInfo", });
		}
	}
	...
	public static class RASDIALPARAMS extends Structure {
		...
		@Override
		protected List<String> getFieldOrder() {
			return Arrays.asList(new String[] { "dwSize", "szEntryName", "szPhoneNumber", "szCallbackNumber", "szUserName",
					"szPassword", "szDomain", });
		}
	}
	...
	public static class RASCONN extends Structure {
		...
		@Override
		protected List<String> getFieldOrder() {
			return Arrays.asList(new String[] { "dwSize", "hrasconn", "szEntryName", "szDeviceType", "szDeviceName",
					"szPhonebook", "dwSubEntry", "guidEntry", "dwFlags", "luid", "guidCorrelationId" });
		}
	}
	...
	public static class RAS_STATS extends Structure {
		...
		@Override
		protected List<String> getFieldOrder() {
			return Arrays.asList(new String[] { "dwSize", "dwBytesXmited", "dwBytesRcved", "dwFramesXmited", "dwFramesRcved",
					"dwCrcErr", "dwTimeoutErr", "dwAlignmentErr", "dwHardwareOverrunErr", "dwFramingErr", "dwBufferOverrunErr",
					"dwCompressionRatioIn", "dwCompressionRatioOut", "dwBps", "dwConnectDuration", });
		}
	}
	...
	/**
	 * The RASPPPIP structure contains the result of a PPP IP projection
	 * operation.
	 */
	public static class RASPPPIP extends Structure {
		...
		/**
		 * A value that specifies the size, in bytes, of the structure. This
		 * member must be set before it is used in a function call.
		 */
		public int dwSize;
		/**
		 * A value that specifies the result of the PPP control protocol
		 * negotiation. A value of zero indicates success. A non-zero value
		 * indicates failure, and indicates the fatal error that occurred during
		 * the control protocol negotiation.
		 */
		public int dwError;
		/**
		 * An array that contains a null-terminated string that is the client's
		 * IP address on the RAS connection. This address string has the form
		 * a.b.c.d.
		 */
		public char[] szIpAddress = new char[RAS_MaxIpAddress + 1];
		/**
		 * An array that contains a null-terminated string that is the server's
		 * IP address on the RAS connection. This string is in a.b.c.d form. PPP
		 * does not require that servers provide this address, but servers will
		 * consistently return the address anyway. Other PPP vendors may not
		 * provide the address. If the address is not available, this member
		...
		protected List<String> getFieldOrder() {
			return Arrays.asList(
					new String[] { "dwSize", "dwError", "szIpAddress", "szServerIpAddress", "dwOptions", "dwServerOptions", });
		}
	}
	...
	/**
	 * The RASCONNSTATUS structure describes the current status of a remote
	 * access connection. It is returned by the RasGetConnectStatus function.
	 */
	public static class RASCONNSTATUS extends Structure {
		...
		public int dwSize;
		/**
		 * Specifies a RASCONNSTATE enumerator value that indicates the current
		 * state of the RasDial connection process; that is, the piece of the
		 * RasDial process that is currently executing.
		 */
		public int rasconnstate;
		/**
		 * If nonzero, indicates the reason for failure. The value is one of the
		 * error values from the RasError.h header file or one of
		 * ERROR_NOT_ENOUGH_MEMORY or ERROR_INVALID_HANDLE.
		 */
		public int dwError;
		/**
		 * A string that specifies the type of the current device, if available.
		 * For example, common device types supported by RAS are "modem", "pad",
		 * "switch", "ISDN", or "null". See RASENTRY for a complete list of
		 * possible device types.
		 */
		public char[] szDeviceType = new char[RAS_MaxDeviceType + 1];
		/**
		 * A string that specifies the name of the current device, if available.
		 * This would be the name of the modem - for example, "Hayes SmartModem
		 * 2400"; the name of the PAD, for example "US Sprint"; or the name of a
		 * switch device, for example "Racal-Guardata".
		 */
		public char[] szDeviceName = new char[RAS_MaxDeviceName + 1];
		/**
		 * A string that indicates the phone number dialed for this specific
		 * connection.
		 */
		public char[] szPhoneNumber = new char[RAS_MaxPhoneNumber + 1];
		/**
		 * A RASTUNNELENDPOINT structure that contains the local client endpoint
		 * information of a virtual private network (VPN) endpoint.
		 */
		public RASTUNNELENDPOINT localEndPoint;
		/**
		 * A RASTUNNELENDPOINT structure that contains the remote server
		 * endpoint information of a virtual private network (VPN) endpoint.
		 */
		public RASTUNNELENDPOINT remoteEndPoint;
		/**
		 * A RASCONNSUBSTATE enumeration that specifies state information of an
		 * Internet Key Exchange version 2 (IKEv2) VPN tunnel.
		 */
		public int rasconnsubstate;

		@Override
		protected List<String> getFieldOrder() {
			return Arrays.asList(new String[] { "dwSize", "rasconnstate", "dwError", "szDeviceType", "szDeviceName",
					"szPhoneNumber", "localEndPoint", "remoteEndPoint", "rasconnsubstate" });
		}
	}

	/**
	 * The RASCREDENTIALS structure is used with the RasGetCredentials and
	 * RasSetCredentials functions to specify the user credentials associated
	 * with a RAS phone-book entry.
	 */
	public static class RASCREDENTIALS extends Structure {
		...
		public int dwSize;
		/**
		 * Specifies a set of bit flags. These flags indicate the members of
		 * this structure that are valid. On input, set the flags to indicate
		 * the members of interest.
		 */
		public int dwMask;
		/**
	...
	/**
	 * The RASIPADDR structure contains an IP address of the form "a.b.c.d". The
	 * RASENTRY structure uses this structure to specify the IP addresses of
	 * various servers associated with an entry in a RAS phone book.
	 */
	...
	/**
	 * The RASENTRY structure describes a phone-book entry. The
	 * RasSetEntryProperties and RasGetEntryProperties functions use this
	...
	public static class RASENTRY extends Structure {
		...
		/**
		 * Specifies the size, in bytes, of the RASENTRY structure. Before
		 * calling RasSetEntryProperties or RasGetEntryProperties, set dwSize to
		 * sizeof(RASENTRY) to identify the version of the structure.
		 */
		public int dwSize;
		/**
		 * A set of bit flags that specify connection options. Set one or more
		 * of the following flags.
		 */
		public int dwfOptions;
		/**
		 * Specifies the TAPI country/region identifier. Use the
		 * RasGetCountryInfo function to enumerate country/region identifiers.
		 * This member is ignored unless the dwfOptions member specifies the
		 * RASEO_UseCountryAndAreaCodes flag.
		 */
		public int dwCountryID;
		/**
		 * Specifies the country/region code portion of the phone number. The
		 * country/region code must correspond to the country/region identifier
		 * specified by dwCountryID. If dwCountryCode is zero, the
		 * country/region code is based on the country/region identifier
		 * specified by dwCountryID. This member is ignored unless dwfOptions
		 * specifies the RASEO_UseCountryAndAreaCodes flag.
		 */
		public int dwCountryCode;
		/**
		 * Specifies the area code as a null-terminated string. If the dialing
		 * location does not have an area code, specify an empty string (""). Do
		 * not include parentheses or other delimiters in the area code string.
		 * For example, "206" is a valid area code; "(206)" is not. This member
		 * is ignored unless the dwfOptions member specifies the
		 * RASEO_UseCountryAndAreaCodes flag.
		 */
		public char[] szAreaCode = new char[RAS_MaxAreaCode + 1];
		/**
		 * Specifies a null-terminated device-type specific destination string.
		 * The following table describes the contents of the szLocalPhoneNumber
		 * member for various device types.
		 */
		public char[] szLocalPhoneNumber = new char[RAS_MaxPhoneNumber + 1];
		/**
		 * Specifies the offset, in bytes, from the beginning of the structure
		 * to a list of consecutive null-terminated strings. The last string is
		 * terminated by two consecutive null characters. The strings are
		 * alternate phone numbers that RAS dials in the order listed if the
		 * primary number (see szLocalPhoneNumber) fails to connect. The
		 * alternate phone number strings are ANSI or Unicode, depending on
		 * whether you use the ANSI or Unicode version of the structure.
		 */
		 ...
		 * connection is active. This member is ignored unless dwfOptions
		 * specifies the RASEO_SpecificNameServers flag.
		 */
		public RASIPADDR ipaddrDns;
		/**
		 * Specifies the IP address of a secondary or backup DNS server to be
		 * used while this connection is active. This member is ignored unless
		 ...
		 * this connection is active. This member is ignored unless dwfOptions
		 * specifies the RASEO_SpecificNameServers flag.
		 */
		...
		/**
		 * Specifies the network protocols to negotiate. This member can be a
		 * combination of the following flags.
		 */
		public int dwfNetProtocols;
		/**
		 * Specifies the framing protocol used by the server. PPP is the
		 * emerging standard. SLIP is used mainly in UNIX environments. This
		 * member can be one of the following flags.
		 */
		public int dwFramingProtocol;
		/**
		 * Specifies a null-terminated string that contains the name of the
		 * script file. The file name should be a full path. This field is only
		 * used for analog dial-up connections.
		 */
		public char[] szScript = new char[MAX_PATH];
		/**
		 * Windows 2000 or later: This member is no longer supported. The
		 * szCustomDialDll member of the RASENTRY structure specifies the path
		 * to the custom-dial DLL. For more information on custom dialers, see
		 * RAS Custom Dialers.
		 */
		public char[] szAutodialDll = new char[MAX_PATH];
		/**
		 * Windows 2000 or later: This member is no longer supported. See RAS
		 * Custom Dialers for more information on custom dialers.
		 */
		public char[] szAutodialFunc = new char[MAX_PATH];
		/**
		 * Specifies a null-terminated string that indicates the RAS device type
		 * referenced by szDeviceName. This member can be one of the following
		 * string constants.
		 */
		public char[] szDeviceType = new char[RAS_MaxDeviceType + 1];
		/**
		 * Contains a null-terminated string that contains the name of a TAPI
		 * device to use with this phone-book entry, for example, "XYZ Corp
		 * 28800 External". To enumerate all available RAS-capable devices, use
		 * the RasEnumDevices function.
		 */
		public char[] szDeviceName = new char[RAS_MaxDeviceName + 1];
		/**
		 * Contains a null-terminated string that identifies the X.25 PAD type.
		 * Set this member to "" unless the entry should dial using an X.25 PAD.
		 * The szX25PadType string maps to a section name in PAD.INF.
		 */
		public char[] szX25PadType = new char[RAS_MaxPadType + 1];
		/**
		 * Contains a null-terminated string that identifies the X.25 address to
		 * which to connect . Set this member to "" unless the entry should dial
		 * using an X.25 PAD or native X.25 device.
		 */
		public char[] szX25Address = new char[RAS_MaxX25Address + 1];
		/**
		 * Contains a null-terminated string that specifies the facilities to
		 * request from the X.25 host at connection. This member is ignored if
		 * szX25Address is an empty string ("").
		 */
		public char[] szX25Facilities = new char[RAS_MaxFacilities + 1];
		/**
		 * Contains a null-terminated string that specifies additional
		 * connection information supplied to the X.25 host at connection. This
		 * member is ignored if szX25Address is an empty string ("").
		 */
		...
		/**
		 * Specifies the number of multilink subentries associated with this
		 * entry. When calling RasSetEntryProperties, set this member to zero.
		 * To add subentries to a phone-book entry, use the
		 * RasSetSubEntryProperties function.
		 */
		public int dwSubEntries;
		/**
		 * Specifies whether RAS should dial all of this entry's multilink
		 * subentries when the entry is first connected. This member can be one
		 * of the following values.
		 */
		public int dwDialMode;
		/**
		 * Specifies a percent of the total bandwidth available from the
		 * currently connected subentries. RAS dials an additional subentry when
		 * the total bandwidth used exceeds dwDialExtraPercent percent of the
		 * available bandwidth for at least dwDialExtraSampleSeconds seconds.
		 */
		public int dwDialExtraPercent;
		/**
		 * Specifies the number of seconds that current bandwidth usage must
		 * exceed the threshold specified by dwDialExtraPercent before RAS dials
		 * an additional subentry.
		 */
		public int dwDialExtraSampleSeconds;
		/**
		 * Specifies a percent of the total bandwidth available from the
		 * currently connected subentries. RAS terminates (hangs up) an existing
		 * subentry connection when total bandwidth used is less than
		...
		/**
		 * Specifies the number of seconds that current bandwidth usage must be
		 * less than the threshold specified by dwHangUpExtraPercent before RAS
		 * terminates an existing subentry connection.
		 */
		public int dwHangUpExtraSampleSeconds;
		/**
		 * Specifies the number of seconds after which the connection is
		 * terminated due to inactivity. Note that unless the idle time out is
		 * disabled, the entire connection is terminated if the connection is
		 * idle for the specified interval. This member can specify a number of
		 * seconds, or one of the following values.
		 */
		public int dwIdleDisconnectSeconds;
		/**
		 * The type of phone-book entry. This member can be one of the following
		 * types.
		 */
		public int dwType;
		/**
		 * The type of encryption to use with the connection. The encryption is
		 * either provided by IPSec (for L2TP/IPSec connections) or by Microsoft
		 * Point-to-Point Encryption (MPPE). This member can be one of the
		 * following values.
		 */
		public int dwEncryptionType;
		/**
		 * This member is used for Extensible Authentication Protocol (EAP).
		 * This member contains the authentication key provided to the EAP
		 * vendor.
		 */
		public int dwCustomAuthKey;
		/**
		 * The GUID (Globally Unique Identifier) that represents this phone-book
		 * entry. This member is read-only.
		 */
		public GUID guidId;
		/**
		 * A null-terminated string that contains the full path and file name
		 * for the dynamic link library (DLL) that implements the custom-dialing
		 * functions. This DLL should export Unicode versions of functions named
		 * RasCustomDial, RasCustomHangup, RasCustomEntryDlg, and
		 * RasCustomDialDlg. These functions should have prototypes
		 * RasCustomDialFn and RasCustomHangUpFn as defined in Ras.h, and
		 * RasCustomDialDlgFn and RasCustomEntryDlgFn as defined in Rasdlg.h.
		 */
		public char[] szCustomDialDll = new char[MAX_PATH];
		/**
		 * The VPN strategy to use when dialing a VPN connection. This member
		 * can have one of the following values.
		 */
		public int dwVpnStrategy;
		/**
		 * A set of bits that specify connection options. This member is a
		 * continuation of the bits specified in the dwfOptions member. Set one
		...
		/**
		 * Pointer to a string that specifies the Domain Name Service (DNS)
		 * suffix for the connection. This string can be Unicode depending on
		 * the version of the structure you are using.
		 */
		public char[] szDnsSuffix = new char[RAS_MaxDnsSuffix];
		/**
		 * Specifies the TCP window size for all TCP sessions that run over this
		 * connection. Setting this value can increase the throughput of
		 * high-latency devices such as cellular phones.
		 */
		public int dwTcpWindowSize;
		/**
		 * Pointer to a null-terminated string that specifies the full path and
		 * file name of a phone-book (PBK) file. This phone-book file contains
		 * the entry specified by the szPrerequisiteEntry member. This member is
		 * used only for VPN connections.
		 */
		public char[] szPrerequisitePbk = new char[MAX_PATH];
		/**
		 * Pointer to a null-terminated string that specifies a phone-book
		 * entry. This entry should exist in the phone-book file specified by
		 * the szPrerequisitePbk member. The szPrerequisteEntry member specifies
		 * an entry that RAS dials prior to establishing the connection
		 * specified by this RASENTRY structure. This member is used only for
		 * VPN connections.
		 */
		...
		/**
		 * Contains the IPv6 address of the preferred DNS. A RASIPV6ADDR type is
		 * identical to a in6_addr structure.
		 */
		public RASIPV6ADDR ipv6addrDns;
		/**
		 * Contains the IPv6 address of the alternate DNS. A RASIPV6ADDR type is
		 * identical to a in6_addr structure.
		 */
		...
		/**
		 * Specifies the client's IPv6 address negotiated by the server and the
		 * client.
		 */
		 ...
		 * retransmitted without a response before the connection is considered
		 * lost. Increase this value to support connection persistence during
		 * network outages.
		 */
		...
		@Override
		protected List<String> getFieldOrder() {
			return Arrays.asList(new String[] { "dwSize", "dwfOptions", "dwCountryID", "dwCountryCode", "szAreaCode",
					"szLocalPhoneNumber", "dwAlternateOffset", "ipaddr", "ipaddrDns", "ipaddrDnsAlt", "ipaddrWins",
					"ipaddrWinsAlt", "dwFrameSize", "dwfNetProtocols", "dwFramingProtocol", "szScript", "szAutodialDll",
					"szAutodialFunc", "szDeviceType", "szDeviceName", "szX25PadType", "szX25Address", "szX25Facilities",
					"szX25UserData", "dwChannels", "dwReserved1", "dwReserved2", "dwSubEntries", "dwDialMode",
					"dwDialExtraPercent", "dwDialExtraSampleSeconds", "dwHangUpExtraPercent", "dwHangUpExtraSampleSeconds",
					"dwIdleDisconnectSeconds", "dwType", "dwEncryptionType", "dwCustomAuthKey", "guidId", "szCustomDialDll",
					"dwVpnStrategy", "dwfOptions2", "dwfOptions3", "szDnsSuffix", "dwTcpWindowSize", "szPrerequisitePbk",
					"szPrerequisiteEntry", "dwRedialCount", "dwRedialPause", "ipv6addrDns", "ipv6addrDnsAlt",
					"dwIPv4InterfaceMetric", "dwIPv6InterfaceMetric", "ipv6addr", "dwIPv6PrefixLength", "dwNetworkOutageTime", });
		}
	}
	...
	public interface RasDialFunc2 extends StdCallCallback {
		public int dialNotification(int dwCallbackId, int dwSubEntry, HANDLE hrasconn, int unMsg, int rascs, int dwError,
				int dwExtendedError);
	}
}
