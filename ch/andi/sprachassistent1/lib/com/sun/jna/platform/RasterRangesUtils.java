...
/**
 * Methods that are useful to decompose a raster into a set of rectangles. An
 * occupied pixel has two possible meanings, depending on the raster :
 * <ul>
 * <li>if the raster has an alpha layer, occupied means with alpha not null</li>
 * <li>if the raster doesn't have any alpha layer, occupied means not completely
 * black</li>
 * </ul>
 * 
...
public class RasterRangesUtils {
	 ...
	 * pixel is occupied if its alpha value is not null. In a raster without
	 * alpha layer, a pixel is occupied if it is not completely black.
	 * 
	 * @param raster
	...
	public static boolean outputOccupiedRanges(Raster raster, RangesOutput out) {
		...
		if (raster.getParent() == null && bounds.x == 0 && bounds.y == 0) {
			// No support for subraster (as obtained with
			// Image.getSubimage(...))

			DataBuffer data = raster.getDataBuffer();
			if (data.getNumBanks() == 1) {
				// There is always a single bank for all BufferedImage types,
				// except maybe TYPE_CUSTOM
				...
				if (sampleModel instanceof MultiPixelPackedSampleModel) {
					...
					if (packedSampleModel.getPixelBitStride() == 1) {
						// TYPE_BYTE_BINARY
						return outputOccupiedRangesOfBinaryPixels(((DataBufferByte) data).getData(), bounds.width, bounds.height,
								out);
					}
				} else if (sampleModel instanceof SinglePixelPackedSampleModel) {
					if (sampleModel.getDataType() == DataBuffer.TYPE_INT) {
						// TYPE_INT_ARGB, TYPE_INT_ARGB_PRE, TYPE_INT_BGR or
						// TYPE_INT_RGB
	 ...
	 * contiguous values. A pixel is considered occupied if the bitwise AND of
	 * its integer value with the provided occupationMask is not null.
	 * 
	 * @param pixels
	 ...
	 *            mask used to select which bits are used in a pixel to check
	 *            its occupied status. 0xff000000 would only take the alpha
	 *            layer into account, for instance.
	 * @param out
	 *            where to output all the contiguous ranges of non occupied
	 *            pixels
	 * @return true if the output succeeded, false otherwise
	 */
