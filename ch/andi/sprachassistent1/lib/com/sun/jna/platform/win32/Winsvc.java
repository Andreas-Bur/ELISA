 ...
 * by NT, but exposed through the Win32 API. Ported from Winsvc.h. Microsoft
 * Windows SDK 7.0A.
 * 
 * @author EugineLev
 */
public interface Winsvc {
	...
	public static class SERVICE_STATUS extends Structure {
		public static final List<String> FIELDS = createFieldsOrder("dwServiceType", "dwCurrentState", "dwControlsAccepted",
				"dwWin32ExitCode", "dwServiceSpecificExitCode", "dwCheckPoint", "dwWaitHint");

		/**
		 * dwServiceType - the type of service. This member can be one of the
		 * following values: SERVICE_KERNEL_DRIVER, SERVICE_FILE_SYSTEM_DRIVER,
		 ...
		 * context of the LocalSystem account, the following type may also be
		 * specified: SERVICE_INTERACTIVE_PROCESS
		 *
		...
		/**
		 * dwCurrentState - The current state of the service. This member can be
		 * one of the following values: SERVICE_STOPPED, SERVICE_START_PENDING,
		...
		/**
		 * dwControlsAccepted - The control codes the service accepts and
		 * processes in its handler function: SERVICE_ACCEPT_STOP,
		...
		/**
		 * dwWin32ExitCode - The error code the service uses to report an error
		 * that occurs when it is starting or stopping. To return an error code
		 * specific to the service, the service must set this value to
		 * ERROR_SERVICE_SPECIFIC_ERROR to indicate that the
		 * dwServiceSpecificExitCode member contains the error code. The service
		...
		/**
		 * dwServiceSpecificExitCode - A service-specific error code that the
		 * service returns when an error occurs while the service is starting or
		 * stopping. This value is ignored unless the dwWin32ExitCode member is
		 * set to ERROR_SERVICE_SPECIFIC_ERROR.
		 */
		public int dwServiceSpecificExitCode;

		/**
		 * dwCheckPoint - The check-point value the service increments
		...
		/**
		 * dwWaitHint - The estimated time required for a pending start, stop,
	...
	public class SERVICE_STATUS_PROCESS extends Structure {
		...
		/**
		 * This member can be one of the following values: 0, or
	 ...
	 * reporting a status of SERVICE_STOPPED to the service controller. To
	 * configure additional circumstances under which the failure actions are to
	 * be executed, see SERVICE_FAILURE_ACTIONS_FLAG.
	 */
	public class SERVICE_FAILURE_ACTIONS extends ChangeServiceConfig2Info {
		...
		/**
		 * The time after which to reset the failure count to zero if there are
		 * no failures, in seconds. Specify INFINITE to indicate that this value
		 * should never be reset.
		 */
		public int dwResetPeriod;
		/**
		 * The message to be broadcast to server users before rebooting in
		 * response to the SC_ACTION_REBOOT service controller action. If this
		 * value is NULL, the reboot message is unchanged. If the value is an
		 * empty string (""), the reboot message is deleted and no message is
		 * broadcast. This member can specify a localized string using the
		 * following format: "@[path]dllname,-strID" The string with identifier
		 * strID is loaded from dllname; the path is optional. For more
		 * information, see RegLoadMUIString. Windows Server 2003 and Windows
		 * XP: Localized strings are not supported until Windows Vista.
		 */
		public String lpRebootMsg;
		/**
		 * The command line of the process for the CreateProcess function to
		 * execute in response to the SC_ACTION_RUN_COMMAND service controller
		 * action. This process runs under the same account as the service. If
		 * this value is NULL, the command is unchanged. If the value is an
		 * empty string (""), the command is deleted and no program is run when
		 * the service fails.
		 */
		public String lpCommand;
		/**
		 * The number of elements in the lpsaActions array. If this value is 0,
		 * but lpsaActions is not NULL, the reset period and array of failure
		 * actions are deleted.
		 */
		public int cActions;
		/**
		 * A pointer to an array of SC_ACTION structures. If this value is NULL,
		 * the cActions and dwResetPeriod members are ignored.
		 */
		...
		public static final List<String> FIELDS = createFieldsOrder("dwResetPeriod", "lpRebootMsg", "lpCommand", "cActions",
				"lpsaActions");
	...
	public class SC_ACTION extends Structure {
		...
		/**
		 * The action to be performed. This member can be one of the following
		 * values from the SC_ACTION_TYPE enumeration type.
		 */
		public int type;
		/**
		 * The time to wait before performing the specified action, in
		 * milliseconds.
		 */
		public int delay;
	...
	public class SERVICE_FAILURE_ACTIONS_FLAG extends ChangeServiceConfig2Info {
		/**
		 * If this member is TRUE and the service has configured failure
		 * actions, the failure actions are queued if the service process
		 * terminates without reporting a status of SERVICE_STOPPED or if it
		 * enters the SERVICE_STOPPED state but the dwWin32ExitCode member of
		 * the SERVICE_STATUS structure is not ERROR_SUCCESS (0). If this member
		 * is FALSE and the service has configured failure actions, the failure
		 * actions are queued only if the service terminates without reporting a
		 * status of SERVICE_STOPPED. This setting is ignored unless the service
		 * has configured failure actions. For information on configuring
		 * failure actions, see ChangeServiceConfig2.
		 */
	 ...
	 * {@link com.sun.jna.platform.win32.Advapi32#QueryServiceConfig2} functions
	...
	int SERVICE_ALL_ACCESS = WinNT.STANDARD_RIGHTS_REQUIRED | SERVICE_QUERY_CONFIG | SERVICE_CHANGE_CONFIG | SERVICE_QUERY_STATUS
	...
	interface SERVICE_MAIN_FUNCTION extends StdCallLibrary.StdCallCallback {
		 ...
		 *
		 * @param dwArgc
		 *            [in] The number of arguments in the lpszArgv array.
		 * @param lpszArgv
		 *            [in] The null-terminated argument strings passed to the
		 *            service by the call to the StartService function that
		 *            started the service. If there are no arguments, this
		 *            parameter can be NULL. Otherwise, the first argument
		 *            (lpszArgv[0]) is the name of the service, followed by any
		 *            additional arguments (lpszArgv[1] through
		 *            lpszArgv[dwArgc-1]).
		 *
		 *            <p>
		 * 			If the user starts a manual service using the Services
		 *            snap-in from the Control Panel, the strings for the
		 *            lpszArgv parameter come from the properties dialog box for
		 *            the service (from the Services snap-in, right-click the
		 *            service entry, click Properties, and enter the parameters
		 *            in Start parameters.)
		 */
	...
	interface Handler extends StdCallLibrary.StdCallCallback {
		...
		/**
		 * @param fdwControl
		 *            [in] The control code. This parameter can be one of the
		 *            following values.
		 *            <table>
		 *            <tr>
		 *            <th>Control code</th>
		 ...
		 *            </tr>
		 *            <tr>
		 *            <td>SERVICE_CONTROL_CONTINUE<br>
		 * 			0x00000003</td>
		 *            <td>Notifies a paused service that it should resume.</td>
		 *            </tr>
		 *            <tr>
		 *            <td>SERVICE_CONTROL_INTERROGATE<br>
		 * 			0x00000004</td>
		 *            <td>Notifies a service that it should report its current
		 *            status information to the service control manager.<br>
		 * 			The handler should simply return NO_ERROR; the SCM is
		 *            aware of the current state of the service.</td>
		 *            </tr>
		 *            <tr>
		 *            <td>SERVICE_CONTROL_NETBINDADD<br>
		 * 			0x00000007</td>
		 *            <td>Notifies a network service that there is a new
		 *            component for binding. The service should bind to the new
		 *            component.<br>
		 * 			Applications should use Plug and Play functionality
		 *            instead.</td>
		 *            </tr>
		 *            <tr>
		 *            <td>SERVICE_CONTROL_NETBINDDISABLE<br>
		 * 			0x0000000A</td>
		 *            <td>Notifies a network service that one of its bindings
		 *            has been disabled. The service should reread its binding
		 *            information and remove the binding.<br>
		 * 			Applications should use Plug and Play functionality
		 *            instead.</td>
		 *            </tr>
		 *            <tr>
		 *            <td>SERVICE_CONTROL_NETBINDENABLE<br>
		 * 			0x00000009</td>
		 *            <td>Notifies a network service that a disabled binding has
		 *            been enabled. The service should reread its binding
		 *            information and add the new binding.<br>
		 * 			Applications should use Plug and Play functionality
		 *            instead.</td>
		 *            </tr>
		 *            <tr>
		 *            <td>SERVICE_CONTROL_NETBINDREMOVE<br>
		 * 			0x00000008</td>
		 *            <td>Notifies a network service that a component for
		 *            binding has been removed. The service should reread its
		 *            binding information and unbind from the removed
		 *            component.<br>
		 * 			Applications should use Plug and Play functionality
		 *            instead.</td>
		 *            </tr>
		 *            <tr>
		 *            <td>SERVICE_CONTROL_PARAMCHANGE<br>
		 * 			0x00000006</td>
		 *            <td>Notifies a service that its startup parameters have
		 *            changed. The service should reread its startup
		 *            parameters.</td>
		 *            </tr>
		 *            <tr>
		 *            <td>SERVICE_CONTROL_PAUSE<br>
		 * 			0x00000002</td>
		 *            <td>Notifies a service that it should pause.</td>
		 *            </tr>
		 *            <tr>
		 *            <td>SERVICE_CONTROL_SHUTDOWN<br>
		 * 			0x00000005</td>
		 *            <td>Notifies a service that the system is shutting down so
		 *            the service can perform cleanup tasks.<br>
		 * 			If a service accepts this control code, it must stop after
		 *            it performs its cleanup tasks and return NO_ERROR. After
		 *            the SCM sends this control code, it will not send other
		 *            control codes to the service.</td>
		 *            </tr>
		 *            <tr>
		 *            <td>SERVICE_CONTROL_STOP<br>
		 * 			0x00000001</td>
		 *            <td>Notifies a service that it should stop.<br>
		 * 			If a service accepts this control code, it must stop upon
		 *            receipt and return NO_ERROR. After the SCM sends this
		 *            control code, it does not send other control codes.<br>
		 * 			Windows XP: If the service returns NO_ERROR and continues
		 *            to run, it continues to receive control codes. This
		 *            behavior changed starting with Windows Server 2003 and
		 *            Windows XP with SP2.</td>
		 *            </tr>
		 *            </table>
		 * 
		 *            <p>
		 * 			This parameter can also be a user-defined control code, as
		 ...
		 *            </tr>
		 *            <tr>
		 *            <td>Range 128 to 255.</td>
		 *            <td>The service defines the action associated with the
		 *            control code.</td>
		 *            </tr>
		 *            </table>
		 */
	...
	interface HandlerEx extends StdCallLibrary.StdCallCallback {
		 ...
		 *            </tr>
		 *            <tr>
		 *            <td>SERVICE_CONTROL_PRESHUTDOWN<br>
		 * 			0x0000000F</td>
		 *            <td>Notifies a service that the system will be shutting
		 *            down. Services that need additional time to perform
		 *            cleanup tasks beyond the tight time restriction at system
		 *            shutdown can use this notification. The service control
		 *            manager sends this notification to applications that have
		 *            registered for it before sending a
		 *            SERVICE_CONTROL_SHUTDOWN notification to applications that
		 *            have registered for that notification.<br>
		 ...
		 *            SERVICE_PRESHUTDOWN_INFO expires. Because this affects the
		 *            user experience, services should use this feature only if
		 *            it is absolutely necessary to avoid data loss or
		 *            significant recovery time at the next system start.<br>
		 ...
		 * 
		 *            <p>
		 * 			This parameter can also be one of the following extended
		 *            control codes. Note that these control codes are not
		 *            supported by the {@link Handler} function.
		 *            </p>
		 * 
		 *            <table>
		 *            <tr>
		 ...
		 *            </tr>
		 *            <tr>
		 *            <td>SERVICE_CONTROL_DEVICEEVENT<br>
		 * 			0x0000000B</td>
		 *            <td>Notifies a service of device events. (The service must
		 *            have registered to receive these notifications using the
		 *            RegisterDeviceNotification function.) The dwEventType and
		 *            lpEventData parameters contain additional
		 *            information.</td>
		 *            </tr>
		 *            <tr>
		 *            <td>SERVICE_CONTROL_HARDWAREPROFILECHANGE<br>
		 * 			0x0000000C</td>
		 *            <td>Notifies a service that the computer's hardware
		 *            profile has changed. The dwEventType parameter contains
		 *            additional information.</td>
		 *            </tr>
		 *            <tr>
		 *            <td>SERVICE_CONTROL_POWEREVENT<br>
		 * 			0x0000000D</td>
		 *            <td>Notifies a service of system power events. The
		 *            dwEventType parameter contains additional information. If
		 *            dwEventType is PBT_POWERSETTINGCHANGE, the lpEventData
		 *            parameter also contains additional information.</td>
		 *            </tr>
		 *            <tr>
		 *            <td>SERVICE_CONTROL_SESSIONCHANGE<br>
		 * 			0x0000000E</td>
		 *            <td>Notifies a service of session change events. Note that
		 *            a service will only be notified of a user logon if it is
		 *            fully loaded before the logon attempt is made. The
		 *            dwEventType and lpEventData parameters contain additional
		 *            information.</td>
		 *            </tr>
		 *            <tr>
		 *            <td>SERVICE_CONTROL_TIMECHANGE<br>
		 * 			0x00000010</td>
		 *            <td>Notifies a service that the system time has changed.
		 *            The lpEventData parameter contains additional information.
		 *            The dwEventType parameter is not used.<br>
		 * 			Windows Server 2008, Windows Vista, Windows Server 2003
		 *            and Windows XP: This control code is not supported.</td>
		 *            </tr>
		 *            <tr>
		 *            <td>SERVICE_CONTROL_TRIGGEREVENT<br>
		 * 			0x00000020</td>
		 *            <td>Notifies a service registered for a service trigger
		 *            event that the event has occurred.<br>
		 * 			Windows Server 2008, Windows Vista, Windows Server 2003
		 *            and Windows XP: This control code is not supported.</td>
		 *            </tr>
		 *            <tr>
		 *            <td>SERVICE_CONTROL_USERMODEREBOOT<br>
		 * 			0x00000040</td>
		 *            <td>Notifies a service that the user has initiated a
		 *            reboot.<br>
		 * 			Windows Server 2008 R2, Windows 7, Windows Server 2008,
		 *            Windows Vista, Windows Server 2003 and Windows XP: This
		 *            control code is not supported.</td>
		 *            </tr>
		 *            </table>
		 * 
		 *            <p>
		 ...
		 * 
		 * @param dwEventType
		 *            The type of event that has occurred. This parameter is
		 *            used if dwControl is SERVICE_CONTROL_DEVICEEVENT,
		 *            SERVICE_CONTROL_HARDWAREPROFILECHANGE,
		 *            SERVICE_CONTROL_POWEREVENT, or
		 *            SERVICE_CONTROL_SESSIONCHANGE. Otherwise, it is zero.
		 *
		 *            <p>
		 * 			If dwControl is SERVICE_CONTROL_DEVICEEVENT, this
		 ...
		 *
		 *            <p>
		 * 			If dwControl is SERVICE_CONTROL_HARDWAREPROFILECHANGE,
		 ...
		 *
		 *            <p>
		 * 			If dwControl is SERVICE_CONTROL_POWEREVENT, this parameter
		 *            can be one of the values specified in the wParam parameter
		 *            of the WM_POWERBROADCAST message.
		 *            </p>
		 *
		 *            <p>
		 * 			If dwControl is SERVICE_CONTROL_SESSIONCHANGE, this
		 *            parameter can be one of the values specified in the wParam
		 *            parameter of the WM_WTSSESSION_CHANGE message.
		 *            </p>
		 *
		 * @param lpEventData
		 *            [in] Additional device information, if required. The
		 *            format of this data depends on the value of the dwControl
		 *            and dwEventType parameters.
		 *
		 *            <p>
		 * 			If dwControl is SERVICE_CONTROL_DEVICEEVENT, this data
		 *            corresponds to the lParam parameter that applications
		 *            receive as part of a WM_DEVICECHANGE message.
		 *            </p>
		 *
		 *            <p>
		 * 			If dwControl is SERVICE_CONTROL_POWEREVENT and dwEventType
		 *            is PBT_POWERSETTINGCHANGE, this data is a pointer to a
		 *            POWERBROADCAST_SETTING structure.
		 *            </p>
		 *
		 *            <p>
		 * 			If dwControl is SERVICE_CONTROL_SESSIONCHANGE, this
		 *            parameter is a pointer to a WTSSESSION_NOTIFICATION
		 *            structure.
		 *            </p>
		 *
		 *            <p>
		 * 			If dwControl is SERVICE_CONTROL_TIMECHANGE, this data is a
		 *            pointer to a SERVICE_TIMECHANGE_INFO structure.
		 *            </p>
		 * 
		 * @param lpContext
		 *            [in] User-defined data passed from
		 *            {@link com.sun.jna.platform.win32.Advapi32#RegisterServiceCtrlHandlerEx}.
		 *            When multiple services share a process, the lpContext
		 *            parameter can help identify the service.
		 * 
		 * @return The return value for this function depends on the control
		 *         code received.
		 *
		 *         <p>
		 * 		The following list identifies the rules for this return
		 *         value:
		 *         </p>
		 *
		 *         <ul>
		 *         <li>In general, if your service does not handle the control,
		 *         return ERROR_CALL_NOT_IMPLEMENTED. However, your service
		 *         should return NO_ERROR for SERVICE_CONTROL_INTERROGATE even
		 *         if your service does not handle it.</li>
		 *         <li>If your service handles SERVICE_CONTROL_STOP or
		 *         SERVICE_CONTROL_SHUTDOWN, return NO_ERROR.</li>
		 *         <li>If your service handles SERVICE_CONTROL_DEVICEEVENT,
		 *         return NO_ERROR to grant the request and an error code to
		 *         deny the request.</li>
		 *         <li>If your service handles
		 *         SERVICE_CONTROL_HARDWAREPROFILECHANGE, return NO_ERROR to
		 *         grant the request and an error code to deny the request.</li>
		 *         <li>If your service handles SERVICE_CONTROL_POWEREVENT,
		 *         return NO_ERROR to grant the request and an error code to
		 *         deny the request.</li>
