 ...
 * and figures out how many gap insertion errors are there. The hypothesis
 * transcript file should contain timestamps for when each word was entered and
 * exited.
 * <p>
 * The gap insertion detection algorithm works as follows. It takes each
 * hypothesized word individually and see whether it falls into a non-speech
 * region in the reference transcript. If it does, that hypothesized word is
 * counted as a gap insertion.
 */
public class GapInsertionDetector {
	...
	/**
	 * Create a gap insertion detector to detect gap insertions using the given
	 * reference file and hypothesis file.
	 *
	 * @param referenceFile
	 *            the file of references
	 * @param hypothesisFile
	 *            the file of hypotheses
	 * @param showGapInsertions
			...
			if (word != null) {
				...
				if (reference != null) {
					if (reference.isSilenceGap()) {
						...
						while (reference.getEndTime() < word.getEndTime()) {
							reference = referenceFile.nextUtterance();
							if (reference == null || reference.isSilenceGap()) {
								hasGapError = true;
								break;
							}
						}
				...
				if (hasGapError) {
					...
					if (showGapInsertions) {
						log.append("GapInsError: Utterance: ").append(hypothesisFile.getUtteranceCount()).append(" Word: ")
								.append(word.getText()).append(" (").append(word.getStartTime()).append(',')
								.append(word.getEndTime()).append("). ");
						if (reference != null) {
							assert reference.isSilenceGap();
							log.append("Reference: <sil> (").append(reference.getStartTime()).append(',')
									.append(reference.getEndTime()).append(')');
						}
						log.append('\n');
					}
				}
	...
	/*
	 * A command line program for detecting gap insertion errors. To run this
	 * program, type: <code> java GapInsertionDetector {propsFile}
	 * {referenceFile} {hypothesisFile} </code> The propsFile need to have only
	 * one property: <code>
...
class ReferenceFile {
	...
	/**
	 * Returns the next available ReferenceUtterance. This method skips all the
	 * silence gaps.
	 *
	 * @return the next available ReferenceUtterance, or null if the end of file
	 *         has been reached.
	 */
...
class ReferenceUtterance {
	 ...
	 *
	 * @param line
	 *            the line of reference, in the format: [test_name] [category]
	 *            [speaker_name|"inter_segment_gap"] [start_time] [end_time]
...
class HypothesisFile {
	 ...
	 *
	 * @return the next available hypothesis utterance, or null if the end of
...
class HypothesisUtterance {
	...
	/**
	 * Creates a hypothesis utterance from a line of input describing the
	 * hypothesis.
	 */
...
/**
 * A word in the hypothesis, containing information about when the word started
 * and ended.
 */
class HypothesisWord {
	 ...
	 *
	 * @param text
	 *            the text of the hypothesized word
	 * @param startTime
	 *            the starting time of the word
	 * @param endTime
	 *            the ending time of the word
	 */
