...
/**
 * Keeps track of execution times. This class provides methods that can be used
 * for timing processes. The process to be timed should be bracketed by calls to
 * timer.start() and timer.stop(). Repeated operations can be timed more than
 * once. The timer will report the minimum, maximum, average and last time
 * executed for all start/stop pairs when the timer.dump is called.
 * <p>
 * Timer instances can be obtained from a global cache implemented in
 * {@code TimerPool}.
 *
 * @see TimerPool
 */
public class Timer {
	...
	private void showTimesShort(Logger logger) {
		...
		if (notReliable) {
			...
			logger.info(Utilities.pad(name, 20) + ' ' + Utilities.pad(String.valueOf(count), 8) + fmtTime(curTime)
					+ fmtTime(minTime) + fmtTime(maxTime) + fmtTime(avgTime) + fmtTime(sum / 1000.0));
		}
	}
}
