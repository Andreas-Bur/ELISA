 ...
 * it is important to specify it correctly in the configuration of the
 * <a href="../../frontend/util/StreamDataSource.html">StreamDataSource</a>.
 * Note that in the ideal situation, the audio format of the data should be
 * passed into the StreamDataSource, so that no extra configuration is needed.
 ...
 * <a href="../../util/props/doc-files/ConfigurationManagement.html"> Sphinx-4
...
public class BatchModeRecognizer implements Configurable {
	...
	/**
	 * The property for the total number of batch jobs the decoding run is being
	 * divided into.
	 * <p>
	 * The BatchDecoder supports running a subset of a batch. This allows a test
	 * to be distributed among several machines.
	 */
	@S4Integer(defaultValue = 1)
	public final static String PROP_TOTAL_BATCHES = "totalBatches";

	/**
	 * The property that defines whether or not the decoder should use the
	 * pooled batch manager
	 */
	...
	public BatchModeRecognizer(Recognizer recognizer, List<DataProcessor> inputDataProcessors, int skip, int utteranceId,
			int whichBatch, int totalBatches, boolean usePooledBatchManager) {
	 ...
	 * 
	 * @param batchFile
	 *            file with list of files to decode
	 * @throws IOException
	 *             if IO went wrong
	 */
	 ...
	 *
	 * @param filename
	 *            the filename to set the input stream to
	 * @throws IOException
	 *             if an error occurs
	 */
		...
		ci.add("set", new CommandInterface() {
			public String execute(CommandInterpreter ci, String[] args) {
				if (args.length != 4) {
					...
					// System.err.println("tried to configure the CM with " +
					// args );
	...
	public static void main(String[] argv) {
		...
		try {
			 ...
			 * ioe.printStackTrace(); } catch (InstantiationException e) {
