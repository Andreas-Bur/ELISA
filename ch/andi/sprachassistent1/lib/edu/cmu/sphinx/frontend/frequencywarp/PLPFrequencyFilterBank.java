...
/**
 * Filters an input power spectrum through a PLP filterbank. The filters in the
 * filterbank are placed in the frequency axis so as to mimic the critical band,
 * representing different perceptual effect at different frequency bands. The
 * filter outputs are also scaled for equal loudness preemphasis. The filter
 * shapes are defined by the {@link PLPFilter} class. Like the
 * {@link MelFrequencyFilterBank2}, this filter bank has characteristics defined
 * by the {@link #PROP_NUMBER_FILTERS number of filters}, the
 * {@link #PROP_MIN_FREQ minimum frequency}, and the {@link #PROP_MAX_FREQ
 * maximum frequency}. Unlike the {@link MelFrequencyFilterBank2}, the minimum
 * and maximum frequencies here refer to the <b>center</b> frequencies of the
 * filters located at the leftmost and rightmost positions, and not to the
 * edges. Therefore, this filter bank spans a frequency range that goes beyond
 * the limits suggested by the minimum and maximum frequencies.
 *
...
public class PLPFrequencyFilterBank extends BaseDataProcessor {
	...
	/**
	 * The property for the center frequency of the lowest filter in the
	 * filterbank.
	 */
	...
	/**
	 * The property for the center frequency of the highest filter in the
	 * filterbank.
	 */
	...
	/**
	 * Build a PLP filterbank with the parameters given. The center frequencies
	 * of the PLP filters will be uniformly spaced between the minimum and
	 * maximum analysis frequencies on the Bark scale. on the Bark scale.
	 *
	...
	private void buildCriticalBandFilterbank() throws IllegalArgumentException {
		...
		/**
		 * Find center frequencies of filters in the Bark scale translate to
		 * linear frequency and create PLP filters with these center
		 * frequencies.
		 *
		 * Note that minFreq and maxFreq specify the CENTER FREQUENCIES of the
		 * lowest and highest PLP filters
		 */
	...
	/**
	 * This function return the equal loudness preemphasis factor at any
	 * frequency. The preemphasis function is given by
	 * <p>
	 * E(w) = f^4 / (f^2 + 1.6e5) ^ 2 * (f^2 + 1.44e6) / (f^2 + 9.61e6)
	 * <p>
	 * This is more modern one from HTK, for some reason it's preferred over old
	 * variant, and it doesn't require conversion to radians
	 * <p>
	...
	/**
	 * Create an array of equal loudness preemphasis scaling terms for all the
	 * filters
	 */
	...
	private DoubleData process(DoubleData input) throws IllegalArgumentException {
		...
		if (criticalBandFilter == null || sampleRate != input.getSampleRate()) {
			...
			throw new IllegalArgumentException("Window size is incorrect: in.length == " + in.length + ", numberFftPoints == "
					+ ((numberFftPoints >> 1) + 1));
		}
	...
	/**
	 * Reads the next Data object, which is the power spectrum of an audio input
	 * frame. However, it can also be other Data objects like a Signal, which is
	 * returned unmodified.
	 *
