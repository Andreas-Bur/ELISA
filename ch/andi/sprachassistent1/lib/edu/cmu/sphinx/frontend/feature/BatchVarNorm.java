...
/**
 * Applies cepstral variance normalization (CVN), so that each coefficient will
 * have unit variance. You need to put this element after the means normalizer
				...
				if (input instanceof DoubleData) {
					...
					if (variances == null) {
						...
						if (variances.length != cepstrumData.length) {
							throw new Error("Inconsistent cepstrum lengths: sums: " + variances.length + ", cepstrum: "
									+ cepstrumData.length);
						}
					}
