...
/**
 * Concatenates a list raw headerless audio files as one continuous audio
 * stream. A {@link edu.cmu.sphinx.frontend.DataStartSignal DataStartSignal}
 ...
 * Optionally, silence can be added in-between the audio files by setting the
 * property:
 * 
 * <pre>
 ...
 * </pre>
 * 
 * to a audio file for silence. By default, no silence is added. Moreover, one
 * can also specify how many files to skip for every file read.
 * <p>
 * You can also specify the name of a transcript file to write the transcription
 * to. The transcription will be written in HUB-4 style. A sample HUB-4
 ...
 * </pre>
 * 
 * The ConcatFileDataSource will produce such a transcript if the name of the
 * file to write to is supplied in the constructor. This transcript file will be
 * used in detected gap insertion errors, because it accurately describes the
 * "correct" sequence of speech and silences in the concatenated version of the
 * audio files.
 */
public class ConcatFileDataSource extends StreamDataSource implements ReferenceSource {
	...
	/**
	 * The property that specifies the number of files to skip for every file
	 * read.
	 */
	...
	/**
	 * The property that specifies the total number of files to read. The
	 * default value should be no limit.
	 */
	@S4Integer(defaultValue = -1)
	public static final String PROP_TOTAL_FILES = "totalFiles";

	/**
	 * The property that specifies the silence audio file, if any. If this
	 * property is null, then no silences are added in between files.
	 */
	@S4String
	...
	/**
	 * The property that specifies the maximum number of times the silence file
	 * is added between files. If PROP_ADD_RANDOM_SILENCE is set to true, the
	 * number of times the silence file is added is between 1 and this value. If
	 * PROP_ADD_RANDOM_SILENCE is set to false, this value will be the number of
	 * times the silence file is added. So if PROP_MAX_SILENCE is set to 3, then
	 * the silence file will be added three times between files.
	 */
	@S4Integer(defaultValue = 3)
	public static final String PROP_MAX_SILENCE = "maxSilence";

	/**
	 * The property that specifies the name of the transcript file. If this
	 * property is set, a transcript file will be created. No transcript file
	 * will be created if this property is not set.
	 */
	@S4String
	...
	public ConcatFileDataSource(int sampleRate, int bytesPerRead, int bitsPerSample, boolean bigEndian, boolean signedData,
			boolean addRandomSilence, int maxSilence, int skip, String silenceFileName, int startFile, int totalFiles,
			String transcriptFile, String batchFile) {
	...
	public void initialize() {
		...
		try {
			...
			setInputStream(new SequenceInputStream(new InputStreamEnumeration(batchFile, startFile, totalFiles)));
	...
	/**
	 * Returns the audio time in seconds represented by the given number of
	 * bytes.
	 *
	 * @param bytes
	 *            the number of bytes
	 ...
	 * here is to turn the list of audio files into an Enumeration, and then fed
	 * it to a SequenceInputStream, giving the illusion that the audio files are
	...
	class InputStreamEnumeration implements Enumeration<InputStream> {
		...
		/**
		 * Returns the name of next audio file, taking into account file
		 * skipping and the adding of silence.
		 *
