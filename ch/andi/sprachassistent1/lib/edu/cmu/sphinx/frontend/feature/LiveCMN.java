 ...
 * initial CMN estimation, for that some amount of frames are read initially and
 * cmn estimation is calculated from them.
 * <p>
 * The properties that affect this processor are defined by the fields
 * {@link #PROP_INITIAL_CMN_WINDOW}, {@link #PROP_CMN_WINDOW}, and
 * {@link #PROP_CMN_SHIFT_WINDOW}. Please follow the link "Constant Field
 * Values" below to see the actual name of the Sphinx properties.
 * <p>
...
public class LiveCMN extends BaseDataProcessor {
	...
	private void normalize(Data data) {
		...
		if (cepstrum.length != sum.length) {
			throw new Error("Data length (" + cepstrum.length + ") not equal sum array length (" + sum.length + ')');
		}
