 ...
 * that are actually returned, defaulting to 13. When the input is mel-spectrum,
 * the vector returned is the MFCC (Mel-Frequency Cepstral Coefficient) vector,
...
public class DiscreteCosineTransform extends BaseDataProcessor {
	...
	private DoubleData process(DoubleData input) throws IllegalArgumentException {
		...
		if (melcosine == null) {
			...
			throw new IllegalArgumentException("MelSpectrum size is incorrect: melspectrum.length == " + melspectrum.length
					+ ", numberMelFilters == " + numberMelFilters);
		}
