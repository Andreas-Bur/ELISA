 ...
 * a DiscreteCosineTransform. A typical denoised MFCC Cepstrum will use a
...
public class AutoCepstrum extends BaseDataProcessor {
	 ...
	 * pipeline. If "-transform dct" is specified in the feat.params file of the
	 * target acoustic model, then the current DCT component is inserted in the
	 * pipeline.
	 */
	 ...
	 * lifter component is inserted in the pipeline only if "-lifter
	 * &lt;lifterValue&gt;" is specified in the feat.params file of the target
	 * acoustic model.
	 */
	...
	private void initDataProcessors() {
		try {
			...
			if ((featParams.get("-transform") != null) && (featParams.get("-transform").equals("dct"))) {
				dct = new DiscreteCosineTransform2(numFilter, DiscreteCosineTransform.class.getField("PROP_CEPSTRUM_LENGTH")
						.getAnnotation(S4Integer.class).defaultValue());
			} else if ((featParams.get("-transform") != null) && (featParams.get("-transform").equals("kaldi"))) {
				dct = new KaldiDiscreteCosineTransform(numFilter, DiscreteCosineTransform.class.getField("PROP_CEPSTRUM_LENGTH")
						.getAnnotation(S4Integer.class).defaultValue());
			} else {
				dct = new DiscreteCosineTransform(numFilter, DiscreteCosineTransform.class.getField("PROP_CEPSTRUM_LENGTH")
						.getAnnotation(S4Integer.class).defaultValue());
			}
