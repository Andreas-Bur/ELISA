 ...
 * <li>inserting SPEECH_START and SPEECH_END signals around speech and removing
 * non-speech regions
 * </ol>
 * <p>
 * The first step, classification of audio into speech and non-speech, uses Bent
 * Schmidt Nielsen's algorithm. Each time audio comes in, the average signal
 * level and the background noise level are updated, using the signal level of
 * the current audio. If the average signal level is greater than the background
 * noise level by a certain threshold value (configurable), then the current
 * audio is marked as speech. Otherwise, it is marked as non-speech.
 * <p>
 * The second step of this endpointer is documented in the class
 * {@link SpeechMarker SpeechMarker}
 *
 * @see SpeechMarker
 */
public class SpeechClassifier extends AbstractVoiceActivityDetector {
	...
	/**
	 * The property specifying the minimum signal level used to update the
	...
	/**
	 * The property specifying the threshold. If the current signal level is
	 * greater than the background level by this threshold, then the current
	 * signal is marked as speech. Therefore, a lower threshold will make the
	 * endpointer more sensitive, that is, mark more audio as speech. A higher
	 * threshold will make the endpointer less sensitive, that is, mark less
	 * audio as speech.
	 */
	...
	/**
	 * Returns the logarithm base 10 of the root mean square of the given
	 * samples.
	 *
	 * @param samples
	 *            the samples
	...
	/**
	 * Classifies the given audio frame as speech or not, and updates the
	...
	/**
	 * Collects the statistics to provide information about signal to noise
	 * ratio in channel
	 * 
	 * @param isSpeech
	 *            if the current frame is classified as speech
	 */
	...
	/**
	 * Method that returns if current returned frame contains speech. It could
	 * be used by noise filter for example to adjust noise spectrum estimation.
	 * 
	 ...
	 * recognition. The audio is counted noisy if signal to noise ratio is less
	 * then 20dB.
	 * 
