...
/**
 * Slices up a Data object into a number of overlapping windows (usually
 * referred to as "frames" in the speech world). In order to minimize the signal
 * discontinuities at the boundaries of each frame, we multiply each frame with
 * a raised cosine windowing function. Moreover, the system uses overlapping
 * windows to capture information that may occur at the window boundaries. These
 * events would not be well represented if the windows were juxtaposed.
 * <p>
 * The number of resulting windows depends on the {@link #PROP_WINDOW_SIZE_MS
 * window size} and the {@link #PROP_WINDOW_SHIFT_MS window shift} (commonly
 * known as frame shift in speech world). Figure 1 shows the relationship
 * between the original data stream, the window size, the window shift, and the
 * windows returned.
 * <p>
 * <img alt="Framing" src="doc-files/framing.jpg"> <br>
 * <b>Figure 1: Relationship between original data, window size, window shift,
 * and the windows returned.</b>
 * <p>
 * The raised cosine windowing function will be applied to each such window.
 * Since the {@link #getData()} method returns a window, and multiple windows
 * are created for each Data object, this is a 1-to-many processor. Also note
 * that the returned windows should have the same number of data points as the
 * windowing function.
 * <p>
 * The applied windowing function, <i>W(n)</i>, of length <i>N</i> (the window
 ...
 * </pre>
 * 
 * where <b>a</b> is commonly known as the "alpha" value. This variable can be
 * set by the user using the property defined by {@link #PROP_ALPHA}. Please
 * follow the links to the see the constant field values. Some values of alpha
 * receive special names, since they are used so often. A value of 0.46 for the
 * alpha results in a window named Hamming window. A value of 0.5 results in the
 * Hanning window. And a value of 0 results in the Rectangular window. The
 * default for this system is the Hamming window, with alpha 0.46 !). Figure 2
 * below shows the Hamming window function (a = 0.46), using our default window
 * size of 25.625 ms and assuming a sample rate of 16kHz, thus yielding 410
 * samples per window.
 * <p>
 * <img alt="Hamming window" src="doc-files/hamming-window.gif"> <br>
 * <b>Figure 2: The Hamming window function.</b>
 *
 * @see Data
 */
public class RaisedCosineWindower extends BaseDataProcessor {
	...
	/**
	 * The property for window shift in milliseconds, which has a default value
	 * of 10F.
	 */
	...
	/**
	 * The property for the alpha value of the Window, which is the value for
	 * the RaisedCosineWindow.
	 */
		...
		if (outputQueue.isEmpty()) {
			...
			if (input != null) {
				if (input instanceof DoubleData) {
					...
					// should not be necessary if all DataProcessor would
					// forward Signals. Unfortunately this
					...
					if (input instanceof DataStartSignal) {
						...
						// attach the frame-length and the shift-length to the
						// start-signal to allow
	...
	/**
	 * Applies the Windowing to the given Data. The resulting windows are cached
	 * in the outputQueue.
	 *
	 * @param input
	...
	/**
	 * What happens when an DataEndSignal is received. Basically pads up to a
	 * window of the overflow buffer with zeros, and then apply the Hamming
	 * window to it. Checks if buffer has data.
	 */
	...
	/**
	 * Applies the Hamming window to the given double array. The windows are
	 * added to the output queue. Returns the index of the first array element
	 * of next window that is not produced because of insufficient data.
	 *
	 * @param in
	 *            the audio data to apply window and the Hamming window
	 * @param length
	 *            the number of elements in the array to apply the
	 *            RaisedCosineWindow
	...
	private static int getWindowCount(int arraySize, int windowSize, int windowShift) {
		if (arraySize < windowSize) {
			...
			int windowCount = 1;
			for (int windowEnd = windowSize; windowEnd + windowShift <= arraySize; windowEnd += windowShift) {
				windowCount++;
			}
			return windowCount;
		}
	...
	/**
	 * Returns the shift size used to window the incoming speech signal. This
	 * value might be used by other components to determine the time resolution
	 * of feature vectors.
	 * 
	...
	/**
	 * Rounds a given sample-number to the number of samples will be processed
	 * by this instance including the padding samples at the end..
	 * 
	 * @param samples
	 *            samples to round to
...
class DoubleBuffer {
	 ...
	 *            the array to copy from
	 * @param srcPos
	 *            where in the source array to start from
	 * @param length
	...
	/**
	 * If there are less than windowSize elements in this DoubleBuffer, pad the
	 * up to windowSize elements with zero.
	 *
	 * @param windowSize
	 *            the window size
	 */
