...
/**
 * FrontEnd is a wrapper class for the chain of front end processors. It
 * provides methods for manipulating and navigating the processors.
 * <p>
 * The front end is modeled as a series of data processors, each of which
 * performs a specific signal processing function. For example, a processor
 * performs Fast-Fourier Transform (FFT) on input data, another processor
 * performs high-pass filtering. Figure 1 below describes how the front end
 * looks like: <img alt="Frontend" src="doc-files/frontend.jpg"> <br>
 * <b>Figure 1: The Sphinx4 front end.</b>
 * <p>
 * Each such data processor implements the
 * {@link edu.cmu.sphinx.frontend.DataProcessor} interface. Objects that
 * implements the {@link edu.cmu.sphinx.frontend.Data} interface enters and
 * exits the front end, and go between the processors in the front end. The
 * input data to the front end is typically audio data, but this front end
 * allows any input type. Similarly, the output data is typically features, but
 * this front end allows any output type. You can configure the front end to
 * accept any input type and return any output type. We will describe the
 * configuration of the front end in more detail below.
 * <p>
 * <b>The Pull Model of the Front End</b>
 * <p>
 * The front end uses a pull model. To obtain output from the front end, one
 * would call the method:
 * <p>
 * <code> FrontEnd frontend = ... // see how to obtain the front end below <br>Data output = frontend.getData();
 * </code>
 * <p>
 * Calling {@link #getData() getData} on the front end would in turn call the
 * getData() method on the last DataProcessor, which in turn calls the getData()
 * method on the second last DataProcessor, and so on, until the getData()
 * method on the first DataProcessor is called, which reads Data objects from
 * the input. The input to the front end is actually another DataProcessor, and
 * is usually (though not necessarily) part of the front end and is not shown in
 * the figure above. If you want to maintain some control of the input
 * DataProcessor, you can create it separately, and use the
 ...
 * DataProcessor will be prepended to the existing chain of DataProcessors. One
 * common input DataProcessor is the
 * {@link edu.cmu.sphinx.frontend.util.Microphone}, which implements the
 ...
 * {@link edu.cmu.sphinx.frontend.util.StreamDataSource}. It turns a Java
 * {@link java.io.InputStream} into Data objects. It is usually used in batch
 * mode decoding.
 * <p>
 * <b>Configuring the front end</b>
 * <p>
 * The front end must be configured through the Sphinx properties file. For
 * details about configuring the front end, refer to the document
 ...
 * (called a 'pipeline') in Sphinx-4, insert the following lines in the Sphinx-4
 * configuration file:
 * 
 * <pre>
 ...
 * file. The Sphinx-4 front end is connected to the rest of the system via the
 * scorer. We will continue with the above example to show how the scorer will
 * obtain the front end. In the configuration file, the scorer should be
...
public class FrontEnd extends BaseDataProcessor {
	...
	/**
	 * the name of the property list of all the components of the frontend pipe
	 * line
	 */
	...
	/**
	 * Sets the source of data for this front end. It basically sets the
	 * predecessor of the first DataProcessor of this front end.
	 *
	 * @param dataSource
	 *            the source of data
	 */
	...
	/**
	 * Returns the collection of <code>DataProcessor</code>s of this
	 * <code>FrontEnd</code>.
	 * 
	...
	/**
	 * Sets the source of data for this front end. It basically calls
	 * <code>setDataSource(dataSource)</code>.
	 *
	 * @param dataSource
	 *            the source of data
	 */
	...
	/**
	 * Returns the last data processor within the <code>DataProcessor</code>
	 * chain of this <code>FrontEnd</code>.
	 * 
	...
	/**
	 * Returns a description of this FrontEnd in the format: &lt;front end
	 * name&gt; {&lt;DataProcessor1&gt;, &lt;DataProcessor2&gt; ...
	 * &lt;DataProcessorN&gt;}
	 *
