...
/**
 * Defines a filter used by the {@link PLPFrequencyFilterBank} class. The filter
 * is defined by a function in the {@link #PLPFilter Constructor}. A set of
 * equally spaced frequencies in a linear scale is passed to the constructor,
 * which returns the weights for each of the frequency bins, such that the
 * filter has the shape defined by this piecewise function in the bark scale.
 *
...
public class PLPFilter {
	 ...
	 * </pre>
	 * 
	 * The current implementation assumes that the calling routine passes in an
	 * array of frequencies, one for each of the DFT points in the spectrum of
	 * the frame of speech to be filtered. This is used in conjunction with a
	 * specified center frequency to determine the filter.
	 *
	 * @param DFTFrequenciesInHz
	 *            is a double array containing the frequencies in Hertz
	 *            corresponding to each of the DFT points in the spectrum of the
	 *            signal to be filtered.
	 * @param centerFreqInHz
	 *            is the filter's center frequency
	 * @throws IllegalArgumentException
	 *             if center frequency is wrong
	 */
	...
	/**
	 * Compute the PLP spectrum at the center frequency of this filter for a
	 ...
	 * @throws IllegalArgumentException
	 *             if the input spectrum is of a different length than the array
	...
	public double filterOutput(double[] spectrum) throws IllegalArgumentException {
		...
		if (spectrum.length != numDFTPoints) {
			throw new IllegalArgumentException(
					"Mismatch in no. of DFT points " + spectrum.length + " in spectrum and in filter " + numDFTPoints);
		}
