...
public class LexTreeLinguist implements Linguist {
	 ...
	 *
	 * Alleva, F., Huang, X. and Hwang, M.-Y., "Improvements on the
	 * pronunciation prefix tree search organization", Proceedings of ICASSP,
	 * pp. 133-136, Atlanta, GA, 1996.
	 *
	...
	private SearchState getInitialSearchState() {
		...
		return new LexTreeWordState(node, node.getParent(), (new WordSequence(sentenceStartWordArray)).trim(maxDepth - 1), 0f,
				logOne, logOne);
	}
	...
	abstract class LexTreeState implements SearchState, SearchStateArc {
		...
		protected SearchStateArc createWordStateArc(WordNode wordNode, HMMNode lastUnit, LexTreeState previous) {
			...
			if (nextWord == sentenceEndWord) {
				return new LexTreeEndWordState(wordNode, lastUnit, nextWordSequence.trim(maxDepth - 1), smearTerm, logOne,
						languageProbability);
			}
		...
		SearchStateArc[] getCachedArcs() {
			if (cacheEnabled) {
				...
				if (++cacheTrys % 1000000 == 0) {
					System.out.println("Hits: " + cacheHits + " of " + cacheTrys + ' ' + ((float) cacheHits) / cacheTrys * 100f);
				}
				return arcs;
			} else {
				return null;
			...
			if (arcs == null) {
				...
				if (generateUnitStates) {
					for (int i = 0; i < nodes.length; i++) {
						arcs[i] = new LexTreeUnitState(nodes[i], getWordHistory(), getSmearTerm(), getSmearProb(), logOne, logOne,
								this.getNode());
					}
				} else {
					for (int i = 0; i < nodes.length; i++) {
	...
	public class LexTreeUnitState extends LexTreeState implements UnitSearchState {
		 ...
		 *            the history of words
		 */
		LexTreeUnitState(HMMNode hmmNode, WordSequence wordSequence, float smearTerm, float smearProb, float languageProbability,
		...
		public SearchStateArc[] getSuccessors() {
			...
			arcs[0] = new LexTreeHMMState(getHMMNode(), getWordHistory(), getSmearTerm(), getSmearProb(), hmm.getInitialState(),
					...
					for (int i = 0; i < arcs.length; i++) {
						...
						if (arc.getHMMState().isEmitting()) {
							...
							if (arc.getHMMState() == hmmState && logInsertionProbability == arc.getLogProbability()) {
								...
								nextStates[i] = new LexTreeHMMState((HMMNode) getNode(), getWordHistory(), getSmearTerm(),
										getSmearProb(), arc.getHMMState(), logOne, arc.getLogProbability(), parentNode);
							}
						} else {
							nextStates[i] = new LexTreeNonEmittingHMMState((HMMNode) getNode(), getWordHistory(), getSmearTerm(),
									getSmearProb(), arc.getHMMState(), arc.getLogProbability(), parentNode);
						}
					}
				}
	...
	public class LexTreeNonEmittingHMMState extends LexTreeHMMState {
		 ...
		 *            the word history
		 * @param probability
		 *            the probability of the transition occurring
		 * 
		 */
	...
	public class LexTreeWordState extends LexTreeState implements WordSearchState {
		 ...
		 */
		LexTreeWordState(WordNode wordNode, HMMNode lastNode, WordSequence wordSequence, float smearTerm, float smearProb,
	...
	public class LexTreeEndWordState extends LexTreeWordState implements WordSearchState {
		 ...
		 *            the probability of this word occurring
		 */
		LexTreeEndWordState(WordNode wordNode, HMMNode lastNode, WordSequence wordSequence, float smearTerm, float smearProb,
