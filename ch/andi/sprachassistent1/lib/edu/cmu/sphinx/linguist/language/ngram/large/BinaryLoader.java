...
/**
 * Reads a binary NGram language model file ("DMP file") generated by the
 * SphinxBase sphinx_lm_convert.
 * <p>
 ...
 * 10 base. They are converted to the LogMath base.
 */
...
public class BinaryLoader {
	...
	// For convenience, NG Header is regular expression, so there is 2 extra
	// characters in it.
	 ...
	 */
	public BinaryLoader(File location, String format, boolean applyLanguageWeightAndWip, float languageWeight, double wip,
	...
	/**
	 * Returns the location (or offset) into the file where NGrams start at a
	 * specified N order.
	 *
	 * @param n
	 *            the desired order
	...
	/**
	 * Returns the multiplier for the size of a NGram (1 for 16 bits, 2 for 32
	 * bits).
	 *
	 ...
	 *
	 * @param inputStream
	 *            stream to read the language model data
	 * @throws java.io.IOException
	 *             if IO went wrong
	 */
	...
	private void readHeader(DataInputStream stream) throws IOException {
		...
		if ((headerLength != DARPA_TG_HEADER.length() + 1) && (headerLength != DARPA_QG_HEADER.length() + 1)
				&& (headerLength != DARPA_NG_HEADER.length() - 1)) { // not
																		// big-endian
			headerLength = Utilities.swapInteger(headerLength);
			if (headerLength == (DARPA_TG_HEADER.length() + 1) || headerLength == (DARPA_QG_HEADER.length() + 1)
					|| headerLength == (DARPA_NG_HEADER.length() - 1)) {
				bigEndian = false;
	 ...
	 *            true if the given stream is bigEndian, false otherwise
	 * @param tableSize
	 *            the size of the NGram segment table
