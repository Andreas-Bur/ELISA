...
public abstract class SentenceHMMState implements Serializable, SearchState {
	 ...
	 *
	 * @param name
	 *            the name of the current SentenceHMMState
	 * @param parent
	 ...
	 *
	 * @return the word associated with this state, or null if there is no word
	...
	/**
	 * Determines if this state is a fan-in state. The search may need to adjust
	 * the pruning for states that fan in since they are shared by multiple
	 * paths
	 *
	 ...
	 *
	 * @param processed
	 *            the new setting for the processed flag
	 */
	...
	/**
	 * Determines if this state has been 'processed'. The meaning of 'processed'
	 * is not defined here, but is up to the higher levels
	 *
	...
	/**
	 * Connects the arc to this sentence hmm. If the node at the end of the arc
	 * is already pointing to some other node as its predecessor, don't change
	 * that relationship, since its probably a result of the nodes being reused'
	 *
	 * @param arc
	 *            the path to the next state
	 */
	...
	/**
	 * Connects the arc to this sentence hmm, but don't affect the predecessor
	 * relation ship
	 *
	 * @param arc
	...
	private void rawConnect(SentenceHMMStateArc arc) {
		...
		// attach the state-number because the state-signature is not
		// necessarily unique
	 ...
	 *
	 * @return the parent state (or null if this state does not have a parent
	 *         state).
	 */
	...
	/**
	 * Searches the set of arcs for an arc that points to a state with an
	 * identical value
	 *
	 * @param state
	 ...
	 * @param sorted
	 *            if true, states are sorted before visited
