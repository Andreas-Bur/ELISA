...
/**
 * Trie structure that contains ngrams of order 2+ in reversed order. Ngrams are
 * stored in bit array for space efficiency.
 */
...
public class NgramTrie {
	...
	public NgramTrie(int[] counts, int quantProbBoLen, int quantProbLen) {
		...
		for (int i = 1; i <= counts.length - 1; i++) {
			...
			// +8 (or +sizeof(uint64))so that reading bit array doesn't exceed
			// bounds
		...
		// Crazy backwards thing so we initialize using pointers to ones that
	 ...
	 * Search is performed in specified range. Fills range with ngram successors
	 * if ngram was found, makes range invalid otherwise.
	 * 
	 * @param ngramSet
	 *            - set of ngrams of certain order to look in
	 * @param wordId
	 *            - word id to look for
	 * @param range
	 *            - range to look in. range contains ngram successors or is
	 *            invalid after method usage.
	 * @return ngram index that can be converted into byte offset if ngram was
	 *         found, -1 otherwise
	 */
	 ...
	 * Range contains ngram successors after function execution. If ngram is not
	 * found, range will be invalid.
	 * 
	 * @param wordId
	 *            - word id to look for
	 * @param orderMinusTwo
	 ...
	 *            execution
	 * @param quant
	 *            - quantation object to decode compressed backoff stored in
	 *            trie
	...
	/**
	 * Finds ngram of cerain order in specified range and reads it's
	 * probability. Range contains ngram successors after function execution. If
	 * ngram is not found, range will be invalid.
	 * 
	 * @param wordId
	 *            - word id to look for
	 * @param orderMinusTwo
	 ...
	 *            execution
	 * @param quant
	 *            - quantation object to decode compressed probability stored in
	 *            trie
	...
	class MiddleNgramSet extends NgramSet {
		...
		MiddleNgramSet(int memPtr, int quantBits, int entries, int maxVocab, int maxNext) {
			...
			if (entries + 1 >= (1 << 25) || (maxNext >= (1 << 25)))
				throw new Error(
						"Sorry, current implementation doesn't support more than " + (1 << 25) + " n-grams of particular order");
		}
