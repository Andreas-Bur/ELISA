...
public class GDLDumper extends LinguistDumper {
	...
	public GDLDumper(String filename, Linguist linguist, boolean verticalLayout, boolean skipHMMs, boolean dumpArcLabels) {
	...
	public void newProperties(PropertySheet ps) throws PropertyException {
		...
		verticalLayout = ps.getBoolean(PROP_VERTICAL_LAYOUT);
		skipHMMs = ps.getBoolean(PROP_SKIP_HMMS);
		dumpArcLabels = ps.getBoolean(PROP_DUMP_ARC_LABELS);
		setDepthFirst(false); // breadth first traversal
	}
	...
	protected void startDumpNode(PrintStream out, SearchState state, int level) {
		...
		if (skipHMMs && (state instanceof HMMSearchState)) {
			...
			out.println("    node: {" + "title: " + qs(getUniqueName(state)) + " label: " + qs(state.toPrettyString())
					+ " color: " + color + " shape: " + shape + " vertical_order: " + level + '}');
		}
	}
	...
	protected void dumpArc(PrintStream out, SearchState from, SearchStateArc arc, int level) {
		...
		for (SearchStateArc nextArc : arcList) {
			...
			out.println("   edge: { sourcename: " + qs(getUniqueName(from)) + " targetname: "
					+ qs(getUniqueName(nextArc.getState())) + label + " color: " + color + '}');
		}
	}
	 ...
	 * used to hide the ugliness caused by trying to escape a quote character in
	 * certain syntax highlighting editors such as vim.
	 *
	 * @param s
	 *            the string to quote.
