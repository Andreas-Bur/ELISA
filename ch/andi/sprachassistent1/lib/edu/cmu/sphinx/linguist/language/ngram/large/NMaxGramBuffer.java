...
/**
 * Implements a buffer that contains NGrams of model's MAX order. It assumes
 * that the first two bytes of each n-gram entry is the ID of the n-gram.
 */
...
class NMaxGramBuffer extends NGramBuffer {
	 ...
	 * @param n
	 *            the buffer's order
	 * @param firstCurrentNGramEntry
	 *            the first Current NGram Entry
	 */
	public NMaxGramBuffer(byte[] buffer, int numberNGrams, boolean bigEndian, boolean is32bits, int n,
	...
	public int getProbabilityID(int nthFollower) {
		...
		setPosition(nthPosition + ((is32bits()) ? 4 : 2)); // to skip the word
															// ID
