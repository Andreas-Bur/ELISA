...
public class SimpleActiveListManager implements ActiveListManager {
	 ...
	 * called, this property determines whether the lists prior to that returned
	 * by next() are empty (they should be empty). If they are not empty, an
	 ...
	 *
	 * @see
	 * edu.cmu.sphinx.decoder.search.ActiveListManager#setNumStateOrder(java.
	 * lang.Class[])
	 */
		...
		private void checkPriorLists() {
			for (int i = 0; i < listPtr; i++) {
				...
				if (activeList.size() > 0) {
					throw new Error("At while processing state order" + listPtr + ", state order " + i + " not empty");
				}
			}
		}

		public void remove() {
			currentActiveLists[listPtr] = currentActiveLists[listPtr].newInstance();
		}
	}
