...
/**
 * An abstract decoder which implements all functionality which is independent
 * of the used decoding-paradigm (pull/push).
 */
public abstract class AbstractDecoder implements ResultProducer, Configurable {
	...
	/**
	 * If set to true the used search-manager will be automatically allocated in
	 * <code>newProperties()</code>.
	 */
	 ...
	 * non-final results don't trigger notification, because in most application
	 * the utterance final result will be sufficient.
	 */
	@S4Boolean(defaultValue = false)
	 ...
	 * 
	 * @param searchManager
	 *            search manager to use
	 * @param fireNonFinalResults
	 *            to fire result during decoding
	 * @param autoAllocate
	 *            automatic allocate all components
	 * @param resultListeners
	 *            listeners to get noification
	 */
	public AbstractDecoder(SearchManager searchManager, boolean fireNonFinalResults, boolean autoAllocate,
	...
	public void newProperties(PropertySheet ps) throws PropertyException {
		init(ps.getInstanceName(), ps.getLogger(), (SearchManager) ps.getComponent(PROP_SEARCH_MANAGER),
				ps.getBoolean(FIRE_NON_FINAL_RESULTS), ps.getBoolean(AUTO_ALLOCATE),
				ps.getComponentList(PROP_RESULT_LISTENERS, ResultListener.class));
	}

	private void init(String name, Logger logger, SearchManager searchManager, boolean fireNonFinalResults, boolean autoAllocate,
