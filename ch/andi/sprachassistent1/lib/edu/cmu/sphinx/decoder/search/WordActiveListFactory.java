...
/**
 * A factory for WordActiveList. The word active list is active list designed to
 * hold word tokens only. In addition to the usual active list properties such
 * as absolute and relative beams, the word active list allows restricting the
 * number of copies of any particular word in the word beam. Also the word
 * active list can restrict the number of fillers in the beam.
 */
public class WordActiveListFactory extends ActiveListFactory {

	/**
	 * property that sets the max paths for a single word. (zero disables this
	 * feature)
	 */
	...
	/**
	 * property that sets the max filler words allowed in the beam. (zero
	...
	class WordActiveList implements ActiveList {
		 ...
		 *
		 * @param oldToken
		 *            the token to replace (or null in which case, replace works
		 *            like add).
		 * @param newToken
