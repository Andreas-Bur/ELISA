...
public interface ActiveList extends Iterable<Token> {
	 ...
	 * This is sometimes referred to as the beam size
	 */
	@S4Integer(defaultValue = 2000)
	public final static String PROP_ABSOLUTE_BEAM_WIDTH = "absoluteBeamWidth";

	/**
	 * Property that sets the minimum score relative to the maximum score in the
	 * list for pruning. Tokens with a score less than relativeBeamWidth *
	 * maximumScore will be pruned from the list
	 */
	@S4Double(defaultValue = 0.0)
	public final static String PROP_RELATIVE_BEAM_WIDTH = "relativeBeamWidth";
	/**
	 * Property that indicates whether or not the active list will implement
	 * 'strict pruning'. When strict pruning is enabled, the active list will
	 * not remove tokens from the active list until they have been completely
	 * scored. If strict pruning is not enabled, tokens can be removed from the
	 * active list based upon their entry scores. The default setting is false
	 * (disabled).
	 */
	...
	/**
	 * Adds the given token to the list, keeping track of the lowest scoring
	 * token
	 *
	 * @param token
	 *            the token to add
	...
	/**
	 * Purges the active list of excess members returning a (potentially new)
	 * active list
	 *
	...
	/**
	 * Creates a new empty version of this active list with the same general
	 * properties.
	 *
