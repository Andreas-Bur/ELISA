...
public class Fst {
	...
	public static Fst loadModel(String filename) throws IOException, ClassNotFoundException {
		...
		System.err.println("Load Time: " + (Calendar.getInstance().getTimeInMillis() - starttime) / 1000.);
		return obj;
	}
	...
	@Override
	public int hashCode() {
		return 31 * (Arrays.hashCode(isyms) + 31 * (Arrays.hashCode(osyms) + 31 * ((start == null ? 0 : start.hashCode())
				+ 31 * ((states == null ? 0 : states.hashCode()) + 31 * ((semiring == null ? 0 : semiring.hashCode()))))));
	}
	...
	public String toString() {
		...
		sb.append("Fst(start=" + start + ", isyms=" + Arrays.toString(isyms) + ", osyms=" + Arrays.toString(osyms) + ", semiring="
				+ semiring + ")\n");
		int numStates = states.size();
		for (int i = 0; i < numStates; i++) {
			State s = states.get(i);
