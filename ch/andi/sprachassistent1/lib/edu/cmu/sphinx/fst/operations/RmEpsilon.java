	...
	public static Fst get(Fst fst) {
		...
		for (int i = 0; i < numStates; i++) {
			...
			for (int j = 0; j < numArcs; j++) {
				...
				if ((a.getIlabel() != 0) || (a.getOlabel() != 0)) {
					newState.addArc(
							new Arc(a.getIlabel(), a.getOlabel(), a.getWeight(), oldToNewStateMap[a.getNextState().getId()]));
				}
			}
				...
				for (State pathFinalState : cl[oldState.getId()].keySet()) {
					...
					if (s1.getFinalWeight() != semiring.zero()) {
						s.setFinalWeight(semiring.plus(s.getFinalWeight(),
								semiring.times(getPathWeight(oldState, s1, cl), s1.getFinalWeight())));
					}
					...
					for (int j = 0; j < numArcs; j++) {
						...
						if ((a.getIlabel() != 0) || (a.getOlabel() != 0)) {
							Arc newArc = new Arc(a.getIlabel(), a.getOlabel(),
									semiring.times(a.getWeight(), getPathWeight(oldState, s1, cl)),
