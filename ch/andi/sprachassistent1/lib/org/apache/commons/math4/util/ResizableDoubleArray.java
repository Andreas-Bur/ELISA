...
public class ResizableDoubleArray implements DoubleArray, Serializable {
	 ...
	 * {@code double[]} array.
	 * <p>
	 * If the supplied array is null, a new empty array with the default initial
	 * capacity will be created. The input array is copied, not referenced.
	 * Other properties take default values:
	 * <ul>
	 * <li>{@code expansionMode = MULTIPLICATIVE}</li>
	 * <li>{@code expansionFactor = 2.0}</li>
	...
	public ResizableDoubleArray(double[] initialArray) {
		this(initialArray == null || initialArray.length == 0 ? DEFAULT_INITIAL_CAPACITY : initialArray.length,
				DEFAULT_EXPANSION_FACTOR, DEFAULT_CONTRACTION_DELTA + DEFAULT_EXPANSION_FACTOR, DEFAULT_EXPANSION_MODE,
				initialArray);
	}
	 ...
	 * Copy constructor.
	 * <p>
	 * Creates a new ResizableDoubleArray that is a deep, fresh copy of the
	 ...
	 * the array. Returns the discarded first element.
	 * <p>
	 ...
	 * <p>
	 * Returns the value that has been replaced. If the array is empty (i.e. if
	 * {@link #numElements} is zero), an MathIllegalStateException is thrown.
	 *
	 * @param value
	 ...
	 * {@code discardFrontElements(2)} will cause the first two elements to be
	 * discarded, leaving 3,4 in the array.
	 *
	 * @param i
	 ...
	 * <p>
	 * For example, if the array contains the elements 1,2,3,4, invoking
	 * {@code discardMostRecentElements(2)} will cause the last two elements to
	 * be discarded, leaving 1,2 in the array.
	 *
	 * @param i
	...
	/**
	 * Discards the {@code i} first or last elements of the array, depending on
	 * the value of {@code front}.
	 * <p>
	 * For example, if the array contains the elements 1,2,3,4, invoking
	 * {@code discardExtremeElements(2,false)} will cause the last two elements
	 * to be discarded, leaving 1,2 in the array. For example, if the array
	 ...
	 * <p>
	 * If {@code expansionMode} is set to MULTIPLICATIVE, the new array size
	 * will be {@code internalArray.length * expansionFactor}. If
	 * {@code expansionMode} is set to ADDITIVE, the length after expansion will
	 * be {@code internalArray.length + expansionFactor}.
	 */
	 ...
	 * <p>
	 * If the {@code expansionMode} is {@code MULTIPLICATIVE}, contraction is
	 * triggered when the ratio between storage array length and
	 * {@code numElements} exceeds {@code contractionFactor}. If the
	 * {@code expansionMode} is {@code ADDITIVE}, the number of excess storage
	 ...
	 *             if {@code index} is less than zero or is greater than
	 *             {@code getNumElements() - 1}.
	 */
	@Override
	 ...
	 * <p>
	 * This method returns a copy, not a reference to the underlying array, so
	 * that changes made to the returned array have no effect on this
	 * ResizableArray.
	 *
	 ...
	 * multiplied by the {@code expansionFactor} (MULTIPLICATIVE) or if the
	 * expansion is additive (ADDITIVE -- {@code expansionFactor} storage
	 * locations added). The default {@code expansionMode} is MULTIPLICATIVE and
	 * the default {@code expansionFactor} is 2.0.
	 *
	 ...
	 * (available via the {@link #getStartIndex() getStartIndex} method.
	 * <p>
	 ...
	 * lead to this object becoming inconsistent.
	 * <p>
	 ...
	 * <p>
	 * If the specified index is greater than {@code getNumElements() - 1}, the
	 * {@code numElements} property is increased to {@code index +1} and
	 * additional storage is allocated (if necessary) for the new element and
