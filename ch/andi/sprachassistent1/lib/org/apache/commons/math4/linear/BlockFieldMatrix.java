...
public class BlockFieldMatrix<T extends FieldElement<T>> extends AbstractFieldMatrix<T> implements Serializable {
				...
				for (int kBlock = 0; kBlock < blockColumns; ++kBlock) {
					...
					for (int p = pStart; p < pEnd; ++p) {
						...
						for (int nStart = 0; nStart < jWidth; ++nStart) {
							...
							int n = nStart;
							while (l < lEnd - 3) {
								sum = sum.add(tBlock[l].multiply(mBlock[n])).add(tBlock[l + 1].multiply(mBlock[n + jWidth]))
	...
	public FieldMatrix<T> getSubMatrix(final int startRow, final int endRow, final int startColumn, final int endColumn)
		...
		final BlockFieldMatrix<T> out = new BlockFieldMatrix<>(getField(), endRow - startRow + 1, endColumn - startColumn + 1);
			...
			for (int jBlock = 0; jBlock < out.blockColumns; ++jBlock) {
				...
				if (heightExcess > 0) {
					...
					if (widthExcess > 0) {
						...
						copyBlockPart(blocks[index], width, rowsShift, BLOCK_SIZE, columnsShift, BLOCK_SIZE, outBlock, jWidth, 0,
								0);
						copyBlockPart(blocks[index + 1], width2, rowsShift, BLOCK_SIZE, 0, widthExcess, outBlock, jWidth, 0,
								jWidth - widthExcess);
